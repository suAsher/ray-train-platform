  ...
  },
  {
    ...
    blocks: [
      ...
      },
	  {
		kind: 'note',
		title: 'GPU 卡型由团队自动决定',
		text: '用户无需指定 4090、A100、A800 或 H20。平台会把当前团队绑定的资源池固化进任务，同一分布式任务不会混用异构 GPU。旧客户端携带的卡型值会被团队策略覆盖。',
	  },
	  {
		kind: 'warning',
		title: '闲时任务不是分布式任务',
		text: '闲时任务固定为 1 Worker × 1 GPU、最低优先级，必须使用 Ray Train 托管恢复和有效 Checkpoint。团队正常任务需要 GPU 时它会让出，之后重新排队续训。能力未经抢占恢复演练时，提交入口保持禁用。',
	  },
      {
        kind: 'table',
        headers: ['规模字段', '取值约束'],
