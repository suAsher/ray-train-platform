...
${Object.entries(nativeStreamingMetadata).map(([key, value]) => `      '${key}' = '${value}'`).join('\n')}
    ...
    { kind: 'steps', items: [
      ...
      { title: '验收真实双盘并生成审阅材料', body: '/data1 与 /data2 必须独立挂载，准备各自 ray-cache 目录及权限。脚本读取两套线上配置，保留全部旧节点，检查挂载、容量与写删探针；输出目录必须不存在，失败不产生可应用补丁。', code: 'bash ops/storage/nvme-cache/register-node.sh --node NODE_NAME --output-dir /tmp/nvme-NODE_NAME-review', codeLang: 'bash' },
