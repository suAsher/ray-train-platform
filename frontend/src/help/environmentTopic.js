export const environmentTopic = {
  ...
  blocks: [
    { kind: 'note', text: '新通用底座的计划仓库名：harbor.wellspiking.ai/guofeng.su/raytrain-base。当前构建定义目标为 Python 3.10、CUDA 12.1、PyTorch 2.4.1、Ray 2.58.0、MLflow skinny 3.14.0、PyArrow 25.0.1；尚未发布，实际可用 tag / digest 与环境报告由管理员在构建验证后提供。不要直接使用无 tag 的计划地址。' },
    ...
    { kind: 'steps', items: [
      ...
      { title: '选择平台基础环境', body: '通用 raytrain-base 的构建定义与派生模板由平台维护。发布前不能把建议 tag 当可拉取镜像；向管理员取得已发布的完整基础地址、版本约束与配套自检工具。通用底座不含 BEVFusion 等项目代码，现有模型专用镜像不是通用底座。' },
...
BASE_IMAGE='REPLACE_WITH_PUBLISHED_PLATFORM_BASE'
