#!/usr/bin/env python3
"""Download a fixed serving code snapshot using only a mounted job token.

This generic tool contains no model adapter code. It never follows redirects,
...
def _endpoint(base_url, job_id):
    ...
    if not isinstance(job_id, str) or not re.fullmatch(r'job-[0-9a-f]{24}', job_id):
        raise ValueError('invalid serving job ID')
    return base_url + '/jobs/' + job_id + '/model-serving/code'
...
def download(*, base_url, job_id, token_file, sha256, size_bytes, output):
    ...
    try:
        try:
            ...
            error.close()
            raise DownloadError('serving code request rejected (HTTP %d)' % error.code) from None
        with response:
            if response.code != 200:
                raise DownloadError('serving code request returned an unexpected status')
            digest, total = hashlib.sha256(), 0
            with tempfile.NamedTemporaryFile(dir=output.parent, prefix='.serving-code-', delete=False) as destination:
                temporary = Path(destination.name)
                while True:
                    if time.monotonic() - started > 180:
                        raise DownloadError('serving code download exceeded its deadline')
                    chunk = response.read(min(1024 * 1024, size_bytes - total + 1))
                    if not chunk:
                        break
                    total += len(chunk)
                    if total > size_bytes:
                        raise DownloadError('serving code size exceeds the frozen snapshot')
                    digest.update(chunk)
                    destination.write(chunk)
                if total != size_bytes or digest.hexdigest() != sha256:
                    raise DownloadError('serving code size or SHA-256 does not match the frozen snapshot')
                destination.flush()
        ...
        raise DownloadError('serving code transfer or local write failed') from None
    finally:
        if temporary is not None:
            try:
                temporary.unlink(missing_ok=True)
            except OSError:
                raise DownloadError('serving code temporary file cleanup failed') from None
...
def main(argv=None):
    ...
    try:
        ...
        print('Serving code materialization failed; check job state and its fixed code snapshot.', file=sys.stderr)
        return 1
