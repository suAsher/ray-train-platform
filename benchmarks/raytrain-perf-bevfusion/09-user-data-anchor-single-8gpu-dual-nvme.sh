...
spk-rayjob submit \
  ...
  --name "perf-user-anchor-1x8-dual-${RUN_ID}" \
  ...
  --entrypoint 'python3 training_benchmark.py --config configs/westwell_fix_anchor/det/anchorhead/secfpn/lidar/voxelnet_0p075.yaml --train pkl/0825_pkl/mxg128/merged_nuscenes_infos_train.pkl --val pkl/0825_pkl/mxg128/merged_nuscenes_infos_val.pkl --train-limit 4096 --val-limit 512 --samples-per-gpu 8 --workers-per-gpu 8 --learning-rate 0.0001 --recorded-root /mnt/storage/me --stage-cache --copy-workers 32 --timeout 14400' \
  --input-space my-storage \
  --input-path '' \
  --output-path "benchmarks/perf-user-anchor-1x8-dual-${RUN_ID}" \
  --workers 1 \
  --gpus-per-worker 8 \
  --cpu-per-worker 64 \
  ...
  --cache-mode runtime \
  --cache-size 5Ti \
  --watch 2>&1 | tee "runs/09-user-data-anchor-single-8gpu-dual-nvme-${RUN_ID}.log"
