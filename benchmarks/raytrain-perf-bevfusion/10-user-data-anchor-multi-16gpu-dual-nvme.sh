...
spk-rayjob submit \
  ...
  --name "perf-user-anchor-2x8-dual-${RUN_ID}" \
  ...
  --input-path '' \
  --output-path "benchmarks/perf-user-anchor-2x8-dual-${RUN_ID}" \
  --workers 2 \
  --gpus-per-worker 8 \
  --cpu-per-worker 64 \
  ...
  --cache-mode runtime \
  --cache-size 5Ti \
  --watch 2>&1 | tee "runs/10-user-data-anchor-multi-16gpu-dual-nvme-${RUN_ID}.log"
