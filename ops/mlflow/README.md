...
本次故障的直接现象是 PVC 在特定节点挂载时返回 `input/output error`，而 FSX Agent 和 CSI Pod 仍可能显示 Running。节点本地 FUSE 会话已经失效，但还必须单独排查触发因素。本次现场同时观测到两台 IDC DNS 对 FSX 所需 TOS endpoint 出现超时，因此 DNS 不稳定是高概率触发因素，不能再排除。即使 DNS 恢复，已失效的 FUSE bind mount 也不会自动恢复。如果 Agent 日志同时出现 `fusedaemon uds not ready within timeout`、`MODULE_STILL_RUNNING` 或 `/usr/bin/fsx` 缺失，则是 Agent 恢复过程未完成，不应直接重启 MLflow。

恢复顺序必须是：

1. 先确认 `mlflow-fsx-dns-probe` 全部 Ready，再用 `mlflow-fsx-probe` 和 Pod Event 确认出问题的节点。DNS 未恢复时不要反复重启 FSX。
...
kubectl -n mlflow-system get ds,pod -l app.kubernetes.io/name=mlflow-fsx-dns-probe -o wide
