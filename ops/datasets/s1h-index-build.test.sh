...
grep -Fq 'trusted-index-v3.json' "$runner" || fail 'job must publish the multimodal platform index contract'
grep -Fq 'trusted-index-v3.parts' "$runner" || fail 'job must publish bounded content-addressed index parts'
grep -Fq '.raytrain/indexes/$version/trusted-index-v3.json' "$runner" || fail 'each dataset version must receive an immutable index root'
grep -Fq -- '--format multimodal-v2' "$runner" || fail 'job must explicitly select the multimodal index contract'
