...
func NewService(store Store, source Source, clients map[fw.Environment]Upstream, pepper []byte)(*Service,error) {
	if store==nil||source==nil||len(clients)==0{return nil,ErrInvalid}
	box,err:=newCredentialBox(pepper)
	if err!=nil{return nil,err}
	copyClients:=make(map[fw.Environment]Upstream,len(clients))
	for env,client:=range clients {
		if (env!=fw.Production&&env!=fw.Development)||client==nil{return nil,ErrInvalid}
...
func validRequest(actor Actor, request Request) bool {
	if actor.ID==""||actor.TenantID==""||!restrictedID.MatchString(request.JobID)||!restrictedID.MatchString(request.WarehouseID)||!restrictedID.MatchString(request.ModelTypeID)||!restrictedID.MatchString(request.IdempotencyKey){return false}
	...
	if len(request.Paths)<1||len(request.Paths)>8{return false}
	names:=make(map[string]bool,len(request.Paths))
	for _,value:=range request.Paths {
		if value==""||len(value)>1024||!utf8.ValidString(value)||strings.ContainsAny(value,"\\\x00\r\n")||path.IsAbs(value)||path.Clean(value)!=value||value==".."||strings.HasPrefix(value,"../"){return false}
		name:=path.Base(value)
		if names[name]||len(name)>255 {return false}
		names[name]=true
		switch strings.ToLower(path.Ext(name)){case ".pth",".pt",".ckpt",".safetensors",".onnx":default:return false}
	}
	return true
}

func (s *Service) Create(ctx context.Context,actor Actor,request Request,token string)(Operation,error) {
	if !validRequest(actor,request){return Operation{},ErrInvalid}
	client,ok:=s.clients[request.Environment]
	if !ok{return Operation{},ErrInvalid}
	...
	if err!=nil{return Operation{},fw.ErrUnauthorized}
	if err:=checkTarget(ctx,client,token,op);err!=nil{return Operation{},safeError(err)}
...
func (s *Service) Retry(ctx context.Context,id string,actor Actor,token string)(Operation,error) {
	...
	if (op.State!=Failed&&op.State!=WaitingReauth)||op.TargetVersionID!=""{return Operation{},ErrConflict}
	client:=s.clients[op.Environment]
	if client==nil{return Operation{},ErrInvalid}
	credential,err:=s.credentials.seal(op,token)
	if err!=nil{return Operation{},fw.ErrUnauthorized}
	if err:=checkTarget(ctx,client,token,op);err!=nil{return Operation{},safeError(err)}
...
func (s *Service) Cancel(ctx context.Context,id string,actor Actor)(Operation,error) {
	op,err:=s.owned(ctx,id,actor)
	if err!=nil{return Operation{},err}
	if op.State!=Queued&&op.State!=WaitingSource&&op.State!=WaitingReauth&&op.State!=Uploading{return Operation{},ErrConflict}
...
func (s *Service) ListJob(ctx context.Context,actor Actor,jobID string)([]Operation,error) {
	if actor.ID==""||actor.TenantID==""||!restrictedID.MatchString(jobID){return nil,ErrInvalid}
...
func (s *Service) owned(ctx context.Context,id string,actor Actor)(Operation,error) {
	if !restrictedID.MatchString(id)||actor.ID==""||actor.TenantID==""{return Operation{},ErrInvalid}
	op,err:=s.store.Get(ctx,id)
	if err!=nil{return Operation{},safeError(err)}
	if op.OwnerID!=actor.ID||op.TenantID!=actor.TenantID{return Operation{},ErrForbidden}
	return op,nil
}
...
func checkTarget(ctx context.Context,client Upstream,token string,op Operation)error {
	...
	group:=""
	for _,target:=range fw.Environments(){if target.Environment==op.Environment{group=target.GroupID}}
	if warehouse.ID!=op.WarehouseID||warehouse.GroupID!=group||group==""{return fw.ErrForbidden}
	canEdit:=false
	for _,permission:=range warehouse.PermissionCodes{if permission=="edit"||permission=="admin"{canEdit=true}}
	if !canEdit{return fw.ErrForbidden}
	models,err:=client.ListModelTypes(ctx,token,op.WarehouseID)
	if err!=nil{return err}
	for _,model:=range models {
		if model.ID==op.ModelTypeID&&(model.GroupID==""||model.GroupID==group)&&(model.FunctionWarehouseID==""||model.FunctionWarehouseID==op.WarehouseID){return nil}
	}
...
func (s *Service) Run(ctx context.Context) {
	for ctx.Err()==nil {
		...
		if err==nil {s.process(ctx,op);continue}
...
func (s *Service) process(parent context.Context,op Operation) {
	...
	if op.State==Registering||op.TargetVersionID!=""{s.finish(op,Unknown);return}
	token,err:=s.credentials.open(op)
	if err!=nil{s.finish(op,WaitingReauth);return}
	client:=s.clients[op.Environment]
	if client==nil{s.finish(op,Failed);return}
	if err:=checkTarget(ctx,client,token,op);err!=nil{s.fail(op,err);return}
	files,identity,err:=s.source.Prepare(ctx,op)
	if err!=nil{s.fail(op,err);return}
	if !validSource(op,files,identity){s.finish(op,Failed);return}
	op.Files=append([]File(nil),files...)
	op.RunID=identity.RunID;op.ExperimentID=identity.ExperimentID
	if !s.saveStage(ctx,&op,Uploading){return}
	...
	if err!=nil{s.fail(op,err);return}
	if !sameSource(files,identity,currentFiles,currentIdentity){s.finish(op,Failed);return}
	if err=checkTarget(ctx,client,token,op);err!=nil{s.fail(op,err);return}
	...
	if ctx.Err()!=nil||s.store.Renew(ctx,op.ID,op.LeaseID,time.Now().UTC().Add(leaseDuration))!=nil{return}
	if !s.saveStage(ctx,&op,Registering){return}
	if ctx.Err()!=nil{s.finish(op,Unknown);return}
	...
	if version.ID!=""{op.TargetVersionID=version.ID}
	if err!=nil {
		if op.TargetVersionID!=""||errors.Is(err,fw.ErrUnknownOutcome)||ctx.Err()!=nil{s.finish(op,Unknown)}else{s.fail(op,err)}
		return
	}
	if op.TargetVersionID==""{s.finish(op,Unknown);return}
	if !s.saveStage(ctx,&op,Registering){return}
	if _,err=client.VerifyVersion(ctx,token,op.WarehouseID,op.TargetVersionID,request);err!=nil{s.finish(op,Unknown);return}
...
func (s *Service) renewLease(ctx context.Context,cancel context.CancelFunc,op Operation,done chan<- struct{}) {
	...
	for {
		...
		case <-ticker.C:
			...
			err:=s.store.Renew(renewCtx,op.ID,op.LeaseID,time.Now().UTC().Add(leaseDuration));release()
...
func (s *Service) upload(ctx context.Context,client Upstream,token string,op Operation)([]fw.UploadedFile,error) {
	...
	for _,file:=range op.Files {
		...
		if err:=checkTarget(ctx,client,token,op);err!=nil{return nil,err}
		...
		if err!=nil{return nil,err}
		if uploaded.Filename!=file.Name||uploaded.FileSize!=file.Size||!strings.EqualFold(uploaded.FileSHA256,file.SHA256){return nil,ErrInvalid}
...
func validSource(op Operation,files []File,identity SourceIdentity)bool {
	if identity.JobID!=op.JobID||!restrictedID.MatchString(identity.RunID)||!restrictedID.MatchString(identity.ExperimentID)||len(files)!=len(op.Paths){return false}
	names:=make(map[string]bool,len(op.Paths));for _,value:=range op.Paths{names[path.Base(value)]=true}
	for _,file:=range files {
		if !names[file.Name]||!restrictedID.MatchString(file.ModelID)||!storageSegment.MatchString(file.VersionID)||file.Size<=0||file.Size>fw.MaxUploadSize||len(file.SHA256)!=64||strings.ToLower(file.SHA256)!=file.SHA256{return false}
		if _,err:=hex.DecodeString(file.SHA256);err!=nil{return false}
...
func sameSource(files []File,identity SourceIdentity,otherFiles []File,otherIdentity SourceIdentity)bool {
	if identity!=otherIdentity||len(files)!=len(otherFiles){return false}
	counts:=make(map[File]int,len(files));for _,file:=range files{counts[file]++}
	for _,file:=range otherFiles{if counts[file]==0{return false};counts[file]--}
	return true
}

func (s *Service) saveStage(ctx context.Context,op *Operation,state string)bool {
	next:=*op;next.State=state;next.Message=stateMessage(state);next.UpdatedAt=time.Now().UTC()
	if ctx.Err()!=nil||s.store.Save(ctx,next,op.LeaseID)!=nil{return false}
	*op=next;return true
}

func (s *Service) finish(op Operation,state string) {
	lease:=op.LeaseID
	op.State=state;op.Message=stateMessage(state);op.UpdatedAt=time.Now().UTC();op.LeaseID="";op.LeaseExpiresAt=nil
	if state==WaitingSource{op.NextAttemptAt=time.Now().UTC().Add(30*time.Second)}else{op.Credential=nil}
...
func (s *Service) fail(op Operation,err error) {
	switch {
	case errors.Is(err,ErrPending):s.finish(op,WaitingSource)
	case errors.Is(err,fw.ErrUnauthorized):s.finish(op,WaitingReauth)
	default:s.finish(op,Failed)
	}
}

func stateMessage(state string)string {
	switch state {
	case Queued:return "等待同步"
	case WaitingSource:return "等待训练完成或权重副本就绪"
	case Uploading:return "正在上传模型文件"
	case Registering:return "正在创建功能仓版本并核对训练来源"
	case Succeeded:return "模型文件及训练来源已同步并核验"
	case WaitingReauth:return "登录授权已失效，请重新登录后重试同步"
	case Unknown:return "创建结果待确认，请先检查功能仓，避免重复创建"
	case Canceled:return "同步已取消"
	default:return "同步未完成，请检查文件、训练来源或目标权限后重试"
	}
}

func safeError(err error)error {
	if err==nil{return nil}
	for _,known:=range []error{ErrInvalid,ErrForbidden,ErrNotFound,ErrConflict,ErrQuota,fw.ErrUnauthorized,fw.ErrForbidden,fw.ErrInvalid,fw.ErrRateLimited}{if errors.Is(err,known){return known}}
