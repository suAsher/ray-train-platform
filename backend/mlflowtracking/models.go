...
type Actor struct { TenantID string; UserID string }
...
type Pair struct { Key string `json:"key"`; Value string `json:"value"` }
type Metric struct {
	Key string `json:"key"`
...
type MetricPoint struct { Value float64 `json:"value"`;TimestampMS int64 `json:"timestampMs"`;Step int64 `json:"step"` }
type MetricSeries struct {Key string `json:"key"`;Points []MetricPoint `json:"points"`}
...
type ExperimentPage struct {Items []Experiment `json:"items"`;NextCursor string `json:"nextCursor,omitempty"`}
type RunPage struct {Items []Run `json:"items"`;NextCursor string `json:"nextCursor,omitempty"`}
