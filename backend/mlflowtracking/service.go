...
func (s *Service) ready(actor Actor) error {
	...
	if s == nil || s.store == nil || s.provider == nil || len(s.key) < 32 {
...
func (s *Service) CreateExperiment(ctx context.Context, actor Actor, key, name string) (Experiment, error) {
	...
	var upstream string
	if claimed {
		upstream, err = s.provider.CreateExperiment(ctx, record.ID)
	} else {
		var found bool
		upstream, found, err = s.provider.FindExperiment(ctx, record.ID)
		if err == nil && !found {
...
func (s *Service) CreateRun(ctx context.Context, actor Actor, experimentID, key, name string) (Run, error) {
	...
	if !validID(experimentID) || !validKey(key) || !validName(name) {
		return Run{}, ErrInvalid
	}
	experiment, err := s.store.GetExperiment(ctx, actor, experimentID)
	if err != nil {
		return Run{}, err
	}
	...
	if record.Name != name || record.ExperimentID != experimentID {
	...
	var upstream string
	if claimed {
		upstream, err = s.provider.CreateRun(ctx, experiment.UpstreamID, record.ID, name)
	} else {
		var found bool
		upstream, found, err = s.provider.FindRun(ctx, experiment.UpstreamID, record.ID)
		if err == nil && !found {
			return record, ErrPending
		}
	}
	if err != nil || !validID(upstream) {
		return record, ErrPending
	}
...
func (s *Service) GetRun(ctx context.Context, actor Actor, id string) (RunDetail, error) {
	...
	record, experiment, err := s.runAndExperiment(ctx, actor, id)
	if err != nil {
		return RunDetail{}, err
	}
	if record.State == "PENDING" {
		return RunDetail{}, ErrPending
	}
	ctx, cancel := context.WithTimeout(ctx, 20*time.Second)
	defer cancel()
...
func (s *Service) runAndExperiment(ctx context.Context, actor Actor, id string) (Run, Experiment, error) {
	record, err := s.store.GetRun(ctx, actor, id)
	if err != nil {
		return Run{}, Experiment{}, err
	}
	...
	if experiment.State != "READY" {
		return Run{}, Experiment{}, ErrPending
	}
...
func (s *Service) LogRun(ctx context.Context, actor Actor, id string, batch Batch) error {
	...
	_, experiment, err := s.runAndExperiment(ctx, actor, id)
	if err != nil {
		return err
	}
	leaseID, err := s.newID()
	...
	now := s.now().UTC()
	record, err := s.store.ClaimRunLease(ctx, actor, id, leaseID, "", now, now.Add(time.Minute), 0)
	if err != nil {
		return err
	}
	callCtx, cancel := context.WithTimeout(ctx, 20*time.Second)
	callErr := s.provider.LogRun(callCtx, experiment.UpstreamID, record.UpstreamID, record.ID, batch)
	cancel()
	_, releaseErr := s.release(ctx, actor, id, leaseID, "")
	if releaseErr != nil {
		return fmt.Errorf("%w: mutation outcome could not be persisted", ErrUnavailable)
	}
	return callErr
}
...
func (s *Service) FinishRunAt(ctx context.Context, actor Actor, id, status string, endTimeMS int64) (Run, error) {
	...
	if !validID(id) || !terminal(status) || endTimeMS < 0 || endTimeMS > 253402300799999 {
		return Run{}, ErrInvalid
	}
	before, experiment, err := s.runAndExperiment(ctx, actor, id)
	if err != nil {
		return Run{}, err
	}
	if endTimeMS < before.StartTimeMS {
		return Run{}, ErrInvalid
	}
	leaseID, err := s.newID()
	if err != nil {
	...
	now := s.now().UTC()
	record, err := s.store.ClaimRunLease(ctx, actor, id, leaseID, status, now, now.Add(time.Minute), endTimeMS)
	if err != nil {
		return Run{}, err
	}
	...
	if errors.Is(callErr, ErrConflict) {
		...
		if readErr == nil && terminal(snapshot.Status) {
			finishedStatus = snapshot.Status
		}
	}
	cancel()
	...
	if errors.Is(callErr, ErrConflict) && finishedStatus != "" {
		return updated, ErrConflict
	}
	if callErr != nil {
		return updated, ErrPending
	}
	return updated, nil
}
