...
func TestBatchValidationProtectsOwnershipAndFiniteValues(t *testing.T){
	...
	}{if err:=batch.Validate();!errors.Is(err,ErrInvalid){t.Fatalf("invalid batch=%+v %v",batch,err)}}
	for _,invalid:=range []float64{math.NaN(),math.Inf(1),math.Inf(-1)}{
		if err:=(Batch{Metrics:[]Metric{{Key:"loss",Value:&invalid,Timestamp:&timestamp,Step:&step}}}).Validate();!errors.Is(err,ErrInvalid){t.Fatalf("nonfinite metric accepted: %v",err)}
	}
	...
	for _,raw:=range []string{`{"key":"x"}`,`{"key":"x","value":null}`,`{"key":"x","value":1}`,`{"key":"x","value":"v","extra":1}`} {var pair Pair;if json.Unmarshal([]byte(raw),&pair)==nil{t.Fatalf("invalid pair=%s",raw)}}
	var pair Pair;if err:=json.Unmarshal([]byte(`{"key":"x","value":""}`),&pair);err!=nil{t.Fatal(err)}
}
