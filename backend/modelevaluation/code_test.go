...
func TestEvaluatorSupportsExactlyOneImmutableCodeSource(t *testing.T) {
	git := validEvaluator()
	if err := ValidateEvaluator(git); err != nil { t.Fatal(err) }
	code := validCodeSnapshot()
	uploaded := git
	uploaded.Code, uploaded.GitURL, uploaded.GitCommit = &code, "", ""
	if err := ValidateEvaluator(uploaded); err != nil { t.Fatal(err) }
	for _, change := range []func(*Evaluator){
		...
		e := uploaded; change(&e)
		if !errors.Is(ValidateEvaluator(e), ErrInvalid) { t.Fatalf("ambiguous or absent source accepted: %+v", e) }
	}
}
...
func TestCodeSnapshotBoundsAndIdentifiers(t *testing.T) {
	for _, change := range []func(*CodeSnapshot){
		...
		code := validCodeSnapshot(); change(&code)
		if !errors.Is(ValidateCodeSnapshot(code), ErrInvalid) { t.Fatalf("invalid snapshot accepted: %+v", code) }
	}
	code := validCodeSnapshot(); code.SizeBytes = MaxEvaluationCodeSize
	if err := ValidateCodeSnapshot(code); err != nil { t.Fatal(err) }
}

func TestComparisonFingerprintIncludesCodeSnapshotIdentity(t *testing.T) {
	e := validEvaluation(); code := validCodeSnapshot()
	e.Evaluator.Code, e.Evaluator.GitURL, e.Evaluator.GitCommit = &code, "", ""
	baseline := ComparisonFingerprint(e)
	if baseline == "" { t.Fatal("valid code evaluator has no fingerprint") }
	for _, change := range []func(*CodeSnapshot){
		...
		other := e; different := code; change(&different); other.Evaluator.Code = &different
		if hash := ComparisonFingerprint(other); hash == "" || hash == baseline { t.Fatal("code source change did not change fingerprint") }
	}
	if ComparisonFingerprint(validEvaluation()) == baseline { t.Fatal("uploaded code confused with legacy Git source") }
}
