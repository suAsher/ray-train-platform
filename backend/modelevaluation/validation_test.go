...
func TestCanonicalConfigStableAndExact(t *testing.T) {
	a,hashA,err:=CanonicalConfig(json.RawMessage(`{"z":1.0,"a":{"b":1e1,"x":-0}}`));if err!=nil{t.Fatal(err)}
	b,hashB,err:=CanonicalConfig(json.RawMessage(` { "a":{"x":0,"b":10}, "z":1 } `));if err!=nil{t.Fatal(err)}
	if string(a)!=string(b)||hashA!=hashB||len(hashA)!=64{t.Fatalf("not canonical: %s %s",a,b)}
	_,largeA,err:=CanonicalConfig(json.RawMessage(`{"n":9007199254740992}`));if err!=nil{t.Fatal(err)}
	_,largeB,err:=CanonicalConfig(json.RawMessage(`{"n":9007199254740993}`));if err!=nil||largeA==largeB{t.Fatal("large integers rounded into same config")}
}
func TestCanonicalConfigRejectsAmbiguousOrUnboundedInput(t *testing.T) {
	for _,raw:=range []string{`[]`,`null`,`{"x":1,"x":2}`,`{"x":{"a":1,"a":2}}`,`{} {}`,`{"x":NaN}`,`{"x":1e999}`,`{"x":"`+strings.Repeat("a",MaxConfigBytes)+`"}`,strings.Repeat(`{"a":`,40)+`0`+strings.Repeat(`}`,40)} {
		if _,_,err:=CanonicalConfig(json.RawMessage(raw));!errors.Is(err,ErrInvalid){t.Fatalf("accepted bad config %.80s: %v",raw,err)}
	}
}
func TestFrozenEvaluationAndVisibility(t *testing.T) {
	e:=validEvaluation();if err:=ValidateEvaluation(e);err!=nil{t.Fatal(err)}
	if !CanRead(e,"another-team",false){t.Fatal("public result hidden")}
	e.Dataset.Visibility=Team;e.Dataset.TenantID="data-team"
	if CanRead(e,"team",false)||!CanRead(e,"data-team",false)||!CanRead(e,"other",true){t.Fatal("result widened dataset access")}
	e.Dataset.Visibility="UNKNOWN";if CanRead(e,"",true){t.Fatal("invalid visibility must fail closed")}
}
func TestEvaluatorAndRequestValidation(t *testing.T) {
	if err:=ValidateEvaluator(validEvaluator());err!=nil{t.Fatal(err)}
	for _,change:=range []func(*Evaluator){func(e *Evaluator){e.ImageDigest="latest"},func(e *Evaluator){e.GitCommit="main"},func(e *Evaluator){e.GitURL="https://user:secret@example.test/x"},func(e *Evaluator){e.EntryPoint=nil},func(e *Evaluator){e.Protocol="other"}} {
		e:=validEvaluator();change(&e);if !errors.Is(ValidateEvaluator(e),ErrInvalid){t.Fatalf("bad evaluator accepted %+v",e)}
	}
	...
	r.Split="train";if !errors.Is(ValidateRequest(r),ErrInvalid){t.Fatal("training split accepted")}
}

func TestSitesResourcesAndCanonicalInputAreImmutable(t *testing.T) {
	sites:=[]string{"site-b","site-a"};canonical,err:=CanonicalSites(sites)
	if err!=nil||canonical[0]!="site-a"||sites[0]!="site-b"{t.Fatalf("site normalization mutated input: %v %v",sites,err)}
	if _,err:=CanonicalSites([]string{"site-a","site-a"});!errors.Is(err,ErrInvalid){t.Fatal("duplicate sites accepted")}
	if _,err:=CanonicalSites([]string{"../outside"});!errors.Is(err,ErrInvalid){t.Fatal("unsafe site accepted")}
