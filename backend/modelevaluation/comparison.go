...
func Compare(left, right Evaluation) (Comparison, error) {
	...
	for _, evaluation := range []Evaluation{left, right} {
		...
		if evaluation.State != Succeeded || evaluation.ReportState != ReportValid || evaluation.Report == nil {
			result.Reasons = append(result.Reasons, "Both evaluations require a succeeded job and a valid report.")
			return result, nil
		}
		if err := validateReportValue(*evaluation.Report, evaluation); err != nil {
			return result, err
		}
	}
	if ComparisonFingerprint(left) != ComparisonFingerprint(right) {
		result.Reasons = append(result.Reasons, "Dataset selection, evaluator executable, or configuration differs.")
		return result, nil
	}
	leftMetrics, _ := metricMap(left.Report.Metrics)
	rightMetrics, _ := metricMap(right.Report.Metrics)
	if len(leftMetrics) != len(rightMetrics) {
		result.Reasons = append(result.Reasons, "Metric sets differ.")
		return result, nil
	}
	names := make([]string, 0, len(leftMetrics))
	for name, leftMetric := range leftMetrics {
		rightMetric, exists := rightMetrics[name]
		if !exists || leftMetric.Unit != rightMetric.Unit || leftMetric.Direction != rightMetric.Direction {
			result.Reasons = append(result.Reasons, "Metric names, units, or directions differ.")
			return result, nil
		}
	...
	for _, name := range names {
		l, r := leftMetrics[name], rightMetrics[name]
		delta := r.Value - l.Value
		if math.IsInf(delta, 0) || math.IsNaN(delta) {
			return result, invalid("metric difference exceeds numeric bounds")
		}
		outcome := "equal"
		if delta != 0 {
			outcome = "neutral"
			if l.Direction != Neutral {
				outcome = "regressed"
				if (l.Direction == Higher && delta > 0) || (l.Direction == Lower && delta < 0) {
					outcome = "improved"
				}
			}
		}
