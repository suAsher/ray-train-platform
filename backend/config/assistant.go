...
func loadAssistantConfig() (AssistantConfig, error) {
	...
	if raw == "" {
		return cfg, nil
	} // Explicit retrieval-only mode needs no model or key.
	if len(raw) > 16384 {
	...
	if decoder.Decode(&entries) != nil {
		return AssistantConfig{}, fmt.Errorf("ASSISTANT_PROVIDERS must be a supported JSON provider list; credentials belong in referenced environment variables")
	}
