...
func TestHTTPDemandObserverRequiresFreshValidAuthenticatedTLSObservation(t *testing.T) {
 ...
 if err:=os.WriteFile(file,[]byte(token),0600);err!=nil {t.Fatal(err)}
 for _, tc:=range []struct{name,body string;status int;want,wantErr bool}{
  ...
  t.Run(tc.name,func(t *testing.T){
   server:=httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request){
    if r.Method!="GET" || r.URL.RawQuery!="" || r.Header.Get("Authorization")!="Bearer "+token { t.Error("invalid request contract") }
    w.WriteHeader(tc.status);fmt.Fprint(w,tc.body)
   }));defer server.Close()
   observer,err:=NewHTTPDemandObserver(file,"");if err!=nil {t.Fatal(err)}
   observer.url=server.URL;observer.now=func()time.Time{return now}
   if _,err:=observer.Pending(context.Background());err==nil {t.Fatal("untrusted TLS accepted")}
   observer.client.Transport=server.Client().Transport
   got,err:=observer.Pending(context.Background());if got!=tc.want || (err!=nil)!=tc.wantErr {t.Fatalf("pending=%t err=%v",got,err)}
  })
 }
}
