...
func renderRuntime(cfg RenderConfig) (*unstructured.Unstructured,error) {
 if cfg.RuntimeType == "pod" { return RenderPod(cfg) }
...
func RenderPod(cfg RenderConfig) (*unstructured.Unstructured,error) {
 ...
 cfg.RuntimeType = "pod"
 if err := validateRenderConfig(cfg); err != nil { return nil,err }
 for _, name := range []string{cfg.TLSSecretName,cfg.AuthSecretName,cfg.GateCASecretName} {
  if len(name)>63 || !dnsLabelPattern.MatchString(name) { return nil,errors.New("plain inference requires existing TLS, auth and gate CA Secrets") }
 }
 gate,err := url.Parse(cfg.GateURL)
 if err != nil || !strings.HasSuffix(gate.Hostname(),".svc.cluster.local") || gate.Port()!="8443" || gate.Path!="/gate" { return nil,errors.New("plain inference gate must use the fixed in-cluster HTTPS gate on 8443") }
 ...
 container["volumeMounts"]=mounts; spec["volumes"]=volumes
