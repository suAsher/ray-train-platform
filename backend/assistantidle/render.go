...
const (
	...
	assistantComponentLabel   = "raytrain.wellspiking.ai/component"
	dedicatedNodeLabel        = "raytrain.wellspiking.ai/dedicated"
	excludedDedicatedNodeRole = "algorithm"
)
...
func (cfg RenderConfig) withDefaults() RenderConfig {
	...
	if len(cfg.ToleratedTaintKeys) == 0 {
		cfg.ToleratedTaintKeys = []string{"nvidia.com/gpu"}
	}
	return cfg
}
...
func validateRenderConfig(cfg RenderConfig) error {
	for _, item := range []struct{ name, value string }{
		{"name", cfg.Name}, {"namespace", cfg.Namespace}, {"queue", cfg.QueueName}, {"model pvc", cfg.ModelPVC}, {"priority class", cfg.PriorityClassName}, {"instance", cfg.InstanceID},
	} {
	...
	for key, value := range cfg.RequiredNodeLabels {
		if !labelKeySafePattern.MatchString(key) || !safeLabelValue(value) || key == dedicatedNodeLabel && value == excludedDedicatedNodeRole {
	...
	for _, key := range cfg.ToleratedTaintKeys {
		if !labelKeySafePattern.MatchString(key) || seenTaints[key] {
...
func podSpec(cfg RenderConfig, worker bool) map[string]any {
	container := map[string]any{
		...
		"env":             serveEnv(cfg),
		"volumeMounts": []any{
			map[string]any{"name": "model-cache", "mountPath": cfg.ModelMountPath, "readOnly": true},
		},
...
func workerAffinity(cfg RenderConfig) map[string]any {
	expressions := []any{
		...
		map[string]any{"key": dedicatedNodeLabel, "operator": "NotIn", "values": []any{excludedDedicatedNodeRole}},
	}
