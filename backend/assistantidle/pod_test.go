...
func TestRenderPodUsesKueueGateTLSAndOneGPUWithoutRay(t *testing.T) {
 ...
 if obj.GetKind() != "Pod" || obj.GetAPIVersion() != "v1" { t.Fatal(obj) }
 ...
 for _, want := range []string{"kueue.x-k8s.io/admission", "assistant_serve.standalone", "\"nvidia.com/gpu\":\"1\"", "\"automountServiceAccountToken\":false", "/run/assistant/tls", "/run/assistant/auth", "/run/assistant/gate-ca", "\"scheme\":\"HTTPS\"", "\"activeDeadlineSeconds\":3600"} {
  if !strings.Contains(body, want) { t.Fatalf("missing %s", want) }
 }
 for _, bad := range []string{"RayService", "rayStartParams", "6379", "8265", "hostPath", "\"privileged\":true"} {
  if strings.Contains(body, bad) { t.Fatalf("unexpected %s", bad) }
 }
 cfg := podRenderConfig(); cfg.GateURL = "http://assistant-gate.svc.cluster.local:8443/gate"
 if _, err := RenderPod(cfg); err == nil { t.Fatal("accepted HTTP gate") }
 cfg = podRenderConfig(); cfg.TLSSecretName = ""
 if _, err := RenderPod(cfg); err == nil { t.Fatal("accepted missing TLS") }
}
...
func TestPodAdmissionRequiresMatchingWorkloadUIDAndRemovedGate(t *testing.T) {
 ...
 for _, tc := range []struct{name string; ownerUID string; gated bool; want bool}{
  {"admitted", "pod-uid", false, true}, {"stale workload", "other", false, false}, {"still gated", "pod-uid", true, false},
 } {
  t.Run(tc.name,func(t *testing.T){
   pod, err := RenderPod(cfg); if err != nil { t.Fatal(err) }
   pod.SetUID("pod-uid"); pod.SetCreationTimestamp(metav1.NewTime(time.Now()))
   pod.Object["status"] = map[string]any{"phase":"Running", "conditions": []any{map[string]any{"type":"Ready","status":"True"}}}
   if !tc.gated { unstructured.RemoveNestedField(pod.Object, "spec", "schedulingGates") }
   ...
   if err := adapter.Delete(context.Background(), "pod-uid"); err != nil { t.Fatal(err) }
  })
 }
}
