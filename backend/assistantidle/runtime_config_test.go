...
func TestRuntimeConfigRejectsUnsafeOrUnknownConfiguration(t *testing.T){
 ...
 if _,err:=ParseRuntimeConfig(strings.NewReader(string(data)));err!=nil{t.Fatal(err)}
 for _,suffix:=range []string{"{}"," null"}{if _,err:=ParseRuntimeConfig(strings.NewReader(string(data)+suffix));err==nil{t.Fatal("accepted trailing JSON")}}
 if _,err:=ParseRuntimeConfig(strings.NewReader(`{"unknown":true}`));err==nil{t.Fatal("accepted unknown field")}
 cfg.Render.Namespace="ray-train-platform";data,_=json.Marshal(cfg)
 if _,err:=ParseRuntimeConfig(strings.NewReader(string(data)));err==nil{t.Fatal("accepted shared namespace")}
}
