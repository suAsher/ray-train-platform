...
func (c *Controller) Step(ctx context.Context) (Decision,error) {
 ...
 now:=c.now()
 if err!=nil {
  c.gate.Close();c.idleSince=time.Time{}
  ...
  if lookupErr==nil && uid!="" {err=errors.Join(err,c.backend.Delete(cleanup,uid))}
 ...
 if obs.ServiceExists { c.createSince=time.Time{};c.idleSince=time.Time{} }
 ...
 if !idle {c.idleSince=time.Time{}} else if c.idleSince.IsZero(){c.idleSince=now}
 if !c.idleSince.IsZero(){obs.IdleFor=now.Sub(c.idleSince)}
 if obs.ServiceExists {
  if snapshot.CreatedAt.IsZero() || snapshot.CreatedAt.After(now) { obs.Fresh=false } else {
   obs.Runtime=now.Sub(snapshot.CreatedAt);obs.StartingFor=obs.Runtime
  }
 }
 if !c.drainSince.IsZero(){obs.DrainingFor=now.Sub(c.drainSince)+time.Nanosecond}
 d:=Decide(c.config,obs)
 if d.Action!=ActionRouteReady {c.gate.Close()}
 switch d.Action {
 case ActionCreate:
  if !c.createSince.IsZero() {
   if now.Sub(c.createSince)<c.config.StartTimeout{return Decision{Action:ActionWait,State:StateStarting,Reason:"waiting for created object observation"},nil}
   c.createSince=time.Time{};c.idleSince=now
  ...
  if err==nil{c.createSince=now}else{c.idleSince=time.Time{}}
 case ActionRouteReady:
  c.gate.Open(now.Add(3*time.Second))
 case ActionDrain:
  if c.drainSince.IsZero(){c.drainSince=now}
 case ActionDelete:
  if snapshot.UID!="" {
   cleanup,cancel:=context.WithTimeout(context.WithoutCancel(ctx),2*time.Second)
   err=c.backend.Delete(cleanup,snapshot.UID);cancel()
  }
 }
 return d,err
}
...
func (c *Controller) Run(ctx context.Context, report func(Decision,error)) {
 ...
 for {
  ...
  d,err:=c.Step(attempt);cancel()
  if report!=nil{report(d,err)}
