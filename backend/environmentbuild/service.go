...
func NewService(store Store, runner Runner, registry Registry, vault Vault, config Config) (*Service, error) {
	...
	if config.AuthorizationTTL <= 0 || config.AuthorizationTTL > 24*time.Hour {
		config.AuthorizationTTL = 24 * time.Hour
	}
	if config.GlobalConcurrency <= 0 {
		config.GlobalConcurrency = 2
	}
	if config.UserConcurrency <= 0 {
		config.UserConcurrency = 1
	}
	id, err := randomID("controller-")
	if err != nil {
		return nil, err
	}
...
func (s *Service) Create(ctx context.Context, owner Owner, workspaceID string, req CreateRequest) (Build, error) {
	...
	if req.Visibility != "personal" && req.Visibility != "team" {
		return Build{}, ErrInvalid
	}
	// A deterministic opaque ID makes idempotency independent of list pagination.
	sum := sha256.Sum256([]byte(owner.TenantID + "\x00" + owner.UserID + "\x00" + req.IdempotencyKey))
	...
	ws, err := s.store.EnvironmentWorkspace(ctx, owner, workspaceID)
	if err != nil {
		return Build{}, err
	}
	if ws.State != "RUNNING" && ws.State != "READY" {
		return Build{}, ErrConflict
	}
	snapshot, err := s.runner.InspectWorkspace(ctx, ws)
	if err != nil {
	...
	if err != nil {
		...
		if errors.Is(lookupErr, ErrNotFound) {
			_ = s.cleanupAuthorization(ctx, b)
		}
	} else if created.AuthID != b.AuthID {
		_ = s.cleanupAuthorization(ctx, b)
	}
	return created, err
}
func (s *Service) Get(ctx context.Context, o Owner, id string) (Build, error) {
	return s.store.EnvironmentBuild(ctx, o, id)
}
func (s *Service) List(ctx context.Context, o Owner) ([]Build, error) {
	return s.store.ListEnvironmentBuilds(ctx, o)
}
func (s *Service) Versions(ctx context.Context, o Owner) ([]Version, error) {
	return s.store.ListEnvironmentVersions(ctx, o)
}
func (s *Service) Retry(ctx context.Context, o Owner, id, authorizationID string) (Build, error) {
	b, err := s.store.EnvironmentBuild(ctx, o, id)
	if err != nil {
		return Build{}, err
	}
	if (b.Status != Failed && b.Status != AwaitingAuth) || b.CleanedAt == nil {
		return Build{}, ErrConflict
	}
...
func (s *Service) Cancel(ctx context.Context, o Owner, id string) (Build, error) {
	return s.store.CancelEnvironmentBuild(ctx, o, id)
}
func (s *Service) Run(ctx context.Context) {
	...
	for {
		if err := s.Reconcile(ctx); err != nil && ctx.Err() == nil {
			slog.Warn("environment build reconciliation deferred; pending operation remains durable")
		}
		select {
		case <-ctx.Done():
			return
...
func (s *Service) Reconcile(ctx context.Context) error {
	...
	if err := s.purgeExpiredCredentials(ctx); err != nil {
		return err
	}
...
func (s *Service) reconcileBuild(ctx context.Context, b Build) error {
	if b.Status == CancelRequested || b.Terminal() {
		...
		if err := s.runner.Cleanup(ctx, b, retain); err != nil {
			_ = s.store.SaveEnvironmentBuild(ctx, b, s.controllerID)
			return err
		}
		if err := s.cleanupAuthorization(ctx, b); err != nil {
			_ = s.store.SaveEnvironmentBuild(ctx, b, s.controllerID)
			return err
		}
		now := s.now()
	...
	if err != nil {
		if errors.Is(err, ErrAuthorization) {
			return s.fail(ctx, b, AwaitingAuth, "Harbor 拒绝发布，请重新授权后重试")
		}
		var phase *PhaseError
		if errors.As(err, &phase) && phase != nil {
			if message := phaseMessage(phase.Code); message != "" {
				return s.fail(ctx, b, Failed, message)
			}
		}
	...
	switch b.Status {
	case Capturing:
		if len(result.SnapshotJSON) == 0 || len(result.SnapshotJSON) > 1024*1024 {
			return s.fail(ctx, b, Failed, "依赖捕获结果不完整")
		}
		b.SnapshotJSON = result.SnapshotJSON
		b.Status = Building
	case Building:
		var layer struct {
			Digest string `json:"layerDigest"`
		}
		if json.Unmarshal([]byte(result.ChecksJSON), &layer) != nil || !digestPattern.MatchString(layer.Digest) {
			return s.fail(ctx, b, Failed, "环境层摘要缺失")
		}
		b.ChecksJSON = result.ChecksJSON
		b.Status = Validating
	case Validating:
		if !digestPattern.MatchString(result.ArtifactDigest) {
			return s.fail(ctx, b, Failed, "构建产物摘要无效")
		}
		b.ArtifactDigest = result.ArtifactDigest
		b.ChecksJSON = result.ChecksJSON
		b.Status = Pushing
	case Pushing:
		if !digestPattern.MatchString(result.ImageDigest) {
	...
	case VerifyingPull:
		...
		if !digestPattern.MatchString(b.ImageDigest) {
			return s.fail(ctx, b, Failed, "镜像摘要缺失")
		}
		b.Status = Ready
		b.ImageID = "image-" + b.ID
		b.Message = "训练环境已就绪"
		return s.store.FinalizeEnvironmentBuild(ctx, b, s.controllerID)
	default:
		return ErrConflict
	}
	return s.store.SaveEnvironmentBuild(ctx, b, s.controllerID)
}
func (s *Service) fail(ctx context.Context, b Build, status, message string) error {
	b.ResumeStatus = b.Status
	b.Status = status
	b.Message = message
	b.CleanedAt = nil
	return s.store.SaveEnvironmentBuild(ctx, b, s.controllerID)
...
func mergeChecks(existing, latest string) string {
	...
	if len(latest) <= 65536 {
		_ = json.Unmarshal([]byte(latest), &added)
	}
