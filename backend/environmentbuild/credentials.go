...
func (s *Service) RevokeAuthorization(ctx context.Context, owner Owner, id string) error {
 ...
 if err=s.deleteAuthorizationMaterials(ctx,a.ID);err!=nil{return ErrUnavailable}
...
func (s *Service) bindAuthorization(ctx context.Context, owner Owner, id string, b Build) (Authorization,error) {
 ...
 if a.ExpiresAt.Sub(s.now())<time.Minute{return Authorization{},ErrAuthorization}
...
func (s *Service) cleanupAuthorization(ctx context.Context, b Build) error {
 ...
 if err==ErrNotFound{return s.deleteAuthorizationMaterials(ctx,b.AuthID)};return err
}
func (s *Service) putCredentialMaterial(ctx context.Context,a Authorization,sealed []byte)error{
 ...
 if err:=s.store.ReserveEnvironmentCredentialMaterial(ctx,material);err!=nil{return err}
 if err:=s.vault.Put(ctx,a.SecretRef,sealed,a.ExpiresAt);err!=nil{
  ...
  if decodeErr==nil&&readErr==nil&&wanted.Username==existing.Username&&wanted.Secret==existing.Secret{return nil}
  return ErrUnavailable
 }
 return nil
}
func(s *Service)deleteCredentialMaterial(ctx context.Context,ref string)error{
 if err:=s.vault.Delete(ctx,ref);err!=nil{return ErrUnavailable}
...
func(s *Service)deleteAuthorizationMaterials(ctx context.Context,id string)error{
 materials,err:=s.store.EnvironmentCredentialMaterials(ctx,id);if err!=nil{return err}
 for _,m:=range materials{if err=s.deleteCredentialMaterial(ctx,m.Ref);err!=nil{return err}}
 return nil
}
func (s *Service) purgeExpiredCredentials(ctx context.Context) error {
 materials,err:=s.store.ExpiredEnvironmentCredentialMaterials(ctx,s.now());if err!=nil{return err}
 for _,m:=range materials{if err=s.deleteCredentialMaterial(ctx,m.Ref);err!=nil{return err}}
 items,err:=s.store.ExpiredEnvironmentAuthorizations(ctx,s.now());if err!=nil{return err}
 for _,a:=range items{if err=s.deleteAuthorizationMaterials(ctx,a.ID);err!=nil{return fmt.Errorf("credential cleanup unavailable")};if err=s.store.DeleteEnvironmentAuthorization(ctx,a.ID);err!=nil{return err}}
 return nil
}
