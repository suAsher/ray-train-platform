...
func TestRegistryAvailabilityIsNotReportedAsIncorrectCredentials(t *testing.T) {
	for _, tc := range []struct{ failure, want error }{
		...
		if !errors.Is(err,tc.want) || strings.Contains(err.Error(),"fixture-secret"){t.Fatalf("wrong bind classification: %v",err)}
	}
}

func storeBuildForRegistryFailure()Build{return Build{ID:"build",Project:"project",Repository:"image"}}

func TestBackgroundPushCheckDistinguishesRegistryOutageFromDeniedCredentials(t *testing.T){
	for _,tc:=range []struct{failure error;status string}{
		...
		if store.build.Status!=tc.status || store.build.ResumeStatus!=Pushing || runner.steps!=0 || store.build.ArtifactDigest==""{t.Fatalf("wrong background failure state: %+v",store.build)}
		if strings.Contains(store.build.Message,"fixture-secret"){t.Fatal("private registry diagnostic leaked")}
		if tc.status==Failed && (!strings.Contains(store.build.Message,"按结束策略清理") || strings.Contains(store.build.Message,"授权已失效") || strings.Contains(store.build.Message,"没有目标仓库")){t.Fatal("network failure misreported as missing permission")}
		if err:=s.reconcileBuild(context.Background(),store.build);err!=nil{t.Fatal(err)}
		if runner.steps!=0 || runner.cleanups!=1 || !runner.retained || store.build.CleanedAt==nil || !store.revoked{t.Fatal("failed push check did not clean credentials and reach bounded artifact retry state")}
	}
}
