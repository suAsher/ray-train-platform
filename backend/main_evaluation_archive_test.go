...
func TestTrustedEvaluationArchiveCodeLoadedOnlyFromFrozenEvaluator(t *testing.T){
 ...
 loaded,err:=loadTrustedEvaluationRuntime(context.Background(),job,&evaluationLookupStub{value:record});if err!=nil{t.Fatal(err)}
 if loaded.Spec.EvaluationRuntime.CodeID!=source.ArtifactID||loaded.Spec.EvaluationRuntime.CodeSizeBytes!=1234{t.Fatal("code provenance was not loaded")}
 for _,mutate:=range []func(*domain.TrainingJob){func(v *domain.TrainingJob){v.Spec.Source.ArtifactID=strings.Repeat("e",32)},func(v *domain.TrainingJob){v.Spec.Source.ArtifactSHA256=strings.Repeat("e",64)},func(v *domain.TrainingJob){v.Spec.Source.Type="git"},func(v *domain.TrainingJob){v.Spec.Source.URI="tos://private/code"}}{
  candidate:=*job;mutate(&candidate);if _,err:=loadTrustedEvaluationRuntime(context.Background(),&candidate,&evaluationLookupStub{value:record});err==nil{t.Fatal("job source differs from frozen code")}
 }
 record.Evaluator.Code=nil
 if _,err:=loadTrustedEvaluationRuntime(context.Background(),job,&evaluationLookupStub{value:record});err==nil{t.Fatal("archive job loaded without frozen code")}
}
