...
type MLflowIntegrationStore struct {
	repo *GormRepository
	db   *gorm.DB
}

func NewMLflowIntegrationStore(repo *GormRepository) *MLflowIntegrationStore {
	return &MLflowIntegrationStore{repo: repo, db: repo.db}
}

var _ integrations.ManagementStore = (*MLflowIntegrationStore)(nil)

func integrationValue(r MLflowIntegrationRecord) integrations.Identity {
	return integrations.Identity{ID: r.ID, TenantID: r.TenantID, OwnerUserID: r.OwnerUserID, Name: r.Name, AllowCreateExperiments: r.AllowCreateExperiments, RevokedAt: r.RevokedAt, CreatedAt: r.CreatedAt}
}
func integrationError(err error) error {
	if errors.Is(err, gorm.ErrRecordNotFound) || errors.Is(err, ErrTenantRetirementBlocked) || errors.Is(err, ErrMembershipNotFound) {
...
func managedIntegration(tx *gorm.DB, p auth.Principal, id string, lock, allowRevoked bool) (MLflowIntegrationRecord, error) {
	if !auth.IsInteractiveAuthType(p.AuthType) || p.IntegrationID != "" || !integrations.ValidID(id) {
		return MLflowIntegrationRecord{}, integrations.ErrNotFound
	}
	if err := integrationOwner(tx, p.TenantID, p.Subject, lock); err != nil {
		return MLflowIntegrationRecord{}, err
	}
	query := tx
	if lock {
	...
	err := query.Where("id = ? AND tenant_id = ? AND owner_user_id = ?", id, p.TenantID, p.Subject).First(&row).Error
	return row, integrationError(err)
}
func resolveIntegration(tx *gorm.DB, p auth.Principal, lock bool) (MLflowIntegrationRecord, error) {
	if p.AuthType != auth.AuthTypePAT || !integrations.ValidID(p.IntegrationID) || p.Subject != "integration:"+p.IntegrationID {
		return MLflowIntegrationRecord{}, integrations.ErrNotFound
	}
	var row MLflowIntegrationRecord
	if err := tx.Where("id = ? AND tenant_id = ? AND revoked_at IS NULL", p.IntegrationID, p.TenantID).First(&row).Error; err != nil {
		return row, integrationError(err)
	}
	if err := integrationOwner(tx, row.TenantID, row.OwnerUserID, lock); err != nil {
		return row, err
	}
	if lock {
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("id = ? AND revoked_at IS NULL", row.ID).First(&row).Error; err != nil {
			return row, integrationError(err)
		}
	}
	return row, nil
...
func (s *MLflowIntegrationStore) Resolve(ctx context.Context, p auth.Principal) (integrations.Identity, error) {
	r, err := resolveIntegration(s.db.WithContext(ctx), p, false)
	return integrationValue(r), err
}
func (s *MLflowIntegrationStore) Authorize(ctx context.Context, p auth.Principal, experimentID, permission string) (integrations.Identity, error) {
	...
	r, err := resolveIntegration(tx, p, false)
	if err != nil {
		return integrations.Identity{}, err
	}
	if err := integrationExperiment(tx, r, experimentID, false); err != nil {
		return integrations.Identity{}, err
	}
	var grant MLflowIntegrationGrantRecord
	if err := tx.Where("integration_id = ? AND experiment_id = ? AND revoked_at IS NULL", r.ID, experimentID).First(&grant).Error; err != nil {
		return integrations.Identity{}, integrationError(err)
	}
	permissions, err := decodeIntegrationPermissions(grant.PermissionsJSON)
	if err != nil {
		return integrations.Identity{}, err
	}
	if !integrations.HasPermission(permissions, permission) {
...
func decodeIntegrationPermissions(raw string) ([]string, error) {
	var permissions []string
	if err := json.Unmarshal([]byte(raw), &permissions); err != nil {
		return nil, err
	}
	return integrations.NormalizePermissions(permissions)
}
func (s *MLflowIntegrationStore) ListGrantedExperimentIDs(ctx context.Context, p auth.Principal, permission string) ([]string, error) {
	...
	for _, row := range rows {
		permissions, err := decodeIntegrationPermissions(row.PermissionsJSON)
		if err != nil {
			return nil, err
		}
		if integrations.HasPermission(permissions, permission) {
			ids = append(ids, row.ExperimentID)
		}
	}
	return ids, nil
}
