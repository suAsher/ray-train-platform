...
func authorizeEvaluationJob(tx *gorm.DB, jobID string, token []byte, now time.Time) (modelEvaluationRecord, TrainingJobEventTokenRecord, error) {
	...
	if jobID == "" || len(token) != 32 {
		return r, credential, me.ErrUnauthorized
	}
	...
	if evaluationJobTerminal(job.ObservedState) || job.DesiredState == string(domain.DesiredCanceled) {
		return r, credential, me.ErrConflict
	}
	...
	if evaluationJobMatches(r, job) != nil {
		return r, credential, me.ErrUnauthorized
	}
...
func (s *ModelEvaluationRepository) StoreEvaluationReport(ctx context.Context, jobID string, token, raw []byte, now time.Time) (me.Evaluation, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := consumeTrainingEventRate(&credential, now); err != nil {
			return err
		}
		...
		if err := tx.Model(&r).Updates(map[string]any{"report_json": string(canonical), "report_sha256": digest, "report_state": me.ReportValid, "error": "", "revision": gorm.Expr("revision + 1"), "updated_at": now}).Error; err != nil {
			return err
		}
		if err := writeEvaluationAudit(tx, r.ID, "evaluation.report_received", r.OwnerID, now); err != nil {
			return err
		}
		if err := tx.Where("id = ?", r.ID).First(&r).Error; err != nil {
			return err
		}
		e, err = r.evaluation()
		return err
	...
	if err != nil {
		return me.Evaluation{}, err
	}
	if validationErr != nil {
		return me.Evaluation{}, validationErr
	}
	return e, nil
}
...
func (s *ModelEvaluationRepository) FinalizeEvaluationJob(ctx context.Context, input *domain.TrainingJob) error {
	...
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var job JobRecord
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("id = ?", input.ID).First(&job).Error; err != nil {
		...
		if !evaluationJobTerminal(job.ObservedState) {
			if job.ObservedState != string(domain.StateRunning) || r.State == me.Running {
				return nil
			}
			state = me.Running
		} else {
			...
			if reportState == me.ReportPending {
				reportState = me.ReportMissing
			}
			if job.ObservedState == string(domain.StateCanceled) {
				...
				if r.ReportState == me.ReportValid {
					...
					if err == nil && digest == r.ReportSHA256 {
						state, message = me.Succeeded, ""
					} else {
						reportState = me.ReportInvalid
					}
				}
			}
		}
		if err := tx.Model(&r).Updates(map[string]any{"state": state, "report_state": reportState, "error": message, "revision": gorm.Expr("revision + 1"), "updated_at": now, "finished_at": finished}).Error; err != nil {
			return err
		}
