...
func TestModelDescriptionsUseCharacterLimits(t *testing.T) {
 s:=modelTestStore(t);ctx:=context.Background();text:=strings.Repeat("模",4000)
 m,err:=s.CreateModel(ctx,ml.Model{Name:strings.Repeat("型",200),Description:text,OwnerID:"owner",TenantID:"team"});if err!=nil{t.Fatal(err)}
 if _,err:=s.CreateModel(ctx,ml.Model{Name:strings.Repeat("型",201),OwnerID:"owner",TenantID:"team"});!errors.Is(err,ml.ErrInvalid){t.Fatal("201 character name accepted")}
 if _,err:=s.UpdateModel(ctx,m.ID,ml.ModelUpdate{Description:&text,Revision:m.Revision},ml.Actor{ID:"owner"});err!=nil{t.Fatal(err)}
 tooLong:=text+"型";if _,err:=s.UpdateModel(ctx,m.ID,ml.ModelUpdate{Description:&tooLong,Revision:m.Revision+1},ml.Actor{ID:"owner"});!errors.Is(err,ml.ErrInvalid){t.Fatal("4001 character description accepted")}
 v,err:=s.ReserveVersion(ctx,ml.Version{SourceETag:"source-etag",ModelID:m.ID,CreatorID:"owner",SizeBytes:1,IdempotencyKey:"key",RequestSHA256:"hash"});if err!=nil{t.Fatal(err)}
 if _,err:=s.UpdateVersion(ctx,m.ID,v.ID,ml.VersionUpdate{Description:text,Revision:v.Revision},ml.Actor{ID:"owner"});err!=nil{t.Fatal(err)}
 if _,err:=s.UpdateVersion(ctx,m.ID,v.ID,ml.VersionUpdate{Description:tooLong,Revision:v.Revision+1},ml.Actor{ID:"owner"});!errors.Is(err,ml.ErrInvalid){t.Fatal("4001 character version description accepted")}
}
