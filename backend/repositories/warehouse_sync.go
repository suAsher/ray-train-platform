...
func (s *WarehouseSyncStore) transaction(ctx context.Context, fn func(*gorm.DB)error) error {
 if s.db.Dialector.Name()=="sqlite" {warehouseSyncSQLiteMu.Lock();defer warehouseSyncSQLiteMu.Unlock()}
...
func warehouseSyncActive() []string {return []string{ws.WaitingSource,ws.Queued,ws.Uploading,ws.Registering,ws.WaitingReauth}}
func warehouseSyncQuota(tx *gorm.DB,owner string) error {
 var count int64
 if err:=tx.Model(&ws.Operation{}).Where("owner_id = ? AND state IN ?",owner,warehouseSyncActive()).Count(&count).Error;err!=nil{return err}
 if count>=16{return ws.ErrQuota};return nil
}

func (s *WarehouseSyncStore) Create(ctx context.Context,op ws.Operation)(ws.Operation,error){
 if op.ID==""||op.OwnerID==""||op.TenantID==""||op.IdempotencyKey==""||op.RequestSHA256==""||len(op.Credential)==0||(op.State!=ws.Queued&&op.State!=ws.WaitingSource){return ws.Operation{},ws.ErrInvalid}
 var result ws.Operation
 err:=s.transaction(ctx,func(tx *gorm.DB)error{
  if err:=lockWarehouseSyncOwner(tx,op.OwnerID);err!=nil{return err}
  ...
  if err==nil {if prior.RequestSHA256!=op.RequestSHA256{return ws.ErrConflict};result=prior;return nil}
  if !errors.Is(err,gorm.ErrRecordNotFound){return err}
  if err:=warehouseSyncQuota(tx,op.OwnerID);err!=nil{return err}
  now:=time.Now().UTC();op.CreatedAt=now;op.UpdatedAt=now;op.LeaseID="";op.LeaseExpiresAt=nil
  if op.NextAttemptAt.IsZero(){op.NextAttemptAt=now}
  if op.Paths==nil{op.Paths=[]string{}};if op.Files==nil{op.Files=[]ws.File{}}
  if err:=tx.Create(&op).Error;err!=nil{return err};result=op;return nil
 })
 return result,err
}

func warehouseSyncReadError(err error)error{if errors.Is(err,gorm.ErrRecordNotFound){return ws.ErrNotFound};return err}
func (s *WarehouseSyncStore) Get(ctx context.Context,id string)(ws.Operation,error){
 var op ws.Operation;err:=s.db.WithContext(ctx).Where("id = ?",id).First(&op).Error;return op,warehouseSyncReadError(err)
}
...
func (s *WarehouseSyncStore) Claim(ctx context.Context,lease string,now,expires time.Time)(ws.Operation,error){
 if lease==""||!expires.After(now){return ws.Operation{},ws.ErrInvalid}
 var result ws.Operation;found:=false
 err:=s.transaction(ctx,func(tx *gorm.DB)error{
  ...
  for _,item:=range []struct{ registering bool; state,message string }{
   ...
   if item.registering{query=query.Where("state = ?",ws.Registering)}else{query=query.Where("state <> ?",ws.Registering)}
   if err:=query.Updates(map[string]any{"state":item.state,"message":item.message,"credential":nil,"lease_id":"","lease_expires_at":nil,"updated_at":now}).Error;err!=nil{return err}
  }
  ...
  if update.Error!=nil{return update.Error};if update.RowsAffected!=1{return ws.ErrConflict}
  result.LeaseID=lease;result.LeaseExpiresAt=&expires;result.UpdatedAt=now;found=true;return nil
 })
 if err==nil&&!found{err=ws.ErrNotFound};return result,err
}

func (s *WarehouseSyncStore) Save(ctx context.Context,op ws.Operation,lease string)error{
 if op.ID==""||lease==""||(op.LeaseID!=""&&op.LeaseID!=lease){return ws.ErrConflict}
 ...
 if op.LeaseID==""{op.LeaseExpiresAt=nil;fields=append(fields,"lease_expires_at")}
 ...
 if result.Error!=nil{return result.Error};if result.RowsAffected!=1{return ws.ErrConflict};return nil
}

func (s *WarehouseSyncStore) Renew(ctx context.Context,id,lease string,expires time.Time)error{
 now:=time.Now().UTC();if lease==""||!expires.After(now){return ws.ErrConflict}
...
func (s *WarehouseSyncStore) Resume(ctx context.Context,id string,actor ws.Actor,credential []byte)(ws.Operation,error){
 if len(credential)==0{return ws.Operation{},ws.ErrInvalid}
 var op ws.Operation
 err:=s.transaction(ctx,func(tx *gorm.DB)error{
  if err:=lockWarehouseSyncOwner(tx,actor.ID);err!=nil{return err}
  var err error;op,err=loadWarehouseSyncActor(tx,id,actor);if err!=nil{return err}
  if (op.State!=ws.Failed&&op.State!=ws.WaitingReauth)||op.TargetVersionID!=""||op.LeaseID!=""||op.LeaseExpiresAt!=nil{return ws.ErrConflict}
  if op.State==ws.Failed{if err:=warehouseSyncQuota(tx,actor.ID);err!=nil{return err}}
  now:=time.Now().UTC()
...
func (s *WarehouseSyncStore) Cancel(ctx context.Context,id string,actor ws.Actor)(ws.Operation,error){
 ...
 err:=s.transaction(ctx,func(tx *gorm.DB)error{
  var err error;op,err=loadWarehouseSyncActor(tx,id,actor);if err!=nil{return err}
  if op.TargetVersionID!=""{return ws.ErrConflict}
  switch op.State {case ws.WaitingSource,ws.Queued,ws.Uploading,ws.WaitingReauth:default:return ws.ErrConflict}
