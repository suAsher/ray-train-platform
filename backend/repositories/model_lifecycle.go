...
func modelReadError(err error)error {if errors.Is(err,gorm.ErrRecordNotFound){return ml.ErrNotFound};return err}
func writeModelAudit(tx *gorm.DB,m,v string,a ml.Actor,action string,details any)error {
 raw,err:=json.Marshal(details);if err!=nil{return err}
...
func (s *ModelLifecycleStore) GetModel(ctx context.Context,id string)(ml.Model,error) {var m ml.Model;err:=s.db.WithContext(ctx).Where("id = ?",id).First(&m).Error;return m,modelReadError(err)}
func modelLimit(n int)int {if n<=0{return 50};if n>100{return 100};return n}
func (s *ModelLifecycleStore) ListModels(ctx context.Context,f ml.Filter)(ml.ModelPage,error) {
 result:=ml.ModelPage{Items:[]ml.Model{}};limit:=modelLimit(f.Limit)
 query:=s.db.WithContext(ctx).Where("archived = ?",f.Archived)
 if f.OwnerID!=""{query=query.Where("owner_id = ?",f.OwnerID)}
 if f.Cursor!=""{query=query.Where("id > ?",f.Cursor)}
 if f.Q!=""{q:="%"+strings.ToLower(f.Q)+"%";query=query.Where("LOWER(name) LIKE ? OR LOWER(description) LIKE ?",q,q)}
 if err:=query.Order("id ASC").Limit(limit+1).Find(&result.Items).Error;err!=nil{return result,err}
...
func (s *ModelLifecycleStore) UpdateModel(ctx context.Context,id string,u ml.ModelUpdate,a ml.Actor)(ml.Model,error) {
 ...
 err:=s.db.WithContext(ctx).Transaction(func(tx *gorm.DB)error{
  res:=tx.Model(&ml.Model{}).Where("id = ? AND revision = ?",id,u.Revision).Updates(updates);if res.Error!=nil{return res.Error};if res.RowsAffected!=1{return ml.ErrConflict}
  if err:=tx.Where("id = ?",id).First(&m).Error;err!=nil{return err}
  return writeModelAudit(tx,id,"",a,"model.updated",m)
 });return m,err
}
func (s *ModelLifecycleStore) GetVersion(ctx context.Context,modelID,id string)(ml.Version,error) {var v ml.Version;err:=s.db.WithContext(ctx).Where("model_id = ? AND id = ?",modelID,id).First(&v).Error;return v,modelReadError(err)}
func (s *ModelLifecycleStore) ListVersions(ctx context.Context,modelID,cursor string,limit int)(ml.VersionPage,error) {
 result:=ml.VersionPage{Items:[]ml.Version{}};limit=modelLimit(limit);query:=s.db.WithContext(ctx).Where("model_id = ?",modelID)
...
func (s *ModelLifecycleStore) UpdateVersion(ctx context.Context,modelID,id string,u ml.VersionUpdate,a ml.Actor)(ml.Version,error) {
 ...
 err:=s.db.WithContext(ctx).Transaction(func(tx *gorm.DB)error{
  ...
  if err:=tx.Where("id = ?",id).First(&v).Error;err!=nil{return err}
...
func (s *ModelLifecycleStore) FindVersionRequest(ctx context.Context,modelID,creatorID,key string)(ml.Version,error) {
 var v ml.Version;err:=s.db.WithContext(ctx).Where("model_id = ? AND creator_id = ? AND idempotency_key = ?",modelID,creatorID,key).First(&v).Error;return v,modelReadError(err)
}
