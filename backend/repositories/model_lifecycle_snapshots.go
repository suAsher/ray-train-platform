...
func (s *ModelLifecycleStore) ReserveVersion(ctx context.Context, v ml.Version) (ml.Version, error) {
	if strings.TrimSpace(v.SourceETag) == "" || v.SizeBytes < 1 || v.SizeBytes > ml.MaxFileSize || v.CreatorID == "" || v.IdempotencyKey == "" || v.RequestSHA256 == "" {
		return ml.Version{}, ml.ErrInvalid
	}
	var result ml.Version
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var m ml.Model
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("id = ?", v.ModelID).First(&m).Error; err != nil {
			return modelReadError(err)
		}
		// All models of a stable owner share one quota lock across backend replicas.
		if tx.Dialector.Name() == "postgres" {
			if err := tx.Exec("SELECT pg_advisory_xact_lock(hashtextextended(?, 734915))", m.OwnerID).Error; err != nil {
				return err
			}
		}
		err := tx.Where("model_id = ? AND creator_id = ? AND idempotency_key = ?", v.ModelID, v.CreatorID, v.IdempotencyKey).First(&result).Error
		if err == nil {
			if result.RequestSHA256 != v.RequestSHA256 {
		...
		if err := tx.Raw(`SELECT COALESCE(SUM(v.size_bytes),0) AS bytes, COALESCE(SUM(CASE WHEN v.state IN ('PENDING','COPYING') THEN 1 ELSE 0 END),0) AS pending FROM model_versions v JOIN model_catalog m ON m.id = v.model_id WHERE m.owner_id = ?`, m.OwnerID).Scan(&usage).Error; err != nil {
			return err
		}
		// Failed snapshots remain charged until a future safe object cleanup releases them.
		if usage.Bytes > ml.OwnerBudget-v.SizeBytes || usage.Pending >= ml.MaxPending {
			return ml.ErrQuota
		}
		var number int64
		if err := tx.Model(&ml.Version{}).Select("COALESCE(MAX(number),0)").Where("model_id = ?", v.ModelID).Scan(&number).Error; err != nil {
			return err
		}
		v.ID = uuid.NewString()
		v.Number = number + 1
		...
		if err := writeModelAudit(tx, m.ID, v.ID, ml.Actor{ID: v.CreatorID, Name: v.CreatorName}, "version.requested", map[string]any{"number": v.Number, "jobId": v.JobID, "sizeBytes": v.SizeBytes}); err != nil {
			return err
		}
		result = v
		return nil
...
func (s *ModelLifecycleStore) ClaimVersion(ctx context.Context, lease string, now, until time.Time) (ml.Version, error) {
	...
	if err := s.db.WithContext(ctx).Model(&ml.Version{}).Where("state = ? AND lease_expires_at <= ?", ml.Copying, now).Updates(map[string]any{"state": ml.Failed, "error": "Snapshot copy failed; register a new version to retry.", "lease_id": "", "lease_expires_at": nil, "revision": gorm.Expr("revision + 1"), "updated_at": now}).Error; err != nil {
		return ml.Version{}, err
	}
	var v ml.Version
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE", Options: "SKIP LOCKED"}).Where("state = ?", ml.Pending).Order("created_at ASC, id ASC").First(&v).Error; err != nil {
			return modelReadError(err)
		}
...
func (s *ModelLifecycleStore) FinishVersion(ctx context.Context, id, lease, state string, parts []ml.Part, hash string, now time.Time) error {
	if lease == "" || (state != ml.Ready && state != ml.Failed) {
		return ml.ErrInvalid
	}
	...
	if state == ml.Ready {
		...
		updates["parts"] = string(raw)
		updates["sha256"] = hash
		updates["error"] = ""
	} else {
		updates["error"] = "Snapshot copy failed; register a new version to retry."
	}
