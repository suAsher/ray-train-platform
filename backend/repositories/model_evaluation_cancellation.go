...
func validateEvaluationSubmissionReservation(tx *gorm.DB, job *domain.TrainingJob) error {
	...
	if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("id = ?", job.ExternalSubmissionID).First(&r).Error; err != nil { return evaluationReadError(err) }
	if r.JobID != job.ID || r.OwnerID != job.UserID || r.TenantID != job.TenantID { return me.ErrConflict }
	var existing JobRecord
	err := tx.Where("id = ?", job.ID).First(&existing).Error
	if err == nil {
		if evaluationJobMatches(r, existing) != nil { return me.ErrConflict }
		return &IdempotencyConflictError{JobID: existing.ID}
	}
	if !errors.Is(err, gorm.ErrRecordNotFound) { return err }
	if r.State != me.Creating { return me.ErrConflict }
	return nil
}
...
func (s *ModelEvaluationRepository) CancelEvaluationReservation(ctx context.Context, id string) (bool, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if evaluationTerminal(r.State) { cancelled = true; return nil }
		var count int64
		if err := tx.Model(&JobRecord{}).Where("id = ?", r.JobID).Count(&count).Error; err != nil { return err }
		if count != 0 { return nil }
		if r.State != me.Creating { return me.ErrConflict }
		now := time.Now().UTC()
		if err := tx.Model(&r).Updates(map[string]any{"state": me.Cancelled, "report_state": me.ReportMissing, "error": "evaluation was cancelled before job submission", "revision": gorm.Expr("revision + 1"), "updated_at": now, "finished_at": now}).Error; err != nil { return err }
		cancelled = true
