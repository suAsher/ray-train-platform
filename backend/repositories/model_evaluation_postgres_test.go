...
func evaluationPostgresStores(t *testing.T, upgrade bool) (*ModelEvaluationRepository, *ModelEvaluationRepository) {
	...
	if upgrade {
		...
		if err := a.Create(&old).Error; err != nil {
			t.Fatal(err)
		}
	}
	for i := 0; i < 2; i++ {
	...
	var maxVersion int
	if err := a.Raw("SELECT MAX(version) FROM schema_migrations").Scan(&maxVersion).Error; err != nil || maxVersion != 56 {
		t.Fatalf("schema version=%d error=%v", maxVersion, err)
	}
	if upgrade {
		var old ml.Model
		if err := a.Where("id = ?", "schema49-preserved").First(&old).Error; err != nil || old.Name != "existing model" {
			t.Fatalf("schema49 data changed: %+v %v", old, err)
		}
	}
...
func TestModelEvaluationPostgresSchema49UpgradeReservationACL(t *testing.T) {
	...
	if err := a.db.Where("state = ?", me.Failed).First(&row).Error; err != nil {
		t.Fatal(err)
	}
	for _, update := range []map[string]any{{"job_id": "replace-job"}, {"snapshot_json": "{}"}, {"state": me.Creating, "finished_at": nil}} {
		if err := a.db.Model(&modelEvaluationRecord{}).Where("id = ?", row.ID).Updates(update).Error; err == nil {
			t.Fatalf("immutable evaluation mutation accepted: %v", update)
		}
	}
	var evaluator modelEvaluatorRecord
	if err := a.db.First(&evaluator).Error; err != nil {
		t.Fatal(err)
	}
	if err := a.db.Model(&evaluator).Update("snapshot_json", "{}").Error; err == nil {
		t.Fatal("evaluator executable changed")
	}
}
...
func TestModelEvaluationPostgresReportIdentityTerminalAndFinite(t *testing.T) {
	...
	var row modelEvaluationRecord
	if err := a.db.Where("state = ?", me.Succeeded).First(&row).Error; err != nil {
		t.Fatal(err)
	}
	if err := a.db.Model(&row).Update("report_json", "{}").Error; err == nil {
		t.Fatal("immutable accepted report changed")
	}
}
...
func TestModelEvaluationPostgresConcurrentReservationsAndQuota(t *testing.T) {
	...
	for _, different := range []bool{false, true} {
		...
		for i, s := range []*ModelEvaluationRepository{a, b} {
			go func(i int, s *ModelEvaluationRepository) {
				<-start
				e := base
				e.ID = uuid.NewString()
				e.JobID = "job-" + uuid.NewString()
				e.IdempotencyKey = fmt.Sprint("race-", different)
				if different && i == 1 {
					e.RequestSHA256 = strings.Repeat("f", 64)
				}
				got, created, err := s.ReserveEvaluation(ctx, e)
		...
		if !different {
			if one.err != nil || two.err != nil || one.e.ID != two.e.ID || one.created == two.created {
				t.Fatalf("duplicate reservation: %+v %+v", one, two)
			}
		} else if !(one.err == nil && errors.Is(two.err, me.ErrConflict) || two.err == nil && errors.Is(one.err, me.ErrConflict)) {
			t.Fatalf("different request race: %+v %+v", one, two)
		}
	}
	for i := 2; i < maxActiveModelEvaluations-1; i++ {
		reserveEvaluationFixture(t, a, base, fmt.Sprint("quota-", i))
	}
	start := make(chan struct{})
	...
	for i, s := range []*ModelEvaluationRepository{a, b} {
		go func(i int, s *ModelEvaluationRepository) {
			...
			e.ID = uuid.NewString()
			e.JobID = "job-" + uuid.NewString()
			e.IdempotencyKey = fmt.Sprint("quota-race-", i)
			_, _, err := s.ReserveEvaluation(ctx, e)
			results <- err
		}(i, s)
	}
	close(start)
	one, two := <-results, <-results
	if !(one == nil && errors.Is(two, me.ErrQuota) || two == nil && errors.Is(one, me.ErrQuota)) {
		t.Fatalf("active quota race: %v %v", one, two)
	}
	var count int64
	if err := a.db.Model(&modelEvaluationRecord{}).Where("owner_id = ? AND state = ?", base.OwnerID, me.Creating).Count(&count).Error; err != nil || count != maxActiveModelEvaluations {
		t.Fatalf("active rows=%d error=%v", count, err)
	}
}
...
func TestModelEvaluationPostgresCanonicalConfigRoundTrip(t *testing.T) {
	...
	for _, read := range []func() (me.Evaluation, error){
		...
		func() (me.Evaluation, error) {
			return s.FindEvaluationRequest(context.Background(), e.TenantID, e.OwnerID, e.IdempotencyKey)
		},
	} {
		got, err := read()
		if err != nil {
		...
		if string(got.Config) != string(canonical) || hex.EncodeToString(sum[:]) != hash || !strings.Contains(string(got.Config), "9007199254740993") {
			t.Fatalf("JSONB changed runtime config bytes: %s hash=%s", got.Config, got.ConfigSHA256)
		}
	}
}
...
func TestModelEvaluationPostgresCancelVersusJobCreation(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		done, err := a.CancelEvaluationReservation(ctx, first.ID)
		if err != nil || !done {
			t.Fatalf("cancel retry %v %v", done, err)
		}
	}
	job := evaluationSubmissionTestJob(first)
	if err := NewGormRepository(b.db).Create(ctx, &job, "evaluation:"+first.ID); !errors.Is(err, me.ErrConflict) {
		t.Fatalf("late job after cancel: %v", err)
	}
	for _, table := range []string{"training_jobs", "outbox_events", "idempotency_keys"} {
		var count int64
		if err := a.db.Table(table).Count(&count).Error; err != nil || count != 0 {
			t.Fatalf("cancel left %s rows=%d error=%v", table, count, err)
		}
	}
	for i := 0; i < 8; i++ {
		...
		go func() {
			...
			created <- NewGormRepository(a.db).Create(ctx, &job, "evaluation:"+e.ID)
		}()
		go func() {
			<-start
			done, err := b.CancelEvaluationReservation(ctx, e.ID)
			cancelled <- cancellation{done, err}
		}()
		close(start)
		createErr, cancelResult := <-created, <-cancelled
		if cancelResult.err != nil {
			t.Fatal(cancelResult.err)
		}
		var jobs int64
		if err := a.db.Model(&JobRecord{}).Where("id = ?", e.JobID).Count(&jobs).Error; err != nil {
			t.Fatal(err)
		}
		current, err := a.GetEvaluation(ctx, e.ID, e.TenantID, false)
		if err != nil {
			t.Fatal(err)
		}
		if cancelResult.done {
			if jobs != 0 || current.State != me.Cancelled || !errors.Is(createErr, me.ErrConflict) {
				t.Fatalf("cancel won but late job appeared: jobs=%d state=%s create=%v", jobs, current.State, createErr)
			}
		} else {
			if createErr != nil || jobs != 1 || current.State != me.Creating {
				t.Fatalf("create won but reservation changed: jobs=%d state=%s create=%v", jobs, current.State, createErr)
			}
			job := evaluationSubmissionTestJob(e)
			var conflict *IdempotencyConflictError
			if err := NewGormRepository(a.db).Create(ctx, &job, "evaluation:"+e.ID); !errors.As(err, &conflict) || conflict.JobID != e.JobID {
				t.Fatalf("existing job replay: %v", err)
			}
		}
	}
}
