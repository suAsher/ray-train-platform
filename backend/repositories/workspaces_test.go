...
func TestAdministrativeWorkspaceLookupAndUpdateRemainIDScoped(t *testing.T) {
	...
	if err := repo.CreateWorkspace(ctx, workspace, 3600); err != nil { t.Fatal(err) }
	...
	if !ok { t.Fatal("administrative workspace lookup and ID-scoped updates are missing") }
	for _, tenant := range []string{"", "tenant-a"} {
		got, err := store.GetWorkspaceByID(ctx, "ws-original", tenant)
		if err != nil || got.UserID != "owner" { t.Fatalf("lookup tenant=%q got=%+v err=%v", tenant, got, err) }
	}
	if _, err := store.GetWorkspaceByID(ctx, "ws-original", "tenant-b"); err == nil { t.Fatal("cross-tenant lookup must fail") }
	if err := store.UpdateWorkspaceStateByID(ctx, "ws-original", domain.WorkspaceStopped); err != nil { t.Fatal(err) }
	replacement := *workspace
	replacement.ID = "ws-replacement"
	if err := repo.CreateWorkspace(ctx, &replacement, 3600); err != nil { t.Fatal(err) }
	if err := store.UpdateWorkspaceStateByID(ctx, "ws-original", domain.WorkspaceStopped); err == nil { t.Fatal("stale target update must fail") }
	got, err := repo.GetWorkspace(ctx, "tenant-a", "owner")
	if err != nil || got.ID != "ws-replacement" || got.State != domain.WorkspaceSubmitted { t.Fatalf("replacement workspace changed: %+v err=%v", got, err) }
}
