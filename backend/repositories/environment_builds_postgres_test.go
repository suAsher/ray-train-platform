...
func TestEnvironmentBuildPostgresAdmissionOwnerAndCredentialRoundTrip(t *testing.T){
 ...
 if err:=one.db.Create(&WorkspaceRecord{ID:"workspace-a",TenantID:owner.TenantID,UserID:owner.UserID,Namespace:"tenant-a",RayClusterName:"workspace-cluster",ObservedState:"RUNNING"}).Error;err!=nil{t.Fatal(err)}
 ...
 if err=s.CheckTarget(ctx,owner,authorization.ID,"public","test-env");err!=nil{t.Fatalf("PG credential round trip: %v",err)}
 first,err:=s.Create(ctx,owner,"workspace-a",environmentRequest(authorization.ID));if err!=nil{t.Fatal(err)}
 second:=first;second.ID="env-second";second.IdempotencyKey="second-request";second.Tag="env-second";second.OwnerID="other-owner"
 if _,err=one.CreateEnvironmentBuild(ctx,second);err!=nil{t.Fatal(err)}
 type result struct{build *eb.Build;err error}
 results:=make(chan result,2);start:=make(chan struct{})
 for i,r:=range []*GormRepository{one,two}{go func(i int,r *GormRepository){<-start;out,err:=r.ClaimEnvironmentBuild(ctx,fmt.Sprintf("worker-%d",i),time.Now().UTC(),time.Minute,1,1);results<-result{out,err}}(i,r)}
 close(start);x,y:=<-results,<-results
 if x.err!=nil||y.err!=nil||(x.build==nil)==(y.build==nil){t.Fatalf("global concurrent admission: %+v %+v",x,y)}
 // An expired lease can be reclaimed without launching a second operation.
 selected:=x.build;if selected==nil{selected=y.build}
 reclaimed,err:=two.ClaimEnvironmentBuild(ctx,"replacement",time.Now().UTC().Add(2*time.Minute),time.Minute,1,1)
 if err!=nil||reclaimed==nil||reclaimed.ID!=selected.ID{t.Fatalf("lease recovery: %+v %v",reclaimed,err)}
 if err=one.SaveEnvironmentBuild(ctx,*selected,selected.LeaseOwner);err!=eb.ErrConflict{t.Fatalf("stale controller saved: %v",err)}
}
func TestEnvironmentBuildPostgresRegistrationRollsBackAndRetainsPublishedDigest(t *testing.T){
 a,_:=evaluationPostgresStores(t,false);repo:=NewGormRepository(a.db);ctx:=context.Background();now:=time.Now().UTC()
 owner:=environmentOwner();ensurePostgresArtifactPrincipal(t,repo,ctx,auth.Principal{Subject:owner.UserID,Username:owner.UserID,TenantID:owner.TenantID,Roles:[]string{"Engineer"}})
 ...
 if _,err:=repo.CreateEnvironmentBuild(ctx,b);err!=nil{t.Fatal(err)}
 leased,err:=repo.ClaimEnvironmentBuild(ctx,"finalizer",now,time.Minute,1,1);if err!=nil||leased==nil{t.Fatal(err)}
 ready:=*leased;ready.Status=eb.Ready;ready.ImageID="image-env-finalize"
 ...
 if err=repo.db.Exec("CREATE FUNCTION reject_environment_version() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'injected version write failure'; END; $$").Error;err!=nil{t.Fatal(err)}
 if err=repo.db.Exec("CREATE TRIGGER reject_environment_version BEFORE INSERT ON environment_versions FOR EACH ROW EXECUTE FUNCTION reject_environment_version()").Error;err!=nil{t.Fatal(err)}
 if err=repo.FinalizeEnvironmentBuild(ctx,ready,"finalizer");err==nil{t.Fatal("injected version failure ignored")}
 var count int64;if err=repo.db.Model(&PlatformImageRecord{}).Where("id = ?",ready.ImageID).Count(&count).Error;err!=nil||count!=0{t.Fatal("half-registered catalog image survived transaction")}
 kept,err:=repo.EnvironmentBuild(ctx,owner,ready.ID);if err!=nil||kept.Status!=eb.VerifyingPull||kept.ImageDigest!=digest{t.Fatal("published digest/state lost on registration failure")}
