...
func TestMLflowIntegrationAuditActionIsExplicitAndAllowlisted(t *testing.T) {
	for _,action:=range []MLflowAuditAction{"",MLflowAuditRunLogBatch,"unexpected.action"} {
		t.Run(string(action),func(t *testing.T){
			...
			if action=="unexpected.action" {if err==nil {t.Fatal("unlisted action accepted")};return}
			if err!=nil {t.Fatal(err)}
			var record AuditLogRecord
			if err:=repo.db.First(&record).Error;err!=nil {t.Fatal(err)}
			want:="mlflow.dashboard.proxy";if action==MLflowAuditRunLogBatch {want="mlflow.run.log_batch"}
			if record.Action!=want {t.Fatalf("action=%s want=%s",record.Action,want)}
		})
	}
}
