...
func TestAssistantAuditPostgresUsesExistingSchemaAndNoContent(t *testing.T) {
	...
	if err := r.CreateAssistantAuditLog(context.Background(), p, "test-request", "auto", true); err != nil {
		t.Fatal(err)
	}
	var record AuditLogRecord
	if err := db.Where("request_id = ?", "test-request").First(&record).Error; err != nil {
		t.Fatal(err)
	}
	if record.ResourceType != "assistant_query" || record.Action != "assistant.query.started" || record.UserID != "member" || record.TenantID != "team" {
	...
	if len(payload) != 3 || payload["mode"] != "auto" || payload["includesLogs"] != true || payload["outcome"] != "started" || strings.Contains(record.PayloadJSON, p.Username) {
		t.Fatal("audit must store only allowed intent metadata")
	}
	if err := r.CreateAssistantAuditLog(context.Background(), p, "test-request-2", "arbitrary", false); err == nil {
