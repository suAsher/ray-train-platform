...
func (s *ModelEvaluationRepository) ReserveEvaluation(ctx context.Context, e me.Evaluation) (me.Evaluation, bool, error) {
	...
	if e.JobID == "" {
		return me.Evaluation{}, false, me.ErrInvalid
	}
	...
	if err := me.ValidateEvaluation(e); err != nil {
		return me.Evaluation{}, false, err
	}
	if e.OwnerID == "" || e.TenantID == "" || len(e.IdempotencyKey) < 1 || len(e.IdempotencyKey) > 128 || !validEvaluationDigest(e.RequestSHA256) {
		return me.Evaluation{}, false, me.ErrInvalid
	}
	created := false
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if tx.Dialector.Name() == "postgres" {
			if err := tx.Exec("SELECT pg_advisory_xact_lock(hashtextextended(?, 0))", "model-evaluations:"+e.OwnerID).Error; err != nil {
				return err
			}
		}
		...
		if err == nil {
			if previous.RequestSHA256 != e.RequestSHA256 {
		...
		var count int64
		if err := tx.Model(&modelEvaluationRecord{}).Where("owner_id = ? AND state IN ?", e.OwnerID, []string{me.Creating, me.Submitted, me.Running}).Count(&count).Error; err != nil {
			return err
		}
		if count >= maxActiveModelEvaluations {
			return me.ErrQuota
		}
		if err := validateEvaluationSources(tx, e); err != nil {
			return err
		}
...
func validateEvaluationSources(tx *gorm.DB, e me.Evaluation) error {
	var model ml.Model
	if err := tx.Clauses(clause.Locking{Strength: "SHARE"}).Where("id = ? AND archived = ?", e.ModelID, false).First(&model).Error; err != nil {
		return evaluationReadError(err)
	}
	var version ml.Version
	if err := tx.Clauses(clause.Locking{Strength: "SHARE"}).Where("id = ? AND model_id = ?", e.VersionID, e.ModelID).First(&version).Error; err != nil {
		return evaluationReadError(err)
	}
	if version.State != ml.Ready || version.SHA256 != e.ModelSHA256 || version.FileName != e.FileName {
		return me.ErrNotReady
	}
	var dataset DatasetRecord
	if err := tx.Clauses(clause.Locking{Strength: "SHARE"}).Where("id = ?", e.Dataset.ID).First(&dataset).Error; err != nil {
		return evaluationReadError(err)
	}
	var dataVersion DatasetVersionRecord
	if err := tx.Clauses(clause.Locking{Strength: "SHARE"}).Where("id = ? AND dataset_id = ?", e.Dataset.VersionID, e.Dataset.ID).First(&dataVersion).Error; err != nil {
		return evaluationReadError(err)
	}
	if dataVersion.State != string(domain.DatasetVersionReady) || dataVersion.ManifestSHA256 == nil || *dataVersion.ManifestSHA256 != e.Dataset.ManifestSHA256 || dataVersion.SchemaVersion != e.Dataset.SchemaVersion {
		return me.ErrNotReady
	}
	samples := dataVersion.ValSamples
	if e.Dataset.Split == "test" {
		samples = dataVersion.TestSamples
	}
	if samples != e.Dataset.SampleCount {
		return me.ErrConflict
	}
	tenant := ""
	if dataset.OwnerTenantID != nil {
		tenant = *dataset.OwnerTenantID
	}
	if dataset.Visibility != e.Dataset.Visibility || tenant != e.Dataset.TenantID {
		return me.ErrConflict
	}
	var evaluator modelEvaluatorRecord
	if err := tx.Clauses(clause.Locking{Strength: "SHARE"}).Where("id = ?", e.Evaluator.ID).First(&evaluator).Error; err != nil {
		return evaluationReadError(err)
	}
...
func (s *ModelEvaluationRepository) MarkEvaluationSubmitted(ctx context.Context, id, jobID string) error {
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("id = ? AND job_id = ?", id, jobID).First(&r).Error; err != nil {
			return evaluationReadError(err)
		}
		if err := evaluationJobMatches(r, job); err != nil {
			return err
		}
		...
		now := time.Now().UTC()
		if err := tx.Model(&r).Updates(map[string]any{"state": me.Submitted, "revision": gorm.Expr("revision + 1"), "updated_at": now}).Error; err != nil {
			return err
		}
...
func (s *ModelEvaluationRepository) FailEvaluationSubmission(ctx context.Context, id, message string) error {
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&r).Updates(map[string]any{"state": me.Failed, "report_state": me.ReportMissing, "error": "evaluation job submission failed", "revision": gorm.Expr("revision + 1"), "updated_at": now, "finished_at": now}).Error; err != nil {
			return err
		}
...
func evaluationJobMatches(r modelEvaluationRecord, job JobRecord) error {
	if r.JobID != job.ID || r.OwnerID != job.UserID || r.TenantID != job.TenantID || job.TrainingEngine != string(domain.TrainingEngineRayTrain) || job.SubmissionOrigin != string(domain.SubmissionOriginEvaluation) || job.ExternalSubmissionID != r.ID {
		return me.ErrConflict
	}
	return nil
}
