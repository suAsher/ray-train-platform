...
func trackingExpRecord(value tracking.Experiment)MLflowTrackingExperimentRecord{return MLflowTrackingExperimentRecord{value.ID,value.TenantID,value.UserID,value.IdempotencyHash,value.Name,value.State,value.UpstreamID,value.CreatedAt,value.UpdatedAt}}
func trackingExpValue(value MLflowTrackingExperimentRecord)tracking.Experiment{return tracking.Experiment{ID:value.ID,TenantID:value.TenantID,UserID:value.UserID,IdempotencyHash:value.IdempotencyHash,Name:value.Name,State:value.State,UpstreamID:value.UpstreamID,CreatedAt:value.CreatedAt,UpdatedAt:value.UpdatedAt}}
func trackingRunRecord(v tracking.Run)MLflowTrackingRunRecord{return MLflowTrackingRunRecord{v.ID,v.ExperimentID,v.TenantID,v.UserID,v.IdempotencyHash,v.Name,v.State,v.UpstreamID,v.StartTimeMS,v.EndTimeMS,v.CreatedAt,v.UpdatedAt,v.FinishedAt,v.FinishStatus,v.LeaseID,v.LeaseExpiresAt}}
func trackingRunValue(v MLflowTrackingRunRecord)tracking.Run{return tracking.Run{ID:v.ID,ExperimentID:v.ExperimentID,TenantID:v.TenantID,UserID:v.UserID,IdempotencyHash:v.IdempotencyHash,Name:v.Name,State:v.State,UpstreamID:v.UpstreamID,StartTimeMS:v.StartTimeMS,EndTimeMS:v.EndTimeMS,CreatedAt:v.CreatedAt,UpdatedAt:v.UpdatedAt,FinishedAt:v.FinishedAt,FinishStatus:v.FinishStatus,LeaseID:v.LeaseID,LeaseExpiresAt:v.LeaseExpiresAt}}
...
func(s *MLflowTrackingStore)ReserveExperiment(ctx context.Context,input tracking.Experiment)(tracking.Experiment,bool,error){
	...
	if insert.RowsAffected==1{return trackingExpValue(record),true,nil}
...
func(s *MLflowTrackingStore)ListExperiments(ctx context.Context,actor tracking.Actor,after string,limit int)([]tracking.Experiment,error){
	...
	items:=make([]tracking.Experiment,0,len(rows));for _,row:=range rows{items=append(items,trackingExpValue(row))};return items,nil
}
...
func(s *MLflowTrackingStore)ReserveRun(ctx context.Context,input tracking.Run)(tracking.Run,bool,error){
	...
	if insert.RowsAffected==1{return trackingRunValue(record),true,nil}
...
func(s *MLflowTrackingStore)ListRuns(ctx context.Context,actor tracking.Actor,experimentID,after string,limit int)([]tracking.Run,error){
	...
	if err:=query.Find(&rows).Error;err!=nil{return nil,err}
	items:=make([]tracking.Run,0,len(rows));for _,row:=range rows{items=append(items,trackingRunValue(row))};return items,nil
}
