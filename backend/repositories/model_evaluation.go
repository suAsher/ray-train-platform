...
func (r modelEvaluatorRecord) evaluationEvaluator() (me.Evaluator, error) {
	var e me.Evaluator
	if err := json.Unmarshal([]byte(r.SnapshotJSON), &e); err != nil {
		return e, err
	}
...
func (r modelEvaluationRecord) evaluation() (me.Evaluation, error) {
	...
	if err != nil || digest != e.ConfigSHA256 {
		return me.Evaluation{}, me.ErrInvalid
	}
	e.Config = canonical
	e.ID, e.JobID, e.State, e.ReportState = r.ID, r.JobID, r.State, r.ReportState
	if err := json.Unmarshal([]byte(r.JobSpecJSON), &e.JobSpec); err != nil {
		return e, err
	}
	...
	if r.ReportJSON != "" && r.ReportJSON != "null" {
		if err := json.Unmarshal([]byte(r.ReportJSON), &e.Report); err != nil {
			return e, err
		}
	}
	return e, nil
...
func (s *ModelEvaluationRepository) CreateEvaluator(ctx context.Context, e me.Evaluator) (me.Evaluator, error) {
	...
	if err := me.ValidateEvaluator(e); err != nil {
		return me.Evaluator{}, err
	}
	if e.OwnerID == "" || e.TenantID == "" {
		return me.Evaluator{}, me.ErrInvalid
	}
	...
	if result.Error != nil {
		return me.Evaluator{}, evaluationReadError(result.Error)
	}
	if result.RowsAffected != 1 {
		return me.Evaluator{}, me.ErrConflict
	}
	return e, nil
}
...
func (s *ModelEvaluationRepository) GetEvaluator(ctx context.Context, id string) (me.Evaluator, error) {
	...
	if err := s.db.WithContext(ctx).Where("id = ?", id).First(&r).Error; err != nil {
		return me.Evaluator{}, evaluationReadError(err)
	}
...
func (s *ModelEvaluationRepository) GetEvaluation(ctx context.Context, id, tenantID string, admin bool) (me.Evaluation, error) {
	...
	if err := evaluationVisible(s.db.WithContext(ctx), tenantID, admin).Where("id = ?", id).First(&r).Error; err != nil {
		return me.Evaluation{}, evaluationReadError(err)
	}
	return r.evaluation()
}
...
func (s *ModelEvaluationRepository) FindEvaluationRequest(ctx context.Context, tenantID, ownerID, key string) (me.Evaluation, error) {
	if tenantID == "" || ownerID == "" || key == "" {
		return me.Evaluation{}, me.ErrInvalid
	}
	var r modelEvaluationRecord
	if err := s.db.WithContext(ctx).Where("tenant_id = ? AND owner_id = ? AND idempotency_key = ?", tenantID, ownerID, key).First(&r).Error; err != nil {
		return me.Evaluation{}, evaluationReadError(err)
	}
	return r.evaluation()
}
