...
func(s *MLflowTrackingStore)ClaimRunLease(ctx context.Context,actor tracking.Actor,id,leaseID,finish string,now,until time.Time,endTimeMS int64)(tracking.Run,error){
	if leaseID==""||!until.After(now)||(finish!=""&&(!trackingTerminal(finish)||endTimeMS<0||endTimeMS>253402300799999)){return tracking.Run{},tracking.ErrInvalid}
	...
	err:=s.db.WithContext(ctx).Transaction(func(tx *gorm.DB)error{
		...
		if finish!=""&&endTimeMS<record.StartTimeMS{return tracking.ErrInvalid}
		if trackingTerminal(record.State){if record.State==finish{result=trackingRunValue(record);return nil};return tracking.ErrConflict}
		...
		if finish!=""{updates["state"]="FINISHING";updates["finish_status"]=finish;if record.State!="FINISHING"{updates["end_time_ms"]=endTimeMS}}
		...
		if err:=scopedTracking(tx,actor).Where("id = ?",id).First(&record).Error;err!=nil{return err};result=trackingRunValue(record);return nil
	})
	return result,err
}
...
func(s *MLflowTrackingStore)ReleaseRunLease(ctx context.Context,actor tracking.Actor,id,leaseID,finish string,now time.Time)(tracking.Run,error){
	...
	err:=s.db.WithContext(ctx).Transaction(func(tx *gorm.DB)error{
		...
		if finish!=""&&record.State!="FINISHING"{return tracking.ErrConflict}
		...
		if finish!=""{updates["state"]=finish;updates["finish_status"]=finish;updates["finished_at"]=time.UnixMilli(record.EndTimeMS).UTC()}
