...
func validDatasetVersion(state DatasetVersionState) DatasetVersion {
	return DatasetVersion{
		...
		ManifestObjectKey: "ray-train/platform/datasets/dataset-labeled-full/manifests/labeled-full-20260830.2+sha256-12ab34cd.parquet",
		SchemaVersion:     "bev-v1",
		TrainSamples:      100,
		ValSamples:        20,
		TestSamples:       10,
...
func TestDatasetVersionValidateManifestAndCounts(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "object key outside internal prefix", state: DatasetVersionReady, mutate: func(version *DatasetVersion) { version.ManifestObjectKey = "other/manifests/version.parquet" }},
