...
func TestTrainingEventValidateAcceptsOnlyManagedEventTypes(t *testing.T) {
	...
	for _, eventType := range []TrainingEventType{"", "CHECKPOINT_STARTED", "worker_group_started"} {
		if err := (TrainingEvent{ID: "event-1", Type: eventType}).Validate(); err == nil {
...
func TestTrainingEventValidateBoundsUntrustedFields(t *testing.T) {
	tests := []TrainingEvent{
		{ID: "../event", Type: TrainingEventProgress},
		{ID: "event-1", Type: TrainingEventProgress, Generation: -1},
		{ID: "event-1", Type: TrainingEventProgress, Epoch: -1},
		{ID: "event-1", Type: TrainingEventProgress, Step: -1},
		{ID: "event-1", Type: TrainingEventCheckpointComplete, Checkpoint: &TrainingCheckpoint{ID: "checkpoint-1", ObjectPath: "/mnt/data/output/checkpoint", MetricValue: floatPointer(math.NaN()), Complete: true}},
	}
