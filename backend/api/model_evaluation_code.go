...
func (h *Handler) acquireEvaluationCodeOperation(c *gin.Context) (func(), bool) {
	if h.evaluationCode == nil || h.evaluationCodeOperations == nil {
		h.writeError(c, 503, "EVALUATION_CODE_UNAVAILABLE", "评估代码快照服务暂不可用")
		return nil, false
	}
	select {
	case h.evaluationCodeOperations <- struct{}{}:
		return func() { <-h.evaluationCodeOperations }, true
	default:
		c.Header("Retry-After", "5")
		h.writeError(c, 429, "EVALUATION_CODE_BUSY", "评估代码快照读写繁忙，请稍后重试")
		return nil, false
	}
}
func (h *Handler) evaluationCodeSource(c *gin.Context, id string) (*domain.SourceArtifact, bool) {
	...
	if h.evaluationSourceArtifacts == nil {
		h.writeError(c, 503, "EVALUATION_CODE_UNAVAILABLE", "代码上传记录服务暂不可用")
		return nil, false
	}
	p := actorPrincipal(c)
	artifact, err := h.evaluationSourceArtifacts.GetSourceArtifact(c.Request.Context(), p.TenantID, p.Subject, id)
	if err != nil || artifact == nil || artifact.ID != id || artifact.TenantID != p.TenantID || artifact.UserID != p.Subject {
		h.writeError(c, 404, "SOURCE_ARTIFACT_NOT_FOUND", "代码包不存在或不属于当前用户和团队")
		return nil, false
	}
	if artifact.State != domain.SourceArtifactReady {
		h.writeError(c, 409, "SOURCE_ARTIFACT_NOT_READY", "请先完成代码包上传和校验")
		return nil, false
	}
	if artifact.SizeBytes < 1 || artifact.SizeBytes > me.MaxEvaluationCodeSize || !artifactSHAValid(artifact.SHA256) {
		h.writeError(c, 400, "EVALUATION_CODE_INVALID", "评估代码包必须为有效 ZIP 且不超过64 MiB")
		return nil, false
	}
	copy := *artifact
	return &copy, true
...
func evaluationCodeRequestID(p auth.Principal, key string) string {
	sum := sha256.Sum256([]byte("model-evaluator-code\x00" + p.TenantID + "\x00" + p.Subject + "\x00" + key))
...
func (h *Handler) publishModelEvaluatorCode(c *gin.Context, evaluator me.Evaluator, artifact domain.SourceArtifact) {
	...
	if err == nil {
		if !sameEvaluatorCodeDefinition(existing, evaluator) {
	...
	if !errors.Is(err, me.ErrNotFound) {
	...
	if snapshot != *evaluator.Code {
		h.writeError(c, 503, "EVALUATION_CODE_SNAPSHOT_MISMATCH", "代码快照与上传校验记录不一致，请保留请求键重试")
		return
	}
	...
	if err != nil {
		...
		if readErr == nil && sameEvaluatorCodeDefinition(existing, evaluator) {
			h.writeSuccess(c, 200, existing)
			return
		}
		h.modelEvaluationError(c, err)
...
func evaluationCodeJobSource(evaluator me.Evaluator) domain.CodeSource {
	if evaluator.Code != nil {
		return domain.CodeSource{Type: "evaluation-archive", ArtifactID: evaluator.Code.ID, ArtifactSHA256: evaluator.Code.SHA256}
	}
...
func (h *Handler) downloadEvaluationCode(c *gin.Context) {
	...
	if !ok || evaluation.Evaluator.Code == nil {
		h.modelEvaluationError(c, me.ErrNotFound)
		return
	}
	snapshot := *evaluation.Evaluator.Code
	if h.modelEvaluationError(c, me.ValidateCodeSnapshot(snapshot)) {
		return
	}
	if c.GetHeader("Range") != "" {
		h.writeError(c, 416, "EVALUATION_RANGE_UNSUPPORTED", "evaluation code downloads do not support ranges")
		return
	}
	...
	if reader == nil {
		h.writeError(c, 503, "EVALUATION_CODE_UNAVAILABLE", "评估代码快照读取失败")
		return
	}
	defer reader.Close()
	if size != snapshot.SizeBytes {
		h.writeError(c, 503, "EVALUATION_CODE_SNAPSHOT_MISMATCH", "评估代码快照大小校验失败")
		return
	}
	...
	c.Header("Content-Disposition", mime.FormatMediaType("attachment", map[string]string{"filename": "evaluation-code-" + snapshot.ID + ".zip"}))
	c.Header("Content-Length", strconv.FormatInt(size, 10))
	c.Header("X-Content-SHA256", snapshot.SHA256)
	c.Header("Accept-Ranges", "none")
	// Open verifies size and SHA against the immutable, Publish-validated ZIP.
	// Streaming remains bounded and never exposes a storage key or URL.
	n, copyErr := io.CopyBuffer(c.Writer, io.LimitReader(reader, size), make([]byte, 64<<10))
	if copyErr != nil || n != size {
		if c.Writer.Written() {
			_ = c.Error(me.ErrNotReady)
			return
		}
		for _, header := range []string{"Content-Type", "Content-Disposition", "Content-Length", "X-Content-SHA256"} {
			c.Header(header, "")
		}
