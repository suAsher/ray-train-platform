...
func (s *warehouseSyncAPIStore) Create(_ context.Context, op ws.Operation) (ws.Operation, error) {
	s.creates++
	s.operation = op
	return op, nil
}
func (s *warehouseSyncAPIStore) Get(context.Context, string) (ws.Operation, error) {
	return s.operation, nil
}
func (s *warehouseSyncAPIStore) Resume(_ context.Context, _ string, _ ws.Actor, credential []byte) (ws.Operation, error) {
	s.resumes++
	s.operation.State = ws.Queued
	s.operation.Credential = credential
	return s.operation, nil
}
func (s *warehouseSyncAPIStore) Cancel(context.Context, string, ws.Actor) (ws.Operation, error) {
	s.cancels++
	s.operation.State = ws.Canceled
	s.operation.Credential = nil
	return s.operation, nil
}

type warehouseSyncAPIUpstream struct {
	ws.Upstream
	checks int
}

func (u *warehouseSyncAPIUpstream) GetWarehouse(context.Context, string, string) (fw.Warehouse, error) {
	u.checks++
	return fw.Warehouse{ID: "warehouse-1", GroupID: "d49a984edd3d0648a43ab050d3cc0262", PermissionCodes: []string{"edit"}}, nil
}
func (u *warehouseSyncAPIUpstream) ListModelTypes(context.Context, string, string) ([]fw.ModelType, error) {
	return []fw.ModelType{{ID: "model-type-1"}}, nil
}
...
func warehouseSyncAPIRouter(t *testing.T, job domain.TrainingJob, enabled bool) (*gin.Engine, *warehouseSyncAPIStore, *warehouseSyncAPIUpstream) {
	...
	store := &warehouseSyncAPIStore{}
	upstream := &warehouseSyncAPIUpstream{}
	...
	r := gin.New()
	r.Use(auth.OAuth2ProxyMiddleware(&warehouseAPIOIDC{}, warehouseAPIAccounts{}, warehouseAPIPAT{}, warehouseAPILocal{}, true, auth.OAuth2ProxyOptions{}))
...
func warehouseSyncAPIPost(r http.Handler, path, body, bearer string) *httptest.ResponseRecorder {
	...
	req.Header.Set("Idempotency-Key", "sync-request-1")
...
func TestWarehouseSyncCreateChecksJobOwnerTenantAndTerminalState(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			job := warehouseSourceJob()
			job.UserID = tc.owner
			job.TenantID = tc.tenant
			job.ObservedState = tc.state
			r, store, upstream := warehouseSyncAPIRouter(t, job, true)
			body := warehouseSyncAPIInput
			if tc.automatic {
				body = strings.Replace(body, `"automatic":false`, `"automatic":true`, 1)
			}
			w := warehouseSyncAPIPost(r, "", body, "")
			if w.Code != tc.want {
				t.Fatalf("status %d: %s", w.Code, w.Body.String())
			}
			if tc.want != 202 && (store.creates != 0 || upstream.checks != 0) {
				t.Fatal("forbidden source reached target or persisted")
			}
			if tc.want == 202 && (store.creates != 1 || store.operation.OwnerID != "alice" || store.operation.TenantID != "local" || len(store.operation.Credential) == 0) {
				t.Fatal("accepted sync lost authenticated owner or durable encrypted authorization")
			}
			if strings.Contains(w.Body.String(), "verified-proxy") || strings.Contains(w.Body.String(), "credential") {
				t.Fatal("sync response leaked credential")
			}
		})
	}
}
func TestWarehouseSyncDisabledAndPATCannotCreate(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			r, store, upstream := warehouseSyncAPIRouter(t, warehouseSourceJob(), tc.enabled)
			w := warehouseSyncAPIPost(r, "", warehouseSyncAPIInput, tc.bearer)
			if w.Code != tc.want || store.creates != 0 || upstream.checks != 0 {
				t.Fatalf("invalid delegation status %d: %s", w.Code, w.Body.String())
			}
		})
	}
}
	...
	for _, action := range []string{"retry", "cancel"} {
		for _, scope := range []string{"owner", "foreign owner", "foreign tenant"} {
			t.Run(action+" "+scope, func(t *testing.T) {
				...
				state := ws.WaitingSource
				if action == "retry" {
					state = ws.WaitingReauth
				}
				...
				if scope == "foreign owner" {
					store.operation.OwnerID = "bob"
				}
				if scope == "foreign tenant" {
					store.operation.TenantID = "other"
				}
				...
				if scope != "owner" && (store.resumes != 0 || store.cancels != 0 || upstream.checks != 0) {
					t.Fatal("foreign sync mutation reached upstream/store")
				}
				if scope == "owner" && store.resumes+store.cancels != 1 {
					t.Fatal("authorized action was not performed")
				}
			})
		}
	}
