	...
	for _, role := range []string{domain.RoleSuperAdmin, domain.RoleEngineer} {
		t.Run(role, func(t *testing.T) {
			upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.Method != "GET" || r.URL.Path != "/mlflow/api/2.0/mlflow/runs/get" || r.URL.Query().Get("run_id") != runID {
					t.Fatalf("unexpected lookup: %s %s", r.Method, r.URL)
				}
				if r.Header.Get("Authorization") != "" || r.Header.Get("Cookie") != "" {
					t.Error("leaked browser credentials")
				}
			...
			if w.Code != 200 {
				t.Fatalf("shared Run lookup: %d %s", w.Code, w.Body.String())
			}
			var payload struct {
				Data struct {
					URL string `json:"url"`
			...
			if !strings.HasPrefix(payload.Data.URL, "/mlflow/?access_token=") {
				t.Fatalf("changed Portal contract: %s", payload.Data.URL)
			}
			exchange := httptest.NewRecorder()
			mlflowProxyRouter(h).ServeHTTP(exchange, httptest.NewRequest("GET", payload.Data.URL, nil))
			if exchange.Code != 302 || exchange.Header().Get("Location") != "/mlflow/#/experiments/14/runs/"+runID {
				t.Fatalf("incorrect Run redirect: %d %s", exchange.Code, exchange.Header().Get("Location"))
			}
		})
	}
}
...
func TestPublicDashboardRunLookupRejectsMissingOrMismatchedRun(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		w := httptest.NewRecorder()
		mlflowAccessRouter(h, &principal, true).ServeHTTP(w, httptest.NewRequest("POST", "/api/v1/mlflow-dashboard-access", strings.NewReader(`{"runId":"`+runID+`"}`)))
		if w.Code != 404 || len(store.created) != 0 {
			t.Fatalf("unsafe Run redirect: %d tickets=%d", w.Code, len(store.created))
		}
		upstream.Close()
	}
}
