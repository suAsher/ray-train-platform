...
func (h *Handler) InitializeFunctionWarehouseSync(ctx context.Context, store ws.Store, pepper []byte) error {
	...
	for env, client := range h.functionWarehouses {
		if upstream, ok := client.(ws.Upstream); ok { clients[env] = upstream }
	}
...
func (h *Handler) createWarehouseSync(c *gin.Context) {
	...
	if _, ok := h.jobArtifactRoot(c, p, job); !ok { return }
	op, err := h.warehouseSync.Create(c.Request.Context(), a, input, token)
	if h.warehouseSyncError(c, err) { return }
...
func (h *Handler) warehouseSyncError(c *gin.Context, err error) bool {
	...
	switch {
	case errors.Is(err, ws.ErrInvalid): h.writeError(c, 400, "WAREHOUSE_SYNC_INVALID", "请检查目标、版本名及权重相对路径；每次最多选择8个文件")
	case errors.Is(err, ws.ErrForbidden): h.writeError(c, 403, "WAREHOUSE_SYNC_FORBIDDEN", "没有此同步操作或来源文件的管理权限")
	case errors.Is(err, ws.ErrNotFound): h.writeError(c, 404, "WAREHOUSE_SYNC_NOT_FOUND", "同步记录不存在")
	case errors.Is(err, ws.ErrConflict): h.writeError(c, 409, "WAREHOUSE_SYNC_CONFLICT", "操作状态已变化；登记结果待确认时，请核对功能仓后再发起新的同步")
	case errors.Is(err, ws.ErrQuota): h.writeError(c, 429, "WAREHOUSE_SYNC_QUOTA", "待处理同步过多，请等待已有操作结束")
	default: return h.functionWarehouseError(c, err)
	}
	return true
}
