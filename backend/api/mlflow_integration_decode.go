...
func (h *Handler) decodeMLflowIntegrationBatch(c *gin.Context) (observability.MLflowLogBatch, bool) {
	...
	if err != nil {
		...
		if errors.As(err, &tooLarge) {
			h.writeError(c, 413, "MLFLOW_BATCH_TOO_LARGE", "MLflow batch exceeds 256 KiB")
		} else {
			h.writeError(c, 400, "MLFLOW_BATCH_INVALID", "could not read MLflow batch")
		}
		return batch, false
	}
	check := json.NewDecoder(bytes.NewReader(body))
	if err := checkMLflowJSONValue(check, 0); err != nil {
		h.writeError(c, 400, "MLFLOW_BATCH_INVALID", "MLflow batch contains invalid or duplicate fields")
		return batch, false
	}
	...
	if err == nil {
		...
		if decoder.Decode(&extra) != io.EOF {
			err = observability.ErrMLflowBatchInvalid
		}
	}
	if err == nil {
		err = batch.Validate()
	...
	if err != nil {
		h.writeError(c, 400, "MLFLOW_BATCH_INVALID", "MLflow batch is invalid")
		return batch, false
	}
	return batch, true
}
...
func checkMLflowJSONValue(decoder *json.Decoder, depth int) error {
	...
	case '{':
		...
		for decoder.More() {
			...
			seen[key] = struct{}{}
			if err := checkMLflowJSONValue(decoder, depth+1); err != nil {
				return err
			}
		}
	case '[':
