...
func (h *Handler) mlflowSDKGuard(limiter SourceArtifactLimiter, write bool) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !ok { sdkError(c, 401, "UNAUTHENTICATED", "authentication is required"); return }
		if p.AuthType != auth.AuthTypePAT || !p.HasScope(domain.PATScopeExperimentsRead) || (write && !p.HasScope(domain.PATScopeExperimentsWrite)) {
			sdkError(c, 403, "PERMISSION_DENIED", "a PAT with explicit external experiment scopes is required"); return
		}
		...
		if allowed, _ := limiter.Allow(p.TenantID+"\x00"+p.Subject, action); !allowed {
			c.Header("Retry-After", "60"); sdkError(c, 429, "REQUEST_LIMIT_EXCEEDED", "request rate limit exceeded"); return
		}
		if h.mlflowTracking == nil { sdkError(c, 503, "TEMPORARILY_UNAVAILABLE", "external tracking is unavailable"); return }
...
func sdkServiceError(c *gin.Context, err error) {
	switch {
	case errors.Is(err, mlflowtracking.ErrInvalid), errors.Is(err, observability.ErrMLflowBatchInvalid): sdkError(c, 400, "INVALID_PARAMETER_VALUE", "invalid tracking request")
	case errors.Is(err, mlflowtracking.ErrNotFound): sdkError(c, 404, "RESOURCE_DOES_NOT_EXIST", "external run was not found")
	case errors.Is(err, mlflowtracking.ErrConflict), errors.Is(err, mlflowtracking.ErrBusy): sdkError(c, 409, "INVALID_STATE", "run state conflicts with the request")
	case errors.Is(err, mlflowtracking.ErrPending): sdkError(c, 409, "INVALID_STATE", "operation is pending; read its state before retrying")
	default: sdkError(c, 503, "TEMPORARILY_UNAVAILABLE", "tracking outcome is uncertain; verify the run before retrying writes")
	}
}
...
func (h *Handler) getMLflowSDKRun(c *gin.Context) {
	id := c.Query("run_id")
	if !mlflowDashboardRunIDPattern.MatchString(id) || len(c.Request.URL.Query()["run_id"]) != 1 { sdkServiceError(c, mlflowtracking.ErrInvalid); return }
	detail, err := h.mlflowTracking.GetRun(c.Request.Context(), sdkActor(c), id)
	if err != nil { sdkServiceError(c, err); return }
	if detail.Run.ID != id { sdkServiceError(c, mlflowtracking.ErrNotFound); return }
	if detail.Run.State!="RUNNING" && detail.Run.State!="FINISHED" && detail.Run.State!="FAILED" && detail.Run.State!="KILLED" { sdkServiceError(c,mlflowtracking.ErrPending);return }
	metrics := make([]gin.H, 0, len(detail.Latest))
	for key, value := range detail.Latest { metrics = append(metrics, gin.H{"key":key, "value":value, "timestamp":0, "step":0}) }
	params := make([]gin.H, 0, len(detail.Params))
	for key, value := range detail.Params { params = append(params, gin.H{"key":key, "value":value}) }
...
func decodeMLflowSDKBody(reader io.Reader, target *mlflowSDKWriteRequest) error {
	...
	if err := checkMLflowJSONValue(json.NewDecoder(bytes.NewReader(body)), 0); err != nil { return err }
	d := json.NewDecoder(bytes.NewReader(body)); d.DisallowUnknownFields()
	if err := d.Decode(target); err != nil { return err }
	if !mlflowDashboardRunIDPattern.MatchString(target.RunID) { return mlflowtracking.ErrInvalid }
	var extra any
	if d.Decode(&extra) != io.EOF { return mlflowtracking.ErrInvalid }
	return nil
}

func (h *Handler) writeMLflowSDKRun(c *gin.Context) {
	var req mlflowSDKWriteRequest
	if err := decodeMLflowSDKBody(c.Request.Body, &req); err != nil { sdkServiceError(c, mlflowtracking.ErrInvalid); return }
	...
	if err!=nil {sdkServiceError(c,mlflowtracking.ErrInvalid);return}
	var input mlflowtracking.Batch
	encoded,_:=json.Marshal(batch)
	if err=json.Unmarshal(encoded,&input);err!=nil {sdkServiceError(c,mlflowtracking.ErrInvalid);return}
	err=h.sdkAuditWrite(c,func()error{return h.mlflowTracking.LogRun(c.Request.Context(),sdkActor(c),req.RunID,input)})
	if err!=nil {sdkServiceError(c,err);return};c.JSON(200,gin.H{})
}
...
func (req mlflowSDKWriteRequest) logBatch(operation string) (observability.MLflowLogBatch,error) {
	...
	if req.Status!="" || req.EndTime!=nil {return batch,mlflowtracking.ErrInvalid}
	if operation=="log-batch" {
		if req.Key!="" || len(req.Value)>0 || req.Timestamp!=nil || req.Step!=nil {return batch,mlflowtracking.ErrInvalid}
	} else {
		if len(batch.Metrics)+len(batch.Params)+len(batch.Tags)>0 {return batch,mlflowtracking.ErrInvalid}
		switch operation {
		case "log-metric":
			var value *float64
			if json.Unmarshal(req.Value,&value)!=nil {return batch,mlflowtracking.ErrInvalid}
		...
		case "log-parameter","set-tag":
			var value *string
			if req.Timestamp!=nil || req.Step!=nil || json.Unmarshal(req.Value,&value)!=nil || value==nil {return batch,mlflowtracking.ErrInvalid}
			pair:=observability.MLflowKeyValue{Key:req.Key,Value:*value}
			if operation=="set-tag" {batch.Tags=[]observability.MLflowKeyValue{pair}} else {batch.Params=[]observability.MLflowKeyValue{pair}}
		default:return batch,mlflowtracking.ErrInvalid
		}
	}
...
func (h *Handler) sdkAuditWrite(c *gin.Context, fn func()error) error {
	...
	if path.Base(c.FullPath())=="update" {action=repositories.MLflowAuditTrackingRunFinish}
	_,err:=h.auditMLflowTrackingWrite(c.Request.Context(),p,http.MethodPost,c.Request.URL.Path,c.GetHeader("X-Request-ID"),action,func(context.Context)(int,error){
		err:=fn();status,_,_:=mlflowTrackingErrorDetails(err);return status,err
	})
	return err
}
