...
func TestGrantedTrackingRejectsBeforeUpstream(t *testing.T){
 ...
 _,err:=service.GetRun(context.Background(),actor,strings.Repeat("b",32))
...
func TestGrantedTrackingPreservesHumanAndNamespacesMachine(t *testing.T){
 ...
 if _,err:=service.CreateExperiment(context.Background(),machine,"same-key","Machine");err!=nil {t.Fatal(err)}
 ...
 if _,err:=service.CreateExperiment(context.Background(),other,"same-key","Machine");err!=nil||upstream.createKey==firstKey {t.Fatal("keys collide between machines",err)}
}
...
func TestGrantedTrackingChecksWriteGrantAndCursorIdentity(t *testing.T){
 ...
 actor:=mlflowtracking.Actor{TenantID:"team",UserID:"integration:"+strings.Repeat("a",32),IntegrationID:strings.Repeat("a",32)}
 if err:=service.LogRun(context.Background(),actor,runID,mlflowtracking.Batch{});err!=nil||access.permission!="write" {t.Fatal("write grant not checked",err)}
 page,err:=service.ListRuns(context.Background(),actor,expID,1,"")
 ...
 if _,err=service.ListRuns(context.Background(),other,expID,1,page.NextCursor);!errors.Is(err,mlflowtracking.ErrInvalid){t.Fatal("cross identity cursor accepted",err)}
 access.denied=true
 if _,err=service.ListRuns(context.Background(),actor,expID,1,page.NextCursor);!errors.Is(err,mlflowtracking.ErrNotFound){t.Fatal("revoked grant cursor usable",err)}
}
