...
func (s *ownedImageStore) ListImagesForUser(ctx context.Context, tenant, user, kind string) ([]domain.PlatformImage, error) {
	return s.ListImages(ctx, tenant, kind)
}
...
func TestRuntimeResolutionChecksImageOwnerWithoutRoleBypass(t *testing.T) {
	...
	for _, tc := range []struct {
		tenant, user string
		allowed      bool
	}{{"team-a", "owner", true}, {"team-a", "admin", false}, {"team-b", "owner", false}, {"team-a", "", false}} {
		_, err := service.resolveRuntime(context.Background(), tc.tenant, tc.user, domain.JobSpec{Image: image.Reference})
		if tc.allowed && err != nil {
			t.Fatal(err)
		}
		if !tc.allowed && !errors.Is(err, ErrSubmissionImageNotAllowed) {
			t.Fatalf("private ref bypass %+v: %v", tc, err)
		}
	}
}
...
func TestImageListAndWorkspaceHidePersonalImageFromAdministrators(t *testing.T) {
	...
	for _, role := range []string{domain.RoleEngineer, domain.RoleTenantAdmin, domain.RoleSuperAdmin} {
		...
		for _, suffix := range []string{"", "?includeAllTenants=true"} {
			...
			imageScopeRouter(store, p).ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/api/v1/images"+suffix, nil))
			if strings.Contains(w.Body.String(), image.Reference) {
				t.Fatalf("private list leak: %s", w.Body.String())
			}
		}
		workspaceImage := image
		workspaceImage.Kind = domain.ImageKindWorkspace
		...
		if _, ok := h.resolveWorkspaceImage(c, p.TenantID, workspaceImage.Reference); ok {
			t.Fatalf("%s opened private workspace image", role)
		}
	}
}
...
func TestSharedRuntimeContractsNeverInheritOwnerImage(t *testing.T) {
	...
	for _, input := range []SubmissionInput{
		...
		if got := imageUserID(input); got != "" {
			t.Fatalf("shared contract inherited owner %q", got)
		}
	}
	if got := imageUserID(SubmissionInput{Principal: p, Origin: domain.SubmissionOriginPortal}); got != "owner" {
		t.Fatalf("ordinary training lost owner: %q", got)
	}
}
...
func TestLegacyAdapterFilteredPrivateCatalogDoesNotEnableFallback(t *testing.T) {
	...
	if !errors.Is(err, ErrSubmissionImageNotAllowed) {
		t.Fatalf("private image admitted through fallback: %v", err)
	}
	if store.listCalls != 1 {
		t.Fatalf("catalog must be resolved from one snapshot, got %d queries", store.listCalls)
	}
}
