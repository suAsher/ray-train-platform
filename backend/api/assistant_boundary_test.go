...
func TestAssistantRedactsCompleteQuotedCredentialValues(t *testing.T) {
	for _, input := range []string{
		...
		for _, secret := range []string{"two", "words", "more"} {
			if strings.Contains(got, secret) { t.Fatalf("credential fragment %q survived redaction: %q", secret, got) }
		}
		if !strings.Contains(got, "visible") || !strings.Contains(got, "[已脱敏]") { t.Fatalf("redaction damaged unrelated fields: %q", got) }
	}
}
...
func TestAssistantSlowBodiesReleaseQuerySlots(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		if _, err := fmt.Fprintf(conn, "POST /api/v1/assistant/query HTTP/1.1\r\nHost: test\r\nContent-Type: application/json\r\nContent-Length: 1000\r\nX-Test-Slow-Body: true\r\n\r\n{\"question\":\""); err != nil { t.Fatal(err) }
		select { case <-started: case <-time.After(time.Second): t.Fatal("slow body did not reach query reader") }
	}
	post := func() int {
	...
	if got := post(); got != http.StatusTooManyRequests { t.Fatalf("expected occupied slots, got %d", got) }
	for _, conn := range connections {
		if err := conn.SetReadDeadline(time.Now().Add(8*time.Second)); err != nil { t.Fatal(err) }
		response, err := http.ReadResponse(bufio.NewReader(conn), nil)
		if err != nil { t.Fatal(err) }
		_, _ = io.Copy(io.Discard, response.Body)
		response.Body.Close()
		if response.StatusCode != http.StatusRequestTimeout { t.Fatalf("expected slow-body timeout, got %d", response.StatusCode) }
	}
	for range connections {
		select { case <-finished: case <-time.After(time.Second): t.Fatal("timeout handler did not release its guards") }
	}
	if got := post(); got != http.StatusOK { t.Fatalf("body timeout did not release slots: %d", got) }
}
...
func TestAssistantRejectsUnsupportedReadDeadlineBeforeReading(t *testing.T) {
	...
	if err := http.NewResponseController(w).SetReadDeadline(time.Now()); !errors.Is(err, http.ErrNotSupported) { t.Fatal("test recorder unexpectedly supports deadlines") }
	r.ServeHTTP(w, req)
	if w.Code != http.StatusServiceUnavailable || !strings.Contains(w.Body.String(), "ASSISTANT_TRANSPORT_UNAVAILABLE") { t.Fatalf("unsupported transport was accepted: %d %s", w.Code, w.Body.String()) }
}
