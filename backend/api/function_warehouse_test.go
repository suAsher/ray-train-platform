...
func warehouseAPIRequest(r http.Handler, path, bearer, proxy string) *httptest.ResponseRecorder {
	...
	if proxy != "" {
		req.Header.Set("X-Auth-Request-Access-Token", proxy)
	}
...
func TestFunctionWarehouseDelegatesOnlyVerifiedAuthenticationToken(t *testing.T) {
	for _, tc := range []struct{ name, bearer, proxy, want string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if oidc.calls != 1 || len(client.tokens) != 1 || client.tokens[0] != tc.want {
				t.Fatal("upstream did not receive exactly the token verified by OAuth middleware")
			}
			if client.query != (fw.PageQuery{PageNum: 2, PageSize: 7, Keywords: "weights"}) || !client.deadlinePresent {
				t.Fatal("pagination or bounded request context was lost")
			}
			if strings.Contains(w.Body.String(), tc.want) || w.Header().Get("Cache-Control") != "no-store" {
				t.Fatal("credential or cache boundary violated")
			}
		})
	}
}
...
func TestFunctionWarehouseRejectsNonDelegatableCredentials(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if w.Code != tc.status || warehouseAPIErrorCode(t, w) != tc.code {
				t.Fatalf("unexpected response %d: %s", w.Code, w.Body.String())
			}
			if len(client.tokens) != 0 || oidc.calls != tc.oidcCalls {
				t.Fatal("authentication failure reached upstream or wrong verifier")
			}
		})
	}
}
...
func TestFunctionWarehouseUsesFixedConfiguredEnvironments(t *testing.T) {
	...
	var response struct {
		Data struct {
			Items []fw.Target `json:"items"`
		} `json:"data"`
	}
	if err := json.Unmarshal(w.Body.Bytes(), &response); err != nil {
		t.Fatal(err)
	}
	if w.Code != 200 || len(response.Data.Items) != 1 {
		t.Fatalf("invalid environments: %s", w.Body.String())
	}
	target := response.Data.Items[0]
	if target.Environment != fw.Development || target.BaseURL != "https://spiking-dev.wellspiking.ai" || target.GroupID != "d49a984edd3d0648a43ab050d3cc0262" {
		t.Fatal("environment target changed")
	}
	for _, tc := range []struct {
		path   string
		status int
	...
	if len(client.tokens) != 0 {
		t.Fatal("invalid or unavailable environment reached another target")
	}
}
...
func TestFunctionWarehouseRejectsInvalidQueryBeforeUpstream(t *testing.T) {
	for _, query := range []string{
		...
		t.Run(query, func(t *testing.T) {
			...
			if w.Code != 400 || warehouseAPIErrorCode(t, w) != "WAREHOUSE_QUERY_INVALID" || len(client.tokens) != 0 {
				t.Fatalf("invalid query reached upstream: %d", w.Code)
			}
		})
	}
}
...
func TestFunctionWarehouseModelTypesRequireWarehouseAccess(t *testing.T) {
	for _, denied := range []bool{false, true} {
		t.Run(fmt.Sprint(denied), func(t *testing.T) {
			client := &warehouseAPIClient{}
			if denied {
				client.detailErr = fw.ErrForbidden
			}
			r, _ := warehouseAPIRouter(map[fw.Environment]FunctionWarehouseClient{fw.Development: client}, warehouseAPIAccounts{})
			w := warehouseAPIRequest(r, "/development/warehouses/warehouse-1/model-types", "", "verified-proxy")
			wantStatus, wantCalls := 200, 2
			if denied {
				wantStatus, wantCalls = 403, 1
			}
			if w.Code != wantStatus || len(client.tokens) != wantCalls {
				t.Fatalf("access check behavior: status %d calls %d", w.Code, len(client.tokens))
			}
			for i, token := range client.tokens {
				if token != "verified-proxy" || client.warehouseIDs[i] != "warehouse-1" {
					t.Fatal("model request delegation lost source identity or target")
				}
			}
		})
	}
...
func TestFunctionWarehouseUpstreamErrorsRemainSafe(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.code, func(t *testing.T) {
			...
			if strings.Contains(w.Body.String(), "private upstream") || strings.Contains(w.Body.String(), "verified-proxy") {
				t.Fatal("upstream error leaked details")
			}
...
func TestFunctionWarehouseRouteRateLimitPreventsUpstreamCalls(t *testing.T) {
	...
	for i := 0; i < 120; i++ {
		w := warehouseAPIRequest(r, "/development/warehouses", "", "verified-proxy")
		if w.Code != 200 {
			t.Fatalf("unexpected status before limit at request %d: %d", i+1, w.Code)
		}
	}
	w := warehouseAPIRequest(r, "/development/warehouses", "", "verified-proxy")
	if w.Code != 429 || len(client.tokens) != 120 || w.Header().Get("Retry-After") == "" {
		t.Fatalf("limit did not stop upstream: status %d calls %d", w.Code, len(client.tokens))
	}
}
