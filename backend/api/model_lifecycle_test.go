...
func (s *modelStoreFake) GetVersion(context.Context, string, string) (modellifecycle.Version, error) {
...
func (s *modelStoreFake) ListModels(context.Context, modellifecycle.Filter) (modellifecycle.ModelPage, error) {
	return modellifecycle.ModelPage{Items: []modellifecycle.Model{s.model}}, nil
}
func (s *modelStoreFake) UpdateModel(_ context.Context, _ string, _ modellifecycle.ModelUpdate, _ modellifecycle.Actor) (modellifecycle.Model, error) {
	s.updated = true
	return s.model, nil
}
...
func TestSharedModelReadCrossTenantDoesNotGrantManagement(t *testing.T) {
	...
	r.ServeHTTP(w, httptest.NewRequest("GET", "/api/v1/models", nil))
	if w.Code != 200 {
	...
	if len(response.Data.Items) != 1 || response.Data.Items[0].CanManage {
		t.Fatalf("unexpected shared response: %s", w.Body.String())
	}
	request := httptest.NewRequest("PATCH", "/api/v1/models/model-1", strings.NewReader(`{"description":"stolen","revision":1}`))
	request.Header.Set("Content-Type", "application/json")
	w = httptest.NewRecorder()
	r.ServeHTTP(w, request)
	if w.Code != 403 || store.updated {
		t.Fatalf("foreign mutation accepted: %d", w.Code)
	}
}
func TestSharedModelManagementRequiresInteractiveSession(t *testing.T) {
	...
	request := httptest.NewRequest("PATCH", "/api/v1/models/model-1", strings.NewReader(`{"description":"new","revision":1}`))
	request.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, request)
	if w.Code != http.StatusForbidden || store.updated {
		t.Fatalf("PAT mutation accepted %d", w.Code)
	}
}
func TestModelUpdateRejectsUnknownAndOversizedBody(t *testing.T) {
	for _, body := range []string{`{"ownerId":"other","revision":1}`, `{"description":"` + strings.Repeat("x", 40000) + `","revision":1}`} {
		...
		req := httptest.NewRequest("PATCH", "/api/v1/models/model-1", strings.NewReader(body))
...
func TestSharedModelCreationRequiresStableRequestKey(t *testing.T) {
	for _, key := range []string{"", strings.Repeat("x", 129), "key with space"} {
		...
		r := modelRouter(h, auth.Principal{Subject: "alice", TenantID: "local", AuthType: auth.AuthTypeLocal})
		req := httptest.NewRequest("POST", "/api/v1/models", strings.NewReader(`{"name":"new model"}`))
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set("Idempotency-Key", key)
		w := httptest.NewRecorder()
		r.ServeHTTP(w, req)
		if w.Code != 400 {
			t.Fatalf("bad key status %d", w.Code)
		}
	}
}
