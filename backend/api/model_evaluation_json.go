...
func (h *Handler) decodeEvaluationJSON(c *gin.Context, target any, allowed map[string]bool) bool {
	...
	if err != nil {
		...
		if errors.As(err, &large) {
			h.writeError(c, 413, "EVALUATION_BODY_TOO_LARGE", "评估请求不能超过32 KiB，配置不能超过16 KiB")
		} else {
	...
	check.UseNumber()
	if err := checkEvaluationJSON(check, 0, allowed); err != nil {
		h.modelEvaluationError(c, me.ErrInvalid)
		return false
	}
...
func checkEvaluationJSON(decoder *json.Decoder, depth int, allowed map[string]bool) error {
	...
	case '{':
		...
		for decoder.More() {
			...
			if depth == 0 && key == "resources" {
				nested = evaluationResourceFields
			}
			if err := checkEvaluationJSON(decoder, depth+1, nested); err != nil {
				return err
			}
		}
	case '[':
		for decoder.More() {
			if err := checkEvaluationJSON(decoder, depth+1, nil); err != nil {
				return err
			}
		}
	default:
