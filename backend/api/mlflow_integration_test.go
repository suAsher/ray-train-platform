...
func (p *fakeIntegrationProvider) QueryJobRun(ctx context.Context, tenant, job, owner, run string) (observability.JobExperiment, error) {
	p.reads++
	if tenant != "team-a" || job != "job-01" || owner != "user-a" || run != "0123456789abcdef0123456789abcdef" {
		return observability.JobExperiment{}, errors.New("incorrect identity")
	}
	if _, ok := ctx.Deadline(); !ok {
		return observability.JobExperiment{}, errors.New("missing total timeout")
	}
...
func (p *fakeIntegrationProvider) LogJobRunBatch(ctx context.Context, tenant, job, owner, run string, batch observability.MLflowLogBatch) error {
	p.writes++
	if p.audit == nil || len(p.audit.audits) == 0 {
		return errors.New("write was not audited first")
	}
	if tenant != "team-a" || job != "job-01" || owner != "user-a" || run != "0123456789abcdef0123456789abcdef" {
		return errors.New("incorrect identity")
	}
	if _, ok := ctx.Deadline(); !ok {
		return errors.New("missing total timeout")
	}
	return p.err
}
...
func TestMLflowIntegrationAPIAuthorization(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.status != 200 && provider.reads+provider.writes != 0 {
...
func TestMLflowIntegrationAPIAuditFailureBlocksWrite(t *testing.T) {
	...
	if resp.Code != 503 || provider.writes != 0 {
		t.Fatalf("unaudited write: %d writes=%d", resp.Code, provider.writes)
	}
}
...
func TestMLflowIntegrationAPIBatchValidation(t *testing.T) {
	for _, body := range []string{
		...
		if resp.Code != 400 && resp.Code != 413 {
			t.Fatalf("bad body accepted: %d %s", resp.Code, resp.Body.String())
		}
		if provider.writes != 0 {
			t.Fatal("bad body forwarded")
		}
	}
}

func TestMLflowIntegrationAPIRejectsCaseVariantSchemaFields(t *testing.T) {
	for _, body := range []string{
		`{"Metrics":[{"key":"loss","value":1,"timestamp":1000,"step":1}]}`,
		`{"params":[{"Key":"x","value":"v"}]}`,
		`{"tags":[{"key":"x","Value":"v"}]}`,
		`{"metrics":[{"key":"loss","value":1,"Timestamp":1000,"step":1}]}`,
	} {
		provider := &fakeIntegrationProvider{}
		resp := integrationRequest(integrationRouter(integrationPrincipal(), provider, newFakeMLflowDashboardStore()), "POST", body)
		if resp.Code != 400 || provider.writes != 0 {
			t.Fatalf("case variant accepted: %d %s", resp.Code, resp.Body.String())
		}
	}
}
...
func TestMLflowIntegrationAPIRateLimitAndSafeErrors(t *testing.T) {
	...
	for i := 0; i < 121; i++ {
		...
		if strings.Contains(resp.Body.String(), "s3://") {
		...
		if resp.Code != 502 {
			t.Fatalf("unexpected error: %d %s", resp.Code, resp.Body.String())
		}
	}
	if !limited {
		t.Fatal("no write rate limit")
	}
}
...
func TestMLflowIntegrationAPITerminalRunConflict(t *testing.T) {
	...
	if resp.Code != 409 {
		t.Fatalf("want conflict: %d %s", resp.Code, resp.Body.String())
	}
}
...
func TestMLflowIntegrationAPIBatchConflict(t *testing.T) {
	...
	resp := integrationRequest(integrationRouter(integrationPrincipal(), p, newFakeMLflowDashboardStore()), http.MethodPost, validIntegrationBatch)
	if resp.Code != 409 || !strings.Contains(resp.Body.String(), "MLFLOW_BATCH_CONFLICT") {
		t.Fatalf("want batch conflict: %d %s", resp.Code, resp.Body.String())
	}
}
...
func TestMLflowIntegrationAPIExplicitZeroesAndEmptyValuesAreValid(t *testing.T) {
	...
	if resp.Code != 200 {
		t.Fatalf("explicit zeroes rejected: %d %s", resp.Code, resp.Body.String())
	}
}
...
func TestMLflowIntegrationAPIRejectsOversizedCollections(t *testing.T) {
	for _, field := range []string{"metrics", "params", "tags"} {
		...
		for i := range items {
			...
			if field == "metrics" {
				items[i] = map[string]any{"key": "loss", "value": 1, "timestamp": 1000, "step": i}
			}
		}
		...
		if resp.Code != 400 || provider.writes != 0 {
			t.Fatalf("oversized %s allowed: %d", field, resp.Code)
		}
	}
}
...
func TestMLflowIntegrationAPIAuditContainsOnlyMetadata(t *testing.T) {
	...
	if resp.Code != 200 || len(audit.audits) != 2 {
		t.Fatalf("missing audits: %d %+v", resp.Code, audit.audits)
	}
	if audit.audits[0].Status != 102 || audit.audits[1].Status != 200 || audit.audits[1].Principal.AuthType != auth.AuthTypePAT {
		t.Fatalf("incorrect audit stages: %+v", audit.audits)
	}
	encoded, _ := json.Marshal(audit.audits)
	if strings.Contains(string(encoded), "candidate") || strings.Contains(string(encoded), "epochs") {
