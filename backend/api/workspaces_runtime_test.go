...
func (s *workspaceRuntimeCatalog) DefaultImage(context.Context,string,string) (domain.PlatformImage,error) {
 if s.defaultImage.Reference=="" {return domain.PlatformImage{},repositories.ErrImageNotFound}
...
func TestLaunchWorkspacePreservesCatalogRayVersionAndOwnerBoundary(t *testing.T) {
 ...
 personal:=base;personal.ID="personal-workspace";personal.TenantID="team-a";personal.OwnerUserID="owner";personal.Visibility=domain.ImageVisibilityPersonal
 ...
 cases:=[]struct{name string;image domain.PlatformImage;requested string;isDefault,noCatalog bool;user,tenant,role string;expectedImage,expectedVersion string;status int}{
 ...
 for _,tc:=range cases {t.Run(tc.name,func(t *testing.T){
  ...
  if !tc.noCatalog {store:=&workspaceRuntimeCatalog{ownedImageStore:ownedImageStore{stubImageStore{images:[]domain.PlatformImage{tc.image}}}};if tc.isDefault {store.defaultImage=tc.image};catalog=store}
  ...
  tenant:=tc.tenant;if tenant=="" {tenant="team-a"};user:=tc.user;if user=="" {user="subject-1"};role:=tc.role;if role=="" {role=domain.RoleEngineer}
  router:=gin.New();router.Use(func(c *gin.Context){c.Set("ray-platform-principal",auth.Principal{TenantID:tenant,Subject:user,Roles:[]string{role},AuthType:auth.AuthTypeOIDC});c.Next()});handler.RegisterWorkspaceRoutes(router.Group("/api/v1"))
  // The caller cannot override trusted catalogue metadata with a JSON field.
  request:=httptest.NewRequest(http.MethodPost,"/api/v1/dev-workspaces",strings.NewReader(fmt.Sprintf(`{"image":%q,"gpuCount":0,"rayVersion":"9.99.0"}`,tc.requested)));request.Header.Set("Content-Type","application/json")
  response:=httptest.NewRecorder();router.ServeHTTP(response,request)
  if response.Code!=tc.status {t.Fatalf("status=%d body=%s",response.Code,response.Body.String())}
  ...
  if tc.status!=http.StatusAccepted {if err==nil {t.Fatal("denied private image created a RayCluster")};if !strings.Contains(response.Body.String(),"IMAGE_NOT_ALLOWED") {t.Fatalf("unexpected ACL response %s",response.Body.String())};return}
  if err!=nil {t.Fatal(err)}
  version,_,_:=unstructured.NestedString(cluster.Object,"spec","rayVersion");if version!=tc.expectedVersion {t.Fatalf("RayCluster version=%q want=%q",version,tc.expectedVersion)}
  containers,_,_:=unstructured.NestedSlice(cluster.Object,"spec","headGroupSpec","template","spec","containers")
  if len(containers)!=1 || containers[0].(map[string]any)["image"]!=tc.expectedImage {t.Fatalf("image and Ray version were separated: %v",containers)}
 })}
}
