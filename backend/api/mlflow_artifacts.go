...
func (h *Handler) mlflowArtifactGuard(limiter SourceArtifactLimiter, write bool) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if write && p.AuthType != auth.AuthTypePAT {
			h.writeError(c, 403, "ARTIFACT_PAT_REQUIRED", "artifact writes require a scoped access token")
			c.Abort()
			return
		}
		...
		if allowed, _ := limiter.Allow(p.TenantID+"\x00"+p.Subject+"\x00"+p.IntegrationID, action); !allowed {
			...
			h.writeError(c, 429, "ARTIFACT_RATE_LIMITED", "artifact request rate limit exceeded")
			c.Abort()
			return
		}
...
func (h *Handler) mlflowArtifactScope(c *gin.Context, write, running bool) (trackingartifacts.Scope, bool) {
	id := c.Param("runId")
	if !trackingartifacts.ValidID(id) {
		h.mlflowArtifactError(c, 0, trackingartifacts.ErrInvalid)
		return trackingartifacts.Scope{}, false
	}
	authorizer, ok := h.mlflowTracking.(trackingArtifactAuthorizer)
	if !ok || h.trackingArtifacts == nil {
		h.mlflowArtifactError(c, 0, trackingartifacts.ErrUnavailable)
		return trackingartifacts.Scope{}, false
	}
	...
	if running && run.State != "RUNNING" {
		h.mlflowArtifactError(c, 0, trackingartifacts.ErrConflict)
		return trackingartifacts.Scope{}, false
	}
	...
	if run.ID != id || owner.TenantID == "" || owner.UserID == "" || owner.TenantID != actorPrincipal(c).TenantID {
		h.mlflowArtifactError(c, 0, trackingartifacts.ErrNotFound)
		return trackingartifacts.Scope{}, false
	}
...
func (h *Handler) listMLflowArtifacts(c *gin.Context) {
	scope, ok := h.mlflowArtifactScope(c, false, false)
	if !ok {
		return
	}
	...
	if cursor != "" && !trackingartifacts.ValidID(cursor) {
		h.mlflowArtifactError(c, 0, trackingartifacts.ErrInvalid)
		return
	}
	page, err := h.trackingArtifacts.List(c.Request.Context(), scope, cursor, limit)
	if err != nil {
		h.mlflowArtifactError(c, 0, err)
		return
	}
...
func (h *Handler) initMLflowArtifact(c *gin.Context) {
	scope, ok := h.mlflowArtifactScope(c, true, true)
	if !ok {
		return
	}
	key, ok := h.mlflowTrackingIdempotencyKey(c)
	if !ok {
		return
	}
	if len(key) < 8 {
		h.writeError(c, 400, "ARTIFACT_INVALID_IDEMPOTENCY_KEY", "Idempotency-Key must contain 8 to 128 ASCII characters")
		return
	}
...
func (h *Handler) decodeMLflowArtifactInit(c *gin.Context, target *trackingartifacts.InitInput) bool {
	...
	if err != nil {
		...
		if errors.As(err, &oversized) {
			h.writeError(c, 413, "ARTIFACT_BODY_TOO_LARGE", "artifact initialization body exceeds 64 KiB")
		} else {
			h.mlflowArtifactError(c, 0, trackingartifacts.ErrInvalid)
		}
		return false
...
func parseMLflowArtifactInitJSON(body []byte) (trackingartifacts.InitInput, error) {
	var input trackingartifacts.InitInput
	if !utf8.Valid(body) {
		return input, trackingartifacts.ErrInvalid
	}
...
func (h *Handler) completeMLflowArtifact(c *gin.Context) {
	...
	h.mutateMLflowArtifact(c, 200, func(ctx context.Context) (trackingartifacts.Artifact, error) {
		return h.trackingArtifacts.Complete(ctx, scope, id)
	})
}

func (h *Handler) cancelMLflowArtifact(c *gin.Context) {
	scope, ok := h.mlflowArtifactScope(c, true, false)
	if !ok {
		return
	}
	id, ok := h.mlflowArtifactID(c)
	...
	h.mutateMLflowArtifact(c, 200, func(ctx context.Context) (trackingartifacts.Artifact, error) {
		return h.trackingArtifacts.Cancel(ctx, scope, id)
	})
}
...
func (h *Handler) mutateMLflowArtifact(c *gin.Context, success int, fn func(context.Context) (trackingartifacts.Artifact, error)) {
	...
	status, err := h.auditMLflowTrackingWrite(c.Request.Context(), actorPrincipal(c), c.Request.Method, c.Request.URL.Path, c.GetHeader("X-Request-ID"), repositories.MLflowAuditTrackingArtifactWrite, func(ctx context.Context) (int, error) {
		...
		artifact, err = fn(ctx)
		if err != nil {
			code, _, _ := mlflowArtifactErrorDetails(err)
			return code, err
		}
		return success, nil
	})
	if err != nil {
		h.mlflowArtifactError(c, status, err)
		return
	}
...
type artifactRequestReader struct {
	reader          io.Reader
	declared, count int64
	err             error
}
...
func (r *artifactRequestReader) Read(p []byte) (int, error) {
	...
	r.count += int64(n)
	if r.declared >= 0 && (r.count > r.declared || (err == io.EOF && r.count != r.declared)) {
		r.err = trackingartifacts.ErrInvalid
		return 0, r.err
	}
...
func (h *Handler) putMLflowArtifactPart(c *gin.Context) {
	...
	if c.Request.ContentLength > trackingartifacts.PartSizeBytes {
		h.writeError(c, 413, "ARTIFACT_BODY_TOO_LARGE", "artifact part exceeds 8 MiB")
		return
	}
	...
	h.mutateMLflowArtifact(c, 200, func(ctx context.Context) (trackingartifacts.Artifact, error) {
		...
		if reader.err != nil {
			return trackingartifacts.Artifact{}, reader.err
		}
		if ctx.Err() != nil {
			return trackingartifacts.Artifact{}, trackingartifacts.ErrUnavailable
		}
		return artifact, err
	})
}
...
func (h *Handler) downloadMLflowArtifact(c *gin.Context) {
	...
	if c.GetHeader("Range") != "" {
		h.writeError(c, 416, "ARTIFACT_RANGE_UNSUPPORTED", "artifact downloads do not support byte ranges")
		return
	}
	if h.mlflowDashboardStore == nil {
		h.mlflowArtifactError(c, 0, errMLflowTrackingAuditUnavailable)
		return
	}
	started := time.Now()
	...
	defer func() {
		...
		if err := h.mlflowDashboardStore.CreateMLflowAuditLog(ctx, event); err != nil {
			_ = c.Error(errMLflowTrackingAuditIncomplete)
		}
	}()
	artifact, reader, err := h.trackingArtifacts.Download(c.Request.Context(), scope, id)
	if err != nil {
		status, _, _ = mlflowArtifactErrorDetails(err)
		h.mlflowArtifactError(c, status, err)
		return
	}
	if reader == nil {
	...
	if disposition == "" || artifact.SizeBytes < 1 || artifact.SizeBytes > trackingartifacts.MaxFileBytes {
		h.mlflowArtifactError(c, 0, trackingartifacts.ErrUnavailable)
		return
	}
	// Context cancellation does not interrupt a socket blocked on a slow
	// downloader. Bound this response only, then clear the connection deadline
	// before keep-alive reuse. No asynchronous reset can race with this defer.
	...
	if copyErr != nil || n != artifact.SizeBytes {
		_ = c.Error(trackingartifacts.ErrUnavailable)
		return
	}
	status = 200
}
...
func mlflowArtifactErrorDetails(err error) (int, string, string) {
	...
	case errors.As(err, &oversized):
		return 413, "ARTIFACT_BODY_TOO_LARGE", "artifact part exceeds 8 MiB"
	case errors.Is(err, errMLflowTrackingAuditUnavailable):
		return 503, "ARTIFACT_AUDIT_UNAVAILABLE", "artifact audit is unavailable"
	case errors.Is(err, errMLflowTrackingAuditIncomplete):
		return 503, "ARTIFACT_AUDIT_INCOMPLETE", "artifact operation may have completed; read its state before retrying"
	case errors.Is(err, trackingartifacts.ErrInvalid):
		return 400, "ARTIFACT_INVALID_REQUEST", "artifact request is invalid"
	case errors.Is(err, trackingartifacts.ErrNotFound):
		return 404, "ARTIFACT_NOT_FOUND", "artifact was not found"
	case errors.Is(err, trackingartifacts.ErrConflict):
		return 409, "ARTIFACT_CONFLICT", "artifact or run state conflicts with this request"
	case errors.Is(err, trackingartifacts.ErrQuota):
		return 409, "ARTIFACT_QUOTA_EXCEEDED", "artifact storage or pending upload quota exceeded"
	default:
		return 503, "ARTIFACT_UNAVAILABLE", "artifact storage is unavailable"
	}
}
