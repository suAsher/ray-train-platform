...
func (s warehouseSyncSource) Prepare(ctx context.Context, op ws.Operation) ([]ws.File, ws.SourceIdentity, error) {
	...
	if !ok || s.h.models == nil || s.h.modelSnapshots == nil {
	...
	if job == nil || job.UserID != op.OwnerID || job.TenantID != op.TenantID {
		return nil, identity, ws.ErrForbidden
	}
	...
	case domain.StateFailed, domain.StateCanceled, domain.StateTimedOut:
		if op.Automatic {
			return nil, identity, ws.ErrSourceFailed
		}
	default:
		return nil, identity, ws.ErrPending
	}
	...
	if !ok {
		root, ok = s.h.personalStorageJobArtifactRootContext(ctx, p, job)
	}
	if !ok {
		return nil, identity, ws.ErrForbidden
	}
	...
	if !ok {
		return nil, identity, errors.New("training source is unavailable")
	}
	...
	if model.Archived {
		return nil, identity, ws.ErrConflict
	}
	...
	for i, relative := range op.Paths {
		...
		if v.State == ml.Failed {
			return nil, identity, errors.New("model snapshot failed")
		}
		if v.State != ml.Ready {
			pending = true
			continue
...
func (s warehouseSyncSource) Open(ctx context.Context, file ws.File) (io.ReadCloser, error) {
	...
	if v.State != ml.Ready || v.SHA256 != file.SHA256 || v.SizeBytes != file.Size || v.FileName != file.Name {
		return nil, ws.ErrConflict
	}
...
type warehouseSnapshotReader struct {
	*io.PipeReader
