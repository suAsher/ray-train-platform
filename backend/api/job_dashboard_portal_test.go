...
func TestRay258APIHelperResolvesExactlyOneDashboardPrefix(t *testing.T) {
	...
	for _, base := range []string{jobDashboardBasePath("job-1"), "/raytrain" + jobDashboardBasePath("job-1")} {
		...
		if len(calls) != 5 {
			t.Fatalf("missing API calls: %s", body)
		}
		...
		for i, endpoint := range []string{"api/authentication_mode", "api/grafana_health", "api/prometheus_health", "api/jobs", "api/v0/nodes"} {
			...
			if got := page.ResolveReference(reference).Path; got != base+endpoint {
				t.Fatalf("Ray helper produced duplicate/escaped path: %s; want %s", got, base+endpoint)
			}
		}
	}
}
