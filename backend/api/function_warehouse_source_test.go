...
func TestWarehouseSourceRevalidatesAccountAndJobBeforeReadingWeights(t *testing.T) {
 for _, tc := range []string{"disabled","missing","different account","moved team","roles revoked","foreign job","foreign tenant"} {
  t.Run(tc,func(t *testing.T){
   ...
   switch tc {case "disabled":repo.account.Disabled=true;case "missing":repo.found=false;case "different account":repo.account.ID="other";case "moved team":repo.account.TenantID="other";case "roles revoked":repo.account.Roles=nil;case "foreign job":repo.jobs[0].UserID="other";case "foreign tenant":repo.jobs[0].TenantID="other"}
   _,_,err := source.Prepare(context.Background(),op)
   if err==nil || repo.lookedUp!="alice" || len(models.created)!=0 || exp.calls!=0 || len(snapshots.requests)!=0 {t.Fatal("revoked or foreign source reached snapshot creation")}
  })
 }
}
func TestWarehouseSourceAutomaticAndManualTerminalRules(t *testing.T) {
 for _,tc := range []struct{state domain.State; automatic bool; want error}{
  ...
  t.Run(string(tc.state)+map[bool]string{true:" automatic",false:" manual"}[tc.automatic],func(t *testing.T){
   source,op,repo,_,_,snapshots := warehouseSourceFixture();op.Automatic=tc.automatic;repo.jobs[0].ObservedState=tc.state
   _,_,err:=source.Prepare(context.Background(),op)
   if !errors.Is(err,tc.want){t.Fatalf("got %v want %v",err,tc.want)}
   if tc.want!=nil && len(snapshots.requests)!=0 {t.Fatal("unfinished/failed automatic job was copied")}
  })
 }
}
func TestWarehouseSourceRequiresUnambiguousMLflowBeforeCopy(t *testing.T) {
 for _,failure := range []error{observability.ErrJobSourceUnavailable,observability.ErrAmbiguousJobSource} {
  source,op,_,models,exp,snapshots := warehouseSourceFixture();exp.err=failure
  _,_,err:=source.Prepare(context.Background(),op)
  if !errors.Is(err,failure)||len(models.created)!=0||len(snapshots.requests)!=0{t.Fatal("unverified MLflow source created snapshots")}
 }
}
func TestWarehouseSourcePreparesStableCopiesWithoutDownloading(t *testing.T) {
 for _,state:=range []string{ml.Pending,ml.Ready} {
  t.Run(state,func(t *testing.T){
   source,op,_,models,exp,snapshots:=warehouseSourceFixture();snapshots.state=state
   files,identity,err:=source.Prepare(context.Background(),op)
   if state==ml.Pending {if !errors.Is(err,ws.ErrPending)||len(files)!=0{t.Fatal("pending copy presented as uploadable")}} else if err!=nil||len(files)!=1||files[0].ModelID!="model-1"||files[0].SHA256!=strings.Repeat("a",64){t.Fatalf("invalid ready copy: %v %v",files,err)}
   if identity.JobID!="job-a"||identity.RunID!="run-a"||identity.ExperimentID!="42"||exp.tenant!="local" {t.Fatal("source identity lost")}
   if len(models.created)!=1||models.created[0].IdempotencyKey!="warehouse-sync:operation-1"||len(snapshots.requests)!=1||snapshots.downloads!=0{t.Fatal("copy preparation is not stable or started downloading")}
   r:=snapshots.requests[0]
   if r.SourceRoot!="ray-train/tenants/local/users/alice/train/run-1" || r.RelativePath!="checkpoints/best.pth" || r.FileName!="best.pth" || r.RunID!="run-a" || r.IdempotencyKey!="warehouse-sync:operation-1:0" {t.Fatalf("unsafe or unstable source: %+v",r)}
  })
 }
}
func TestWarehouseSourceSupportsLogicalRunsButRejectsLegacyRoots(t *testing.T) {
 ...
 job:=repo.jobs[0];job.Spec.ResolvedStorage=domain.ResolvedStorageMounts{}
 ...
 if _,_,err:=source.Prepare(context.Background(),op);err!=nil{t.Fatal(err)}
 if snapshots.requests[0].SourceRoot!="ray-train/tenants/local/users/alice/runs/job-a"{t.Fatal("logical source root not rebuilt")}
 repo.jobs=[]domain.TrainingJob{artifactJob("job-a","local")};repo.jobs[0].UserID="alice";repo.jobs[0].ObservedState=domain.StateSucceeded
 if _,_,err:=source.Prepare(context.Background(),op);!errors.Is(err,ws.ErrForbidden){t.Fatalf("legacy source unexpectedly accepted: %v",err)}
}
func TestWarehouseSourceOpenRejectsSnapshotChanges(t *testing.T) {
 for _,change:=range []string{"state","hash","size","name"} {
  t.Run(change,func(t *testing.T){
   ...
   switch change{case "state":v.State=ml.Pending;case "hash":v.SHA256=strings.Repeat("b",64);case "size":v.SizeBytes++;case "name":v.FileName="other.pth"};models.version=v
   reader,err:=source.Open(context.Background(),file)
   if reader!=nil||!errors.Is(err,ws.ErrConflict)||snapshots.downloads!=0{t.Fatal("changed snapshot was opened")}
  })
 }
}
func TestWarehouseSourceCloseReleasesBlockedSnapshotWriter(t *testing.T) {
 ...
 snapshots:=&cancellableSnapshotFake{done:make(chan struct{})};source.h.modelSnapshots=snapshots
