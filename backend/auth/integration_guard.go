...
func IntegrationRouteGuard() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if integrationRouteAllowed(c.Request.Method, c.Request.URL.Path) {
			c.Next()
			return
		}
...
func integrationRouteAllowed(method, path string) bool {
	...
	if strings.HasPrefix(path, sdk) {
		suffix := strings.TrimPrefix(path, sdk)
		if method == http.MethodGet {
			return suffix == "get"
		}
		if method == http.MethodPost {
			switch suffix {
			case "log-batch", "log-metric", "log-parameter", "set-tag", "update":
				return true
			}
		}
		return false
	...
	if len(parts) == 1 {
		return (parts[0] == "capabilities" && method == http.MethodGet) || (parts[0] == "experiments" && (method == http.MethodGet || method == http.MethodPost))
	}
	if len(parts) == 3 && parts[0] == "experiments" && integrationPathID.MatchString(parts[1]) && parts[2] == "runs" {
		return method == http.MethodGet || method == http.MethodPost
	}
	if parts[0] != "runs" || !integrationPathID.MatchString(parts[1]) {
		return false
	}
	if len(parts) == 2 {
	...
	if parts[2] != "artifacts" || !integrationPathID.MatchString(parts[3]) {
		return false
	}
	if len(parts) == 4 {
	...
	if len(parts) == 5 {
		return (parts[4] == "complete" && method == http.MethodPost) || (parts[4] == "content" && method == http.MethodGet)
	}
	if len(parts) == 6 && parts[4] == "parts" && method == http.MethodPut {
		n, err := strconv.Atoi(parts[5])
		return err == nil && n >= 1 && n <= 2560 && strconv.Itoa(n) == parts[5]
	}
	return false
}
