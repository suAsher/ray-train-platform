...
func TestPostgresImageOwnershipUpgradePreservesBaseAndChecksPrivateScope(t *testing.T) {
 ...
 } {if err:=database.Exec(sql).Error;err!=nil {t.Fatal(err)}}
 for i:=0;i<2;i++ {if err:=ApplyMigrations(database);err!=nil {t.Fatal(err)}}
 var preserved int64
 if err:=database.Raw(`SELECT COUNT(*) FROM platform_images WHERE id='base' AND tenant_id IS NULL AND is_default=TRUE AND owner_user_id='' AND visibility='' AND created_by='historical-admin'`).Scan(&preserved).Error;err!=nil||preserved!=1 {t.Fatalf("base image changed: count=%d err=%v",preserved,err)}
 for _,tc:=range []struct{visibility,owner string; tenant any; def,allowed bool}{
  ...
  if (err==nil)!=tc.allowed {t.Fatalf("constraint %+v: %v",tc,err)}
  if err==nil {if err=database.Exec(`DELETE FROM platform_images WHERE id='candidate'`).Error;err!=nil {t.Fatal(err)}}
 }
}
