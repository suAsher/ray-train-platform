...
func main() {
	...
	// Keep the worker-only callback surface absent until the complete IDC
	// configuration is enabled. A disabled feature must not expose a route that
	// accepts receipts merely because the platform happens to have a PAT pepper.
	var idcSyncCallbacks api.IDCDataSyncCallbackStore
	var idcSyncCallbackKey []byte
	if idcSyncManager != nil {
		idcSyncCallbacks = repository
		idcSyncCallbackKey = []byte(cfg.PATPepper)
	}
	...
	reconciler := newReconciler(repository, kubeClient, cfg, evaluationStore)
...
func registerAPIRoutesWithLocalAuth(router *gin.Engine, jobs *api.Handler, pats *api.PersonalAccessTokenHandler, artifacts *api.SourceArtifactHandler, locals *api.LocalAuthHandler, oauthAccounts auth.OAuth2ProxyAccountResolver, oidc auth.OIDCVerifier, pat auth.PATVerifier, localSessions auth.LocalSessionVerifier, kubeClient *k8s.Client, rays *rayapi.Handler, cfg config.Config) {
	...
	protected.Use(auth.IntegrationRouteGuard())
	...
	if cfg.MLflowNativePublicEnabled {
		// Only this native API bypasses identity middleware. Dashboard cookies,
	...
	jobs.RegisterFunctionWarehouseRoutes(interactive)
...
func newReconciler(repository *repositories.GormRepository, client *k8s.Client, cfg config.Config, evaluations ...*repositories.ModelEvaluationRepository) *k8s.Reconciler {
	if client == nil {
		return nil
	}
	reconciler := newReconcilerWithQuotaSync(repository, client, cfg, k8s.RenderOptions{
		...
		TeamNodeAffinityEnabled: cfg.KueueTeamNodeAffinityEnabled,
		...
		DedicatedNodes:          cfg.TrainingDedicatedNodes,
...
func datasetPublicationControllerOptions(cfg config.Config) datasetpublisher.ControllerOptions {
	return datasetpublisher.ControllerOptions{
		...
		CredentialSecretName:  cfg.DatasetPublisherCredentialSecret,
...
func (store *managedCredentialJobStore) GetByID(ctx context.Context, jobID string) (*domain.TrainingJob, error) {
	...
	if job.SubmissionOrigin == domain.SubmissionOriginEvaluation && store.evaluations == nil {
