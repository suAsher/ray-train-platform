...
func (s *Service) Download(ctx context.Context, scope Scope, id string) (Artifact, io.ReadCloser, error) {
	...
	if a.State != "READY" || len(a.UploadedParts) != a.TotalParts {
		return Artifact{}, nil, ErrConflict
	}
	ctx, cancel := context.WithTimeout(ctx, OperationTimeout)
	stream := &partReader{ctx: ctx, cancel: cancel, objects: s.objects, id: id, parts: a.UploadedParts}
...
func (r *partReader) open() error {
	...
	body, size, err := r.objects.Get(r.ctx, r.id, part.Index)
	if err != nil {
		return failure(err)
	}
	if body == nil || size != part.SizeBytes {
		if body != nil {
			body.Close()
		}
		return ErrUnavailable
...
func (r *partReader) Read(p []byte) (int, error) {
	...
	if err == io.EOF {
		...
		r.current = nil
		if r.count != part.SizeBytes || hex.EncodeToString(r.sum.Sum(nil)) != part.SHA256 || closeErr != nil {
			r.Close()
