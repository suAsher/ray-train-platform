...
func evaluationCodeKey(id string) (string, error) {
	if !me.ValidCodeSnapshotID(id) { return "", me.ErrInvalid }
...
func (s *evaluationCodeStore) Publish(ctx context.Context, id string, artifact domain.SourceArtifact) (me.CodeSnapshot, error) {
	if err := ctx.Err(); err != nil { return me.CodeSnapshot{}, err }
	key, err := evaluationCodeKey(id)
	if err != nil { return me.CodeSnapshot{}, err }
	if artifact.Validate() != nil || artifact.SizeBytes > me.MaxEvaluationCodeSize { return me.CodeSnapshot{}, me.ErrInvalid }
	if artifact.State != domain.SourceArtifactReady { return me.CodeSnapshot{}, me.ErrNotReady }
	if s == nil || s.store == nil { return me.CodeSnapshot{}, me.ErrUnavailable }
	writer, ok := s.store.client.(interface{ Put(context.Context, tosPutRequest) error })
	if !ok { return me.CodeSnapshot{}, me.ErrUnavailable }
	...
	if err != nil { return me.CodeSnapshot{}, err }
	defer spool.Close()
	if err := validateEvaluationCodeZIP(ctx, spool.file, artifact.SizeBytes); err != nil { return me.CodeSnapshot{}, err }
	...
	if err != nil {
		if !errors.Is(err, ErrAlreadyExists) { return me.CodeSnapshot{}, me.ErrUnavailable }
		head, headErr := s.store.client.Head(ctx, s.store.bucket, key)
		if headErr != nil { return me.CodeSnapshot{}, me.ErrUnavailable }
		if head.SizeBytes != artifact.SizeBytes || head.Metadata["sha256"] != artifact.SHA256 { return me.CodeSnapshot{}, me.ErrConflict }
	}
...
func (s *evaluationCodeStore) readVerified(ctx context.Context, key string, size int64, digest string) (*evaluationCodeSpool, error) {
	if s == nil || s.store == nil { return nil, me.ErrUnavailable }
	reader, ok := s.store.client.(tosArtifactReadClient)
	if !ok { return nil, me.ErrUnavailable }
	result, err := reader.ReadArtifact(ctx, tosArtifactReadRequest{Bucket: s.store.bucket, Key: key})
	if err != nil {
		if result.Content != nil { _ = result.Content.Close() }
		if errors.Is(err, ErrNotFound) { return nil, me.ErrNotFound }
		return nil, me.ErrUnavailable
	}
	if result.Content == nil { return nil, me.ErrUnavailable }
	defer result.Content.Close()
	if result.SizeBytes != size || size < 1 || size > me.MaxEvaluationCodeSize { return nil, me.ErrInvalid }
	file, err := os.CreateTemp("", "raytrain-evaluation-code-*")
	if err != nil { return nil, me.ErrUnavailable }
	spool := &evaluationCodeSpool{file: file, name: file.Name()}
	complete := false
	defer func() { if !complete { _ = spool.Close() } }()
	hash := sha256.New()
	...
	if err != nil { return nil, me.ErrUnavailable }
	if written != size || hex.EncodeToString(hash.Sum(nil)) != digest { return nil, me.ErrInvalid }
	if _, err := file.Seek(0, io.SeekStart); err != nil { return nil, me.ErrUnavailable }
	complete = true
	return spool, nil
}
...
func validateEvaluationCodeZIP(ctx context.Context, file *os.File, size int64) error {
	...
	if err != nil || len(archive.File) < 1 || len(archive.File) > maxEvaluationCodeEntries { return me.ErrInvalid }
	...
	for _, member := range archive.File {
		...
		if name == "" || path.IsAbs(name) || name == "." || name == ".." || path.Clean(name) != name || strings.HasPrefix(name, "../") || strings.ContainsAny(name, "\\:%") || strings.IndexFunc(name, unicode.IsControl) >= 0 || seen[name] { return me.ErrInvalid }
		seen[name] = true
		if !member.Mode().IsRegular() && !member.Mode().IsDir() { return me.ErrInvalid }
		if member.UncompressedSize64 > maxEvaluationCodeExpandedSize - expanded { return me.ErrInvalid }
		expanded += member.UncompressedSize64
		if member.Mode().IsDir() {
			if member.UncompressedSize64 != 0 { return me.ErrInvalid }
			continue
		}
		...
		if err := ctx.Err(); err != nil { return err }
		if readErr != nil || closeErr != nil || written != int64(member.UncompressedSize64) { return me.ErrInvalid }
	}
	return nil
}

type evaluationCodeContextReader struct { ctx context.Context; reader io.Reader }
func (r evaluationCodeContextReader) Read(p []byte) (int, error) {
	if err := r.ctx.Err(); err != nil { return 0, err }
	return r.reader.Read(p)
}

type evaluationCodeSpool struct { file *os.File; name string; closed bool }
...
func (s *evaluationCodeSpool) Close() error {
	...
	if closeErr != nil || removeErr != nil && !errors.Is(removeErr, os.ErrNotExist) { return me.ErrUnavailable }
	return nil
}
