...
type evaluationLookupStub struct {
	value modelevaluation.Evaluation
	err   error
	calls int
}

func (s *evaluationLookupStub) GetEvaluationByJobID(_ context.Context, _ string) (modelevaluation.Evaluation, error) {
	s.calls++
...
func TestTrustedEvaluationRuntimeLoadsOnlyMatchingDatabaseOwnership(t *testing.T) {
	...
	if lookup.calls != 1 || trusted.Spec.EvaluationRuntime == nil || trusted.Spec.EvaluationRuntime.EvaluationID != record.ID || trusted.Spec.EvaluationRuntime.ConfigJSON != "{}" {
		t.Fatal("trusted database runtime not loaded")
	}
	if job.Spec.EvaluationRuntime != nil {
		t.Fatal("caller job mutated")
	}
	for _, mutate := range []func(*modelevaluation.Evaluation){func(v *modelevaluation.Evaluation) { v.JobID = "other-job" }, func(v *modelevaluation.Evaluation) { v.OwnerID = "other-owner" }, func(v *modelevaluation.Evaluation) { v.TenantID = "other-team" }} {
		next := record
		mutate(&next)
		if _, err := loadTrustedEvaluationRuntime(context.Background(), job, &evaluationLookupStub{value: next}); err == nil {
			t.Fatal("foreign evaluation metadata accepted")
		}
	}
	if _, err := loadTrustedEvaluationRuntime(context.Background(), job, &evaluationLookupStub{err: errors.New("database down")}); err == nil {
		t.Fatal("lookup failure ignored")
	}
	ordinary := *trusted
	ordinary.SubmissionOrigin = domain.SubmissionOriginAPI
	before := lookup.calls
	normal, err := loadTrustedEvaluationRuntime(context.Background(), &ordinary, lookup)
	if err != nil || lookup.calls != before || normal.Spec.EvaluationRuntime != nil {
		t.Fatal("ordinary job retained injected runtime or queried evaluations")
	}
}

func TestEvaluationDatasetResolverAllowsZeroTrainOnlyForTrustedEvaluation(t *testing.T) {
	version := readyDatasetVersion()
	version.TrainSamples = 0
	...
	if err != nil || mount.TrainSamples != 0 {
		t.Fatalf("val/test-only dataset rejected or fabricated train count: %+v %v", mount, err)
	}
	request.Evaluation = false
	if _, err := resolver.ResolveDatasetManifestMount(context.Background(), request); err == nil {
		t.Fatal("normal training accepted zero train samples")
	}
	request.Evaluation = true
	catalog.version.TrainSamples = -1
	if _, err := resolver.ResolveDatasetManifestMount(context.Background(), request); err == nil {
		t.Fatal("negative train sample count accepted")
	}
}
