...
func integrationClient(t *testing.T, run map[string]any, writes *int) *MLflowClient {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/2.0/mlflow/experiments/get-by-name":
			if r.URL.Query().Get("experiment_name") != "raytrain-team-a" { t.Error("wrong tenant experiment") }
			fmt.Fprint(w, `{"experiment":{"experiment_id":"7"}}`)
		case "/api/2.0/mlflow/runs/get":
			if r.URL.Query().Get("run_id") != integrationRunID { t.Error("wrong exact run") }
			json.NewEncoder(w).Encode(map[string]any{"run": run})
		case "/api/2.0/mlflow/metrics/get-history":
			if r.URL.Query().Get("run_id") != integrationRunID { t.Error("history from wrong run") }
			fmt.Fprint(w, `{"metrics":[{"value":0.5,"timestamp":1000,"step":1}]}`)
		case "/api/2.0/mlflow/runs/log-batch":
			*writes++
			var body map[string]any
			json.NewDecoder(r.Body).Decode(&body)
			if body["run_id"] != integrationRunID || r.Header.Get("Authorization") != "" || r.Header.Get("Cookie") != "" { t.Error("unsafe upstream request") }
			fmt.Fprint(w, `{}`)
		default:
...
func TestMLflowIntegrationReadsExactOlderRunAndSanitizes(t *testing.T) {
	...
	if err != nil || result.Run == nil || result.Run.ID != integrationRunID || result.Run.Params["epochs"] != "5" || len(result.Series) != 1 { t.Fatalf("exact run: %+v %v", result, err) }
	encoded, _ := json.Marshal(result)
	if strings.Contains(string(encoded), "s3://") || strings.Contains(string(encoded), "provenance") { t.Fatalf("leaked internal fields: %s", encoded) }
}
	...
	for _, field := range []string{"run_id", "experiment_id", "platform.job_id", "platform.tenant_id", "platform.submitter_user_id", "platform.provenance"} {
		t.Run(field, func(t *testing.T) {
			...
			if strings.HasPrefix(field, "platform.") {
				for _, tag := range run["data"].(map[string]any)["tags"].([]map[string]any) { if tag["key"] == field { tag["value"] = "forged" } }
			} else { run["info"].(map[string]any)[field] = "forged" }
			writes := 0
			client := integrationClient(t, run, &writes)
			if _, err := client.QueryJobRun(context.Background(), "team-a", "job-01", "user-a", integrationRunID); !errors.Is(err, ErrMLflowRunNotFound) { t.Fatalf("forged binding accepted: %v", err) }
			if err := client.LogJobRunBatch(context.Background(), "team-a", "job-01", "user-a", integrationRunID, MLflowLogBatch{Params: []MLflowKeyValue{{Key:"epochs", Value:"5"}}}); !errors.Is(err, ErrMLflowRunNotFound) || writes != 0 { t.Fatalf("forged write: %v writes=%d", err, writes) }
		})
	}
}
...
func TestMLflowIntegrationWritesOnlyRunningRun(t *testing.T) {
	for _, status := range []string{"RUNNING", "FINISHED", "FAILED", "KILLED"} {
		t.Run(status, func(t *testing.T) {
			...
			if status == "RUNNING" { if err != nil || writes != 1 { t.Fatalf("write failed: %v %d", err, writes) } } else if !errors.Is(err, ErrMLflowRunNotRunning) || writes != 0 { t.Fatalf("terminal write allowed: %v %d", err, writes) }
		})
	}
}
