...
type mlflowIntegrationRun struct {
	...
	Data struct {
		Metrics []struct {Key string `json:"key"`; Value mlflowMetricValue `json:"value"`} `json:"metrics"`
...
func (c *MLflowClient) QueryJobRun(ctx context.Context, tenant, job, owner, runID string) (JobExperiment,error) {
	...
	if err!=nil {return JobExperiment{},err}
...
func (c *MLflowClient) LogJobRunBatch(ctx context.Context, tenant, job, owner, runID string,batch MLflowLogBatch) error {
	...
	if run.Info.Status!="RUNNING" {return ErrMLflowRunNotRunning}
	endpoint,err:=c.endpoint("/api/2.0/mlflow/runs/log-batch")
	if err!=nil {return err}
	body:=struct {RunID string `json:"run_id"`;MLflowLogBatch}{RunID:runID,MLflowLogBatch:batch}
	var result map[string]any
	status,err:=c.doJSON(ctx,http.MethodPost,endpoint,body,&result)
	if status==http.StatusBadRequest||status==http.StatusConflict {return ErrMLflowBatchConflict}
	return err
}
...
func (c *MLflowClient) integrationRun(ctx context.Context,tenant,job,owner,runID string,write bool)(mlflowIntegrationRun,string,error) {
	...
	if c==nil||strings.TrimSpace(c.BaseURL)==""||len(c.ProvenanceKey)<32 {return zero,"",fmt.Errorf("MLflow integration is not configured")}
	if !safeLabelValue(tenant)||!safeLabelValue(job)||strings.TrimSpace(owner)==""||len(owner)>256||!mlflowIntegrationRunID.MatchString(runID) {return zero,"",ErrMLflowRunNotFound}
	prefix:=strings.Trim(strings.TrimSpace(c.ExperimentPrefix),"-");if prefix=="" {prefix="raytrain"}
	if !safeLabelValue(prefix) {return zero,"",fmt.Errorf("MLflow experiment prefix is invalid")}
	name:=prefix+"-"+tenant
	experimentID,found,err:=c.experimentID(ctx,name)
	if err!=nil {return zero,"",err};if !found {return zero,"",ErrMLflowRunNotFound}
	endpoint,err:=c.endpoint("/api/2.0/mlflow/runs/get");if err!=nil {return zero,"",err}
	query:=endpoint.Query();query.Set("run_id",runID);endpoint.RawQuery=query.Encode()
	var payload struct {Run mlflowIntegrationRun `json:"run"`}
	status,err:=c.doJSON(ctx,http.MethodGet,endpoint,nil,&payload)
	if status==http.StatusNotFound {return zero,"",ErrMLflowRunNotFound};if err!=nil {return zero,"",err}
	run:=payload.Run
	if run.Info.ID!=runID||run.Info.ExperimentID!=experimentID||!c.integrationTagsMatch(run,tenant,job,owner,write) {return zero,"",ErrMLflowRunNotFound}
	return run,name,nil
}
...
func (c *MLflowClient) integrationTagsMatch(run mlflowIntegrationRun,tenant,job,owner string,write bool) bool {
	...
	for _,tag:=range run.Data.Tags {
		if _,exists:=tags[tag.Key];exists {return false}
		tags[tag.Key]=tag.Value
	}
	if tags["platform.job_id"]!=job||!hmac.Equal([]byte(tags["platform.provenance"]),[]byte(mlflowProvenanceTag(c.ProvenanceKey,job))) {return false}
	for key,want:=range map[string]string{"platform.tenant_id":tenant,"platform.submitter_user_id":owner} {
		got,present:=tags[key]
		if (present||write)&&got!=want {return false}
	}
	return true
}
...
func sanitizeIntegrationRun(raw mlflowIntegrationRun)(*ExperimentRun,[]string) {
	...
	for _,metric:=range raw.Data.Metrics {
		if len(run.Latest)<maxMLflowMetricKeys&&safeMetricKey(metric.Key)&&metric.Value.Valid {run.Latest[metric.Key]=metric.Value.Value}
	}
	for _,param:=range raw.Data.Params {
		if len(run.Params)<100&&validMLflowIntegrationKey(param.Key) {run.Params[param.Key]=truncate(param.Value,1024)}
	}
	keys:=make([]string,0,len(run.Latest));for key:=range run.Latest {keys=append(keys,key)};sort.Strings(keys)
	return run,keys
}
