...
func TestMLflowIntegrationRealServerSmoke(t *testing.T) {
	...
	var experiment struct { ID string `json:"experiment_id"` }
	smokeMLflowJSON(t, ctx, client, "/api/2.0/mlflow/experiments/create", map[string]any{"name": "raytrain-" + tenant}, &experiment)
	if experiment.ID == "" { t.Fatal("MLflow did not create the isolated experiment") }
	t.Cleanup(func() {
		cleanupCtx, cleanupCancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer cleanupCancel()
		endpoint, endpointErr := client.endpoint("/api/2.0/mlflow/experiments/delete")
		if endpointErr != nil { t.Error(endpointErr); return }
		var result map[string]any
		if _, deleteErr := client.doJSON(cleanupCtx, http.MethodPost, endpoint, map[string]any{"experiment_id": experiment.ID}, &result); deleteErr != nil { t.Errorf("clean isolated experiment: %v", deleteErr) }
	})
	...
	for _, batch := range []MLflowLogBatch{
		...
		if err := client.LogJobRunBatch(ctx, tenant, job, owner, newRun, batch); err != nil { t.Fatalf("real single-field batch: %v", err) }
	}
	result, err := client.QueryJobRun(ctx, tenant, job, owner, newRun)
	if err != nil || result.Run == nil || result.Run.Params["epochs"] != "5" || result.Run.Latest["loss"] != value || len(result.Series) != 1 || len(result.Series[0].Points) != 1 { t.Fatalf("real run readback: %+v %v", result, err) }
	var raw struct { Run mlflowIntegrationRun `json:"run"` }
	endpoint, _ := client.endpoint("/api/2.0/mlflow/runs/get")
	query := endpoint.Query(); query.Set("run_id", newRun); endpoint.RawQuery = query.Encode()
	if _, err := client.doJSON(ctx, http.MethodGet, endpoint, nil, &raw); err != nil { t.Fatal(err) }
	tagFound := false
	for _, tag := range raw.Run.Data.Tags { if tag.Key == "review" && tag.Value == "candidate" { tagFound = true } }
	if !tagFound { t.Fatal("custom tag was not persisted") }
	older, err := client.QueryJobRun(ctx, tenant, job, owner, oldRun)
	if err != nil || older.Run == nil || older.Run.ID != oldRun || older.Run.ID == newRun { t.Fatalf("older legacy attempt was not read exactly: %+v %v", older, err) }
	if err := client.LogJobRunBatch(ctx, tenant, job, owner, oldRun, MLflowLogBatch{Tags: []MLflowKeyValue{{Key: "review", Value: "legacy"}}}); !errors.Is(err, ErrMLflowRunNotFound) { t.Fatalf("legacy run write was allowed: %v", err) }
	...
	if err := client.LogJobRunBatch(ctx, tenant, job, owner, newRun, MLflowLogBatch{Tags: []MLflowKeyValue{{Key: "review", Value: "too-late"}}}); !errors.Is(err, ErrMLflowRunNotRunning) { t.Fatalf("terminal run write was allowed: %v", err) }
}
...
func createSmokeMLflowRun(t *testing.T, ctx context.Context, client *MLflowClient, experimentID string, tags []MLflowKeyValue, start int64) string {
	t.Helper()
	var response struct { Run mlflowIntegrationRun `json:"run"` }
	...
	if !mlflowIntegrationRunID.MatchString(response.Run.Info.ID) { t.Fatal("isolated run was not created") }
...
func smokeMLflowJSON(t *testing.T, ctx context.Context, client *MLflowClient, path string, body, result any) {
	...
	if _, err := client.doJSON(ctx, http.MethodPost, endpoint, body, result); err != nil { t.Fatalf("isolated MLflow %s: %v", path, err) }
}
...
func verifySmokeMLflowVersion(t *testing.T, ctx context.Context, client *MLflowClient) {
	...
	if err != nil || resp.StatusCode != 200 || !strings.HasPrefix(strings.TrimSpace(string(version)), "3.14.") { t.Fatalf("smoke test requires MLflow 3.14.x, status=%d version=%q error=%v", resp.StatusCode, version, err) }
}
