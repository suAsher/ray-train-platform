...
func TestPublishTransportPreservesSafeRetryClassification(t *testing.T) {
 for _, original := range []error{secretTimeout{}, io.ErrUnexpectedEOF, syscall.ECONNRESET} {
  ...
  if !errors.Is(err,ErrUnavailable) || !errors.As(err,&temporary) || !temporary.Temporary() || strings.Contains(err.Error(),"secret-fixture") || errors.Is(err,original) {t.Fatalf("unsafe retry error: %v",err)}
  if _,ok := original.(net.Error); ok { var got net.Error; if !errors.As(err,&got) || !got.Timeout(){t.Fatal("timeout classification lost")} }
 }
}
...
func TestPublishTransportDiagnosticsExposeOnlySafeFields(t *testing.T) {
 ...
 guard := &publishTransport{repository:"team/model",base:roundTripFunc(func(*http.Request)(*http.Response,error){ response:=reply(413,"secret-fixture"); return response,nil })}
 ...
 if _,err=guard.RoundTrip(request); err==nil {t.Fatal("unsafe target accepted")}
 payload,_:=json.Marshal(events)
 if strings.Contains(string(payload),"secret-fixture") || strings.Contains(string(payload),"invalid") || len(events)!=3 || events[1].Status!=413 || events[1].Method!="PATCH" || events[2].Code!="TRANSPORT_TARGET_REJECTED" { t.Fatalf("unsafe or incomplete diagnostics: %s",payload) }
}
...
func TestPublishNonEmptyLayoutRetriesInterruptedLayerAndReportsStages(t *testing.T) {
 ...
 if err!=nil || result.ImageDigest!=digest.String() || patches.Load()<3{t.Fatalf("retry publish failed result=%+v err=%v patches=%d",result,err,patches.Load())}
 if strings.Join(stages,",")!="LAYOUT_VALIDATING,REGISTRY_AUTH,REGISTRY_WRITE,REGISTRY_VERIFY"{t.Fatalf("missing stage evidence: %v",stages)}
}
