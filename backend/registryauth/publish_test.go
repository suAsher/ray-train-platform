...
func testLayout(t *testing.T) (string,string) {
 ...
 p,err:=layout.Write(directory,empty.Index);if err!=nil{t.Fatal(err)}
 if err:=p.AppendImage(empty.Image);err!=nil{t.Fatal(err)}
 digest,err:=empty.Image.Digest();if err!=nil{t.Fatal(err)}
...
func TestLoadPublishedImageChecksFrozenDigest(t *testing.T) {
 ...
 if _,err:=loadPublishImage(context.Background(),directory,"sha256:"+string(make([]byte,64)));!errors.Is(err,ErrArtifact){t.Fatalf("invalid digest accepted: %v",err)}
}

func TestLoadPublishedImageRejectsSymlinkAndCorruptBlob(t *testing.T) {
 directory,digest:=testLayout(t)
 if err:=os.Symlink("/etc/passwd",filepath.Join(directory,"unexpected"));err!=nil{t.Fatal(err)}
 if _,err:=loadPublishImage(context.Background(),directory,digest);!errors.Is(err,ErrArtifact){t.Fatalf("symlink accepted: %v",err)}
 if err:=os.Remove(filepath.Join(directory,"unexpected"));err!=nil{t.Fatal(err)}
 if err:=os.WriteFile(filepath.Join(directory,"blobs","sha256",digest[len("sha256:"):]),[]byte(`{"schemaVersion":2}`),0600);err!=nil{t.Fatal(err)}
 if _,err:=loadPublishImage(context.Background(),directory,digest);!errors.Is(err,ErrArtifact){t.Fatalf("corrupt blob accepted: %v",err)}
}
...
func TestPublishRejectsInvalidTagBeforeReadingCredentials(t *testing.T) {
 ...
 if !errors.Is(err,ErrInvalidTarget){t.Fatalf("expected invalid target, got %v",err)}
}
