...
func TestPublishSeparatesUploadConfirmationFromIdentityTimeout(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodPatch && r.ContentLength == layerSize {
			layerPatches.Add(1)
	...
	if err != nil || result.ImageDigest != digest {
		t.Fatalf("delayed registry confirmation failed: %+v %v layerPatches=%d", result, err, layerPatches.Load())
	}
	if layerPatches.Load() < 1 {
		t.Fatal("non-empty layer PATCH was not exercised")
	}
	if base.ResponseHeaderTimeout != 50*time.Millisecond || client.http.Timeout != 20*time.Second {
		t.Fatal("publishing changed identity-query timeout")
	}
}
