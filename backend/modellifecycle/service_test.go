...
func (r *fakeRepository) FinishVersion(_ context.Context,id,lease,state string,parts []Part,hash string,_ time.Time)error{r.finished=Version{ID:id,LeaseID:lease,State:state,Parts:parts,SHA256:hash};return r.finishErr}
func (r *fakeRepository) RenewVersion(context.Context,string,string,time.Time,time.Time)error{return r.renewErr}
func (r *fakeRepository) ClaimVersion(_ context.Context,lease string,_ time.Time,_ time.Time)(Version,error){r.claims++;if r.claims==1 && !r.failClaim{v:=r.reserved;v.LeaseID=lease;return v,nil};r.runCancel();return Version{},ErrNotFound}
...
func TestRequestVersionPersistsOnlyMetadataAndIdempotency(t *testing.T) {
 src:=&testSource{data:[]byte("weights"),reported:7};repo:=&fakeRepository{findErr:ErrNotFound};objects:=&testObjects{data:map[int][]byte{}};s:=NewService(repo,src,objects)
...
func (s brokenSource) Read(context.Context,string,string)(io.ReadCloser,int64,error){return nil,1,s.err}
func TestRequestVersionValidationAndReadFailures(t *testing.T) {
 ...
 for _,size:=range []int64{0,MaxFileSize+1}{src:=&testSource{reported:size};if _,err:=NewService(repo,src,objects).RequestVersion(ctx,request);!errors.Is(err,ErrInvalid) || !src.closed{t.Fatalf("size %d: %v",size,err)}}
 ...
 }{bad:=request;modify(&bad);if _,err:=NewService(repo,&testSource{reported:1},objects).RequestVersion(ctx,bad);!errors.Is(err,ErrInvalid){t.Fatalf("invalid request accepted: %+v %v",bad,err)}}
 ...
 if _,err:=NewService(repo,&testSource{reported:1},objects).RequestVersion(ctx,request);!errors.Is(err,ErrUnavailable){t.Fatal(err)}
}
...
func TestCopyStreamsMultiplePartsAndWholeDigest(t *testing.T) {
 data:=bytes.Repeat([]byte("x"),PartSize+13);src:=&testSource{data:data,reported:int64(len(data))};objects:=&testObjects{data:map[int][]byte{}}
 service:=NewService(nil,src,objects);v:=Version{ID:"snapshot",SizeBytes:int64(len(data)),SourceRoot:"/source",RelativePath:"weights"}
...
func TestCopyRejectsChangedSourceLengthAndUploadFailure(t *testing.T) {
 ...
 }{t.Run(tc.name,func(t *testing.T){src:=&testSource{data:[]byte(tc.data),reported:tc.reported};s:=NewService(nil,src,&testObjects{data:map[int][]byte{}});if _,_,err:=s.copy(context.Background(),Version{SizeBytes:tc.size});err==nil || !src.closed{t.Fatalf("invalid copy succeeded %v",err)}})}
 src:=&testSource{data:[]byte("a"),reported:1};s:=NewService(nil,src,failingObjects{err:ErrUnavailable})
 if _,_,err:=s.copy(context.Background(),Version{SizeBytes:1});!errors.Is(err,ErrUnavailable){t.Fatal(err)}
 ctx,cancel:=context.WithCancel(context.Background());cancel()
 if _,_,err:=s.copy(ctx,Version{SizeBytes:1});!errors.Is(err,context.Canceled){t.Fatal(err)}
 if _,_,err:=NewService(nil,brokenSource{err:ErrNotFound},nil).copy(context.Background(),Version{SizeBytes:1});!errors.Is(err,ErrNotFound){t.Fatal(err)}
}
func TestProcessPublishesOnlyCompleteSnapshot(t *testing.T) {
 for _,failure:=range []bool{false,true}{
  repo:=&fakeRepository{};src:=&testSource{data:[]byte("weights"),reported:7};var objects Objects=&testObjects{data:map[int][]byte{}}
  if failure{objects=failingObjects{err:ErrUnavailable}}
  s:=NewService(repo,src,objects);s.process(context.Background(),Version{ID:"version",LeaseID:"owned-lease",SizeBytes:7})
 ...
 NewService(repo,&testSource{data:[]byte("a"),reported:1},&testObjects{}).process(ctx,Version{SizeBytes:1})
...
func TestRunDrainsPersistentRequestsAndStops(t *testing.T) {
 for _,empty:=range []bool{false,true}{
  ctx,cancel:=context.WithCancel(context.Background());repo:=&fakeRepository{reserved:Version{ID:"persisted",SizeBytes:1},runCancel:cancel,failClaim:empty}
  s:=NewService(repo,&testSource{data:[]byte("a"),reported:1},&testObjects{data:map[int][]byte{}});s.Run(ctx);cancel()
...
func TestDownloadFullHashManifestAndLengthChecks(t *testing.T) {
 ...
 v:=Version{ID:"v",State:Ready,SizeBytes:int64(len(data)),SHA256:digest(data),Parts:[]Part{{Index:0,SizeBytes:int64(len(data)),SHA256:digest(data)}}}
...
func TestVersionJSONDoesNotExposeStorageOrLease(t *testing.T) {
 raw,err:=json.Marshal(Version{SourceRoot:"private-source",RelativePath:"private-file",LeaseID:"private-lease",IdempotencyKey:"private-key",RequestSHA256:"private-hash",Parts:[]Part{{SHA256:"private-part"}}});if err!=nil{t.Fatal(err)}
 if strings.Contains(string(raw),"private-"){t.Fatalf("private storage metadata exposed: %s",raw)}
}

type waitingSource struct{}
func (waitingSource) Read(ctx context.Context,_ string,_ string)(io.ReadCloser,int64,error){return &waitingReader{ctx:ctx},1,nil}
...
func TestLeaseLossCancelsCopyAndNeverPublishesReady(t *testing.T) {
 ...
 s.process(ctx,Version{ID:"leased",LeaseID:"stale",SizeBytes:1})
