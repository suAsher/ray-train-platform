...
type evaluationRuntimeLookup interface {GetEvaluationByJobID(context.Context,string)(modelevaluation.Evaluation,error)}
...
func loadTrustedEvaluationRuntime(ctx context.Context,job *domain.TrainingJob,lookup evaluationRuntimeLookup)(*domain.TrainingJob,error){
 if job==nil{return nil,fmt.Errorf("job was not found")}
 loaded:=*job
 loaded.Spec.EvaluationRuntime=nil
 if job.SubmissionOrigin!=domain.SubmissionOriginEvaluation{return &loaded,nil}
 if lookup==nil{return nil,fmt.Errorf("evaluation runtime repository is not configured")}
 evaluation,err:=lookup.GetEvaluationByJobID(ctx,job.ID)
 if err!=nil{return nil,fmt.Errorf("load trusted evaluation context: %w",err)}
 if evaluation.JobID!=job.ID||evaluation.TenantID!=job.TenantID||evaluation.OwnerID!=job.UserID{return nil,fmt.Errorf("evaluation ownership does not match job")}
 ...
 if err:=runtime.Validate();err!=nil{return nil,err}
