...
func (c *Client) Upload(ctx context.Context, token, name, prefix string, size int64, digest string, open func(context.Context) (io.ReadCloser, error)) (UploadedFile, error) {
	if !validUploadInput(name, prefix, size, digest) || open == nil {
		return UploadedFile{}, ErrInvalid
	}
	if !validToken(token) {
		return UploadedFile{}, ErrUnauthorized
	}
	...
	if size <= simpleUploadLimit {
		...
		if err != nil || reader == nil {
			return UploadedFile{}, ErrUnavailable
		}
		defer reader.Close()
...
func (c *Client) UploadFile(ctx context.Context, token, name, prefix string, size int64, digest string, reader io.Reader) (UploadedFile, error) {
	if !validUploadInput(name, prefix, size, digest) || size > simpleUploadLimit || reader == nil {
		return UploadedFile{}, ErrInvalid
	}
	if !validToken(token) {
		return UploadedFile{}, ErrUnauthorized
	...
	if file, ok := completedFile(response, name, size, digest); ok {
		return file, nil
	}
...
func (c *Client) uploadChunks(ctx context.Context, token, name, prefix string, size int64, digest string, chunkSize int64, open func(context.Context) (io.ReadCloser, error)) (UploadedFile, error) {
	...
	for index := int64(0); index < chunks; index++ {
		...
		if _, err := io.ReadFull(checked, buffer[:length]); err != nil {
			return UploadedFile{}, ErrUnknownOutcome
		}
		chunk := buffer[:length]
...
func hashSource(ctx context.Context, reader io.Reader, size int64, digest string) (string, error) {
	...
	if count != size || hex.EncodeToString(sha.Sum(nil)) != digest {
		return "", ErrInvalid
	}
...
func (r *integrityReader) Read(p []byte) (int, error) {
	...
	if r.remaining == 0 {
		...
		if extraN != 0 || extraErr != io.EOF || hex.EncodeToString(r.hash.Sum(nil)) != r.digest {
			return n, ErrInvalid
		}
		r.verified = true
		return n, io.EOF
...
func completedFileDepth(data json.RawMessage, name string, size int64, digest string, depth int) (UploadedFile, bool) {
	...
	if status := stringField(object, "status"); status == "error" || status == "uploading" {
		return UploadedFile{}, false
	}
		...
		if raw, ok := object[key]; ok {
			if json.Unmarshal(raw, &file.FileSize) != nil {
				...
				if json.Unmarshal(raw, &text) == nil {
					file.FileSize, _ = strconv.ParseInt(text, 10, 64)
				}
			}
			break
		}
...
func (c *Client) awaitUploadedFile(ctx context.Context, token, name string, size int64, digest string) (UploadedFile, error) {
	for attempt := 0; attempt < 60; attempt++ {
		...
		if err := c.get(ctx, token, "/system/api/file/minio/upload/process", url.Values{"sha256": {digest}}, &data); err != nil {
			return UploadedFile{}, ErrUnknownOutcome
		}
		var status struct {
			Status string `json:"status"`
		}
		if json.Unmarshal(data, &status) != nil || status.Status == "error" {
			return UploadedFile{}, ErrUnknownOutcome
		}
		if file, ok := completedFile(data, name, size, digest); ok {
			return file, nil
		}
		if attempt == 59 {
			break
