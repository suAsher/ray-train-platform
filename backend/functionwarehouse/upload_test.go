...
func TestLiveRelativePreviewUploadResponse(t *testing.T) {
	...
	if !ok || actual != expected { t.Fatalf("live relative URL rejected or changed: %#v", actual) }
	if !strings.Contains(actual.URL, "20260914//2026") { t.Fatal("upstream path was normalized") }
}
...
func TestRelativePreviewURLRejectsUnsafePaths(t *testing.T) {
	for _, address := range []string{
		...
		file.URL = address
		if validUploadedFile(file) { t.Errorf("unsafe upload URL accepted: %q", address) }
	}
}
...
func TestUploadFileStreamingMultipartAndVerifiedResult(t *testing.T) {
	...
	client := testClient(t, func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/system/api/file/minio/upload" || r.URL.Query().Get("prefix") != testUploadPrefix || r.Header.Get("Authorization") != "Bearer token" { t.Error("bad upload contract") }
		reader, err := r.MultipartReader()
		if err != nil { t.Fatal(err) }
		part, err := reader.NextPart()
		if err != nil || part.FormName() != "file" || part.FileName() != "weights.pth" { t.Fatal("missing multipart file") }
		actual, err := io.ReadAll(part)
		if err != nil || !bytes.Equal(actual, data) { t.Fatal("incorrect uploaded bytes") }
		if _, err := reader.NextPart(); err != io.EOF { t.Fatal("multipart was not terminated") }
	...
	if err != nil || file != uploadedFixture(data) { t.Fatalf("upload=%#v error=%v", file, err) }
}
...
func TestUploadProcessVerifiedSnakeCaseResult(t *testing.T) {
	...
	client := testClient(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/system/api/file/minio/upload/process" || r.URL.Query().Get("sha256") != digestOf(data) { t.Error("bad process query") }
	...
	if err != nil || file != uploadedFixture(data) || posts.Load() != 1 { t.Fatalf("upload=%#v error=%v posts=%d", file, err, posts.Load()) }
}
...
func TestUploadRejectsLocalIntegrityMismatch(t *testing.T) {
	...
	for _, mode := range []string{"sha", "short", "long"} {
		t.Run(mode, func(t *testing.T) {
			client := testClient(t, func(w http.ResponseWriter, r *http.Request) { _, _ = io.Copy(io.Discard, r.Body); writeEnvelope(w, uploadedFixture(data)) })
			digest := digestOf(data)
			size := int64(len(data))
			if mode == "sha" { digest = strings.Repeat("0", 64) }
			if mode == "short" { size++ }
			if mode == "long" { size-- }
			if _, err := client.UploadFile(context.Background(), "token", "weights.pth", testUploadPrefix, size, digest, bytes.NewReader(data)); !errors.Is(err, ErrUnknownOutcome) { t.Fatalf("integrity mismatch accepted: %v", err) }
		})
	}
}
...
func TestUploadRejectsUnsafeInputWithoutOpeningSource(t *testing.T) {
	...
	for _, input := range []struct { name, prefix string; size int64 }{
		...
		if !errors.Is(err, ErrInvalid) { t.Errorf("unsafe input accepted: %v", err) }
	}
}
...
func TestChunkUploadTwoPassIntegrityAndExactProtocol(t *testing.T) {
	...
	client := testClient(t, func(w http.ResponseWriter, r *http.Request) {
		if err := r.ParseMultipartForm(1024); err != nil { t.Fatal(err) }
		defer r.MultipartForm.RemoveAll()
		switch r.URL.Path {
		case "/system/api/file/minio/chunk/upload":
			index := int(chunks.Add(1))-1
			if r.FormValue("currentChunk") != fmt.Sprint(index) || r.FormValue("chunks") != "3" || r.FormValue("fileMd5") != hex.EncodeToString(fileMD5[:]) || r.FormValue("path") != testUploadPrefix || r.FormValue("size") != "17" { t.Error("bad chunk fields") }
			...
			if err != nil || !bytes.Equal(actual, expected) || r.FormValue("chunkSize") != fmt.Sprint(len(expected)) || r.FormValue("currentChunkMd5") != hex.EncodeToString(digest[:]) { t.Error("bad chunk bytes or digest") }
			writeEnvelope(w, nil)
		case "/system/api/file/minio/chunk/merge":
			merges.Add(1)
			if r.FormValue("chunkCount") != "3" || r.FormValue("fileName") != "weights.pth" || r.FormValue("sha256") != digestOf(data) || r.FormValue("prefix") != testUploadPrefix { t.Error("bad merge fields") }
			writeEnvelope(w, uploadedFixture(data))
		default: t.Errorf("unexpected endpoint %s", r.URL.Path)
		}
	})
	file, err := client.uploadChunks(context.Background(), "token", "weights.pth", testUploadPrefix, int64(len(data)), digestOf(data), 6, func(context.Context) (io.ReadCloser, error) { opens.Add(1); return io.NopCloser(bytes.NewReader(data)), nil })
	if err != nil || file != uploadedFixture(data) || opens.Load() != 2 || chunks.Load() != 3 || merges.Load() != 1 { t.Fatalf("file=%#v err=%v opens=%d chunks=%d merges=%d", file, err, opens.Load(), chunks.Load(), merges.Load()) }
}
...
func TestChunkSourceChangedBetweenPassesNeverMerges(t *testing.T) {
	...
	var opens atomic.Int32
	client := testClient(t, func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/system/api/file/minio/chunk/upload" { t.Error("merged or cleaned invalid shared chunks") }
		_, _ = io.Copy(io.Discard, r.Body)
		writeEnvelope(w, nil)
	})
	_, err := client.uploadChunks(context.Background(), "token", "weights.pth", testUploadPrefix, int64(len(data)), digestOf(data), 6, func(context.Context) (io.ReadCloser, error) {
		if opens.Add(1) == 1 { return io.NopCloser(bytes.NewReader(data)), nil }
		return io.NopCloser(strings.NewReader(strings.Repeat("z", len(data)))), nil
	})
	if !errors.Is(err, ErrUnknownOutcome) { t.Fatalf("changed source accepted: %v", err) }
}
...
func TestChunkFirstPassMismatchDoesNotMutate(t *testing.T) {
	...
	if !errors.Is(err, ErrInvalid) { t.Fatalf("bad first pass accepted: %v", err) }
}

func TestUploadServerIntegrityFailureRemainsUnknown(t *testing.T) {
	data := []byte("weights")
	client := testClient(t, func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "POST" { _, _ = io.Copy(io.Discard, r.Body); writeEnvelope(w, map[string]any{"status": "uploading"}); return }
	...
	if !errors.Is(err, ErrUnknownOutcome) || strings.Contains(err.Error(), "private") { t.Fatalf("unverified result accepted: %v", err) }
}
