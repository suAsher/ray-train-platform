...
func (c *Client) CreateVersion(ctx context.Context, token string, input CreateVersionRequest) (Version, error) {
	...
	if !canEdit(warehouse.PermissionCodes) {
	...
	if !c.matchesVersion(result, input) {
...
func (c *Client) VerifyVersion(ctx context.Context, token, warehouseID, versionID string, expected CreateVersionRequest) (Version, error) {
	if !validID(versionID) || warehouseID != expected.FunctionWarehouseID || !validCreate(expected) {
		return Version{}, ErrInvalid
	}
	ctx, cancel := context.WithTimeout(ctx, c.requestTimeout)
	defer cancel()
	for pageNum := 1; pageNum <= 20; pageNum++ {
		...
		for _, version := range page.Records {
			if version.ID == versionID {
				if !c.matchesVersion(version, expected) {
					return Version{}, ErrUnknownOutcome
				}
				return version, nil
			}
		}
		if len(page.Records) < 100 || int64(pageNum*100) >= page.Total {
			break
		}
	}
	return Version{}, ErrUnknownOutcome
...
func validCreate(req CreateVersionRequest) bool {
	if !validID(req.FunctionWarehouseID) || !validID(req.ModelTypeID) || !validID(req.JobID) || !validID(req.RunID) || !validID(req.ExperimentID) {
		return false
	}
	if strings.TrimSpace(req.Version) == "" || len(req.Version) > 128 || !utf8.ValidString(req.Version) || strings.ContainsAny(req.Version, "\x00\r\n") || len(req.Description) > 16384 || !utf8.ValidString(req.Description) || strings.ContainsRune(req.Description, 0) {
		return false
	}
...
func (c *Client) matchesVersion(version Version, expected CreateVersionRequest) bool {
	if !validID(version.ID) || version.GroupID != c.target.GroupID || version.FunctionWarehouseID != expected.FunctionWarehouseID || version.ModelTypeID != expected.ModelTypeID || version.Version != expected.Version || version.JobID != expected.JobID || version.RunID != expected.RunID || version.ExperimentID != expected.ExperimentID {
		return false
	}
...
func (c *Client) mutate(ctx context.Context, token, path string, params url.Values, contentType string, body io.Reader, length int64, target any) error {
	...
	if err != nil {
		return &Error{Kind: ErrUnknownOutcome}
	}
	defer response.Body.Close()
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return mutationError(response.StatusCode, response.StatusCode, response.Header.Get("Retry-After"))
	}
	data, err := io.ReadAll(io.LimitReader(response.Body, c.maxResponseBytes+1))
	if err != nil || int64(len(data)) > c.maxResponseBytes {
		return &Error{Kind: ErrUnknownOutcome, StatusCode: response.StatusCode}
	}
	var envelope struct {
		Code *int            `json:"code"`
	...
	if *envelope.Code != 0 {
		return mutationError(*envelope.Code, response.StatusCode, response.Header.Get("Retry-After"))
	}
	if target != nil && (len(envelope.Data) == 0 || json.Unmarshal(envelope.Data, target) != nil) {
		return &Error{Kind: ErrUnknownOutcome, StatusCode: response.StatusCode}
	}
	return nil
}
