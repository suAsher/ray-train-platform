...
func TestCreateVersionUsesConfirmedSourceAndAllowsNewDuplicates(t *testing.T) {
	...
	client := testClient(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if _, ok := body["mlflowRunId"]; ok { t.Error("overwrote report source") }
		writes.Add(1)
		writeEnvelope(w, versionWire(req))
	})
	for i := 0; i < 2; i++ {
		result, err := client.CreateVersion(context.Background(), "token", req)
		if err != nil || result.ID != "version-1" || result.RunID != req.RunID { t.Fatalf("create=%#v error=%v", result, err) }
	}
	if writes.Load() != 2 { t.Fatal("separate intentional creates were deduplicated") }
}
		...
		t.Run(mode, func(t *testing.T) {
			...
			client := testClient(t, func(w http.ResponseWriter, r *http.Request) {
				...
				default:
					...
					if mode == "source-mismatch" { body["runId"] = "other" } else { body["paths"] = "[]" }
					writeEnvelope(w, body)
				}
			})
			_, err := client.CreateVersion(context.Background(), "secret-token", createFixture())
			if !errors.Is(err, ErrUnknownOutcome) || writes.Load() != 1 || strings.Contains(err.Error(), "secret-token") { t.Fatalf("unsafe outcome: %v writes=%d", err, writes.Load()) }
		})
	}
}
	...
	for _, mode := range []string{"read-only", "foreign-model"} {
		client := testClient(t, func(w http.ResponseWriter, r *http.Request) {
			...
			case "/model/functionWarehouse":
				...
				if mode == "read-only" { row.PermissionCodes = []string{"view", "not-admin"} }
				writeEnvelope(w, row)
			case "/model/model/type/list": writeEnvelope(w, []ModelType{{ID: "other"}})
			default: t.Error("unauthorized mutation reached upstream")
			}
		})
		if _, err := client.CreateVersion(context.Background(), "token", createFixture()); !errors.Is(err, ErrForbidden) { t.Errorf("authorization accepted: %v", err) }
	}
}
...
func TestVerifyVersionFindsSourceAndFileOnBoundedPage(t *testing.T) {
	...
	if err != nil || result.ID != "version-1" { t.Fatalf("verify=%#v error=%v", result, err) }
	wrong := req
	wrong.ExperimentID = "another"
	if _, err := client.VerifyVersion(context.Background(), "token", req.FunctionWarehouseID, "version-1", wrong); !errors.Is(err, ErrUnknownOutcome) { t.Fatalf("mismatch accepted: %v", err) }
}
...
func TestLiveCreateResponseRelativeFilePaths(t *testing.T) {
	...
	for _, arrayPaths := range []bool{false, true} {
		...
		if arrayPaths { body["paths"] = req.Paths }
		encoded, err := json.Marshal(body)
		if err != nil { t.Fatal(err) }
		var actual Version
		if err := json.Unmarshal(encoded, &actual); err != nil { t.Fatal(err) }
		if !client.matchesVersion(actual, req) { t.Fatalf("live %t paths response mismatched: %#v", arrayPaths, actual) }
		if actual.Files[0] != liveUploadedFixture() { t.Fatal("live URL changed during version decoding") }
	}
}
