...
func TestEvaluationRuntimeOnlyInjectedIntoEvaluationWorkers(t *testing.T) {
	...
	for _, expected := range []string{"MODEL_EVALUATION_ID", "MODEL_EVALUATION_CONFIG_JSON", "MODEL_EVALUATION_DATASET_MANIFEST_SHA256", "/jobs/" + job.ID + "/model-evaluation", "RAYTRAIN_EVENT_TOKEN_FILE"} {
		if !strings.Contains(string(encoded), expected) {
			t.Fatalf("worker lacks %s", expected)
		}
	}
	for _, pod := range []map[string]any{head, submitter} {
		raw, _ := json.Marshal(pod)
		if strings.Contains(string(raw), "MODEL_EVALUATION_") {
			t.Fatal("evaluation env escaped worker")
		}
	}
	if strings.Contains(string(encoded), "RAYTRAIN_PAT") || strings.Contains(string(encoded), "Authorization") {
		t.Fatal("worker received personal credential")
	}
	job.SubmissionOrigin = domain.SubmissionOriginAPI
	normal, err := RenderRayJob(job, options)
	if err != nil {
		t.Fatal(err)
	}
	raw, _ := json.Marshal(normal.Object)
	if strings.Contains(string(raw), "MODEL_EVALUATION_") {
		t.Fatal("normal job got evaluation runtime")
	}
}

func TestEvaluationRuntimeJSONCannotSupplyTrustedFields(t *testing.T) {
	var spec domain.JobSpec
	if err := json.Unmarshal([]byte(`{"evaluationRuntime":{"EvaluationID":"forged"}}`), &spec); err != nil {
		t.Fatal(err)
	}
	if spec.EvaluationRuntime != nil {
		t.Fatal("public JSON populated trusted runtime")
	}
	...
	if strings.Contains(string(raw), "evaluation-1") {
		t.Fatal("trusted runtime leaked to serialized JobSpec")
	}
}
...
func TestEvaluationRendererFailsClosedWithoutTrustedRuntime(t *testing.T) {
	for _, mutate := range []func(*domain.TrainingJob){
		...
		if _, err := RenderRayJob(job, testRenderOptions()); err == nil {
			t.Fatal("untrusted evaluation runtime rendered")
		}
	}
}

type recordingEvaluationFinalizer struct {
	calls int
	jobID string
	err   error
}

func (f *recordingEvaluationFinalizer) FinalizeEvaluationJob(_ context.Context, job *domain.TrainingJob) error {
	f.calls++
	f.jobID = job.ID
	return f.err
}
func TestTerminalEvaluationFinalizationIndependentOfMLflow(t *testing.T) {
	for _, withMLflow := range []bool{false, true} {
		...
		if withMLflow {
			reconciler.WithExperimentFinalizer(&recordingExperimentFinalizer{err: errors.New("MLflow down")})
		}
		...
		if evaluation.calls != 1 || evaluation.jobID != job.ID {
			t.Fatal("evaluation finalization skipped")
		}
		if withMLflow && err == nil {
			t.Fatal("MLflow failure did not retain retry")
		}
	}
}
func TestTerminalEvaluationFinalizationSkipsNormalAndNonterminalJobs(t *testing.T) {
	for _, normal := range []bool{false, true} {
		...
		reconciler := NewReconciler(&memoryJobStore{job: &job}, nil, testRenderOptions()).WithEvaluationFinalizer(evaluation)
		err := reconciler.processEvent(context.Background(), domain.OutboxEvent{ID: "terminal", EventType: "TRAINING_JOB_TERMINAL", Payload: []byte(`{"job_id":"` + job.ID + `"}`)})
		if evaluation.calls != 0 {
			t.Fatal("ineligible job finalized")
		}
		if !normal && err == nil {
			t.Fatal("nonterminal event accepted")
		}
	}
}
func TestEvaluationFinalizationFailureStillRunsMLflow(t *testing.T) {
	...
	evaluation := &recordingEvaluationFinalizer{err: errors.New("report unavailable")}
	mlflow := &recordingExperimentFinalizer{}
	...
	err := reconciler.processEvent(context.Background(), domain.OutboxEvent{ID: "terminal", EventType: "TRAINING_JOB_TERMINAL", Payload: []byte(`{"job_id":"` + job.ID + `"}`)})
	if err == nil || mlflow.jobID != job.ID {
		t.Fatal("evaluation error swallowed or MLflow skipped")
	}
}

type runtimeReloadStore struct {
	*memoryJobStore
	loaded *domain.TrainingJob
	reads  int
	err    error
}

func (s *runtimeReloadStore) GetByID(_ context.Context, _ string) (*domain.TrainingJob, error) {
	s.reads++
	return s.loaded, s.err
}
func TestEvaluationRuntimeRestoredAfterAttemptSnapshotWithoutLosingLeaseState(t *testing.T) {
	loaded := evaluationRenderFixture()
	loaded.ClusterAttempt = 1
	current := loaded
	current.Spec.EvaluationRuntime = nil
	current.ClusterAttempt = 2
	current.RayJobName = "attempt-two"
	...
	if store.reads != 1 || restored.Spec.EvaluationRuntime == nil || restored.ClusterAttempt != 2 || restored.RayJobName != "attempt-two" {
		t.Fatal("restore lost current attempt or runtime")
	}
	if current.Spec.EvaluationRuntime != nil {
		t.Fatal("restore mutated caller snapshot")
	}
	manifest, err := RenderRayJob(*restored, testRenderOptions())
	if err != nil {
		t.Fatal(err)
	}
	raw, _ := json.Marshal(manifest.Object)
	if !strings.Contains(string(raw), "MODEL_EVALUATION_ID") {
		t.Fatal("new attempt lacks evaluation env")
	}
	ordinary := current
	ordinary.SubmissionOrigin = domain.SubmissionOriginAPI
	if _, err := reconciler.restoreEvaluationRuntime(context.Background(), &ordinary); err != nil {
		t.Fatal(err)
	}
	if store.reads != 1 {
		t.Fatal("normal job triggered extra DB lookup")
	}
}
func TestEvaluationRuntimeRestoreRejectsDifferentDatabaseIdentity(t *testing.T) {
	for _, mutate := range []func(*domain.TrainingJob){
		...
		current := evaluationRenderFixture()
		current.Spec.EvaluationRuntime = nil
		loaded := evaluationRenderFixture()
		mutate(&loaded)
		store := &runtimeReloadStore{memoryJobStore: &memoryJobStore{job: &current}, loaded: &loaded}
		reconciler := NewReconciler(store, nil, testRenderOptions())
		if _, err := reconciler.restoreEvaluationRuntime(context.Background(), &current); err == nil {
			t.Fatal("mismatched trusted runtime copied")
		}
	}
}
...
func TestEvaluationEntrypointAlwaysUsesManagedDriverWrapper(t *testing.T) {
	job := evaluationRenderFixture()
	job.Spec.Entrypoint = domain.Entrypoint{Command: []string{"python", "examples/model-evaluation/smoke_evaluator.py"}}
	...
	if !strings.HasPrefix(entrypoint, "raytrain-managed ") || !strings.Contains(entrypoint, " -- python examples/model-evaluation/smoke_evaluator.py") {
		t.Fatalf("evaluation bypassed managed worker wrapper: %s", entrypoint)
	}
}
...
func TestEvaluationMountedInputDoesNotInitializeTrainShard(t *testing.T) {
	...
	job.SubmissionOrigin = domain.SubmissionOriginEvaluation
	job.Spec.EvaluationRuntime = evaluationRenderFixture().Spec.EvaluationRuntime
	...
	spec, _, _ := nestedMap(manifest.Object, "spec")
	entrypoint, _ := spec["entrypoint"].(string)
	if !strings.Contains(entrypoint, "--data-mode mount") || strings.Contains(entrypoint, "--data-mode streaming") || strings.Contains(entrypoint, "--dataset-") {
		t.Fatalf("evaluation entrypoint initializes training data: %s", entrypoint)
	}
	raw, _ := json.Marshal(manifest.Object)
	if !strings.Contains(string(raw), "PLATFORM_DATASET_MANIFEST_PATH") || !strings.Contains(string(raw), streamingManifestDigest) {
		t.Fatal("mounted input provenance lost")
	}
	normal := job
	normal.SubmissionOrigin = domain.SubmissionOriginAPI
	baseline, err := RenderRayJob(normal, streamingRenderOptions())
	if err != nil {
		t.Fatal(err)
	}
	normalSpec, _, _ := nestedMap(baseline.Object, "spec")
	normalEntry, _ := normalSpec["entrypoint"].(string)
	if !strings.Contains(normalEntry, "--data-mode streaming") || strings.Contains(normalEntry, "--data-mode mount") {
		t.Fatal("normal streaming driver changed")
	}
	if job.Spec.DataMode != domain.DataModeStreaming {
		t.Fatal("evaluation driver override mutated persisted data mode")
	}
}
...
func TestEvaluationMountAllowsZeroTrainSamplesWithoutWeakeningNormalTraining(t *testing.T) {
	...
	options.DatasetManifest.TrainSamples = 0
	if _, err := RenderRayJob(job, options); err != nil {
		t.Fatal(err)
	}
	normal := job
	normal.SubmissionOrigin = domain.SubmissionOriginAPI
	if _, err := RenderRayJob(normal, options); err == nil {
		t.Fatal("normal training accepted zero train samples")
	}
	options.DatasetManifest.TrainSamples = -1
	if _, err := RenderRayJob(job, options); err == nil {
		t.Fatal("evaluation accepted negative train samples")
	}
}
...
func TestEvaluationRuntimeValidationRejectsNonEvaluationInputs(t *testing.T) {
	for _, mutate := range []func(*domain.EvaluationRuntime){
		...
		value := *evaluationRenderFixture().Spec.EvaluationRuntime
		mutate(&value)
		if value.Validate() == nil {
			t.Fatal("invalid evaluation runtime accepted")
		}
	}
}
