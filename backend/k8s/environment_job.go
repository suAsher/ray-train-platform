...
func (r *EnvironmentRunner) renderJob(b environmentbuild.Build) (*batchv1.Job, error) {
	...
	zero := int32(0)
	one := int32(1)
	seconds := int64(r.config.JobTimeout.Seconds())
	uid := int64(1000)
	no := false
	yes := true
	mode := int32(0440)
	...
	case environmentbuild.Validating:
		...
		if json.Unmarshal([]byte(b.ChecksJSON), &provenance) != nil || !environmentDigestPattern.MatchString(provenance.LayerDigest) {
			return nil, fmt.Errorf("environment layer provenance is unavailable")
		}
		if !isDNSSubdomain(r.config.PullSecretName) {
			return nil, fmt.Errorf("environment assembler pull authorization is not configured")
		}
	...
	case environmentbuild.Pushing:
		if !environmentDigestPattern.MatchString(b.ArtifactDigest) {
			return nil, environmentbuild.ErrInvalid
		}
		container.Image = r.config.PublisherImage
