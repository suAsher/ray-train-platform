...
func environmentHash(value string) string { sum:=sha256.Sum256([]byte(value));return hex.EncodeToString(sum[:16]) }
func environmentResourceName(kind,id string) string { return "rt-env-"+kind+"-"+environmentHash(id) }
...
func environmentDeleteOptions(meta metav1.Object) metav1.DeleteOptions {
 uid:=meta.GetUID();rv:=meta.GetResourceVersion()
 return metav1.DeleteOptions{Preconditions:&metav1.Preconditions{UID:&uid,ResourceVersion:&rv}}
}

type EnvironmentVault struct { client *Client; namespace string }
var _ environmentbuild.Vault = (*EnvironmentVault)(nil)
func NewEnvironmentVault(client *Client,namespace string) *EnvironmentVault { return &EnvironmentVault{client:client,namespace:namespace} }
func (v *EnvironmentVault) Put(ctx context.Context,id string,ciphertext []byte,expires time.Time) error {
 if v.client==nil || v.client.kubernetes==nil || !isDNSLabel(v.namespace) || id=="" || len(ciphertext)==0 || len(ciphertext)>64*1024 || !expires.After(time.Now()) {return environmentbuild.ErrInvalid}
 immutable:=true
 ...
 if !apierrors.IsAlreadyExists(err) {return err}
 existing,err:=v.client.kubernetes.CoreV1().Secrets(v.namespace).Get(ctx,secret.Name,metav1.GetOptions{})
 if err!=nil {return err}
 if !environmentOwned(existing,id) || existing.Immutable==nil || !*existing.Immutable || !bytes.Equal(existing.Data["ciphertext"],ciphertext) {return fmt.Errorf("refusing to replace environment authorization")}
 return nil
}
func (v *EnvironmentVault) Get(ctx context.Context,id string) ([]byte,error) {
 ...
 if err!=nil || !expires.After(time.Now()) {return nil,environmentbuild.ErrAuthorization}
 if len(secret.Data["ciphertext"])==0 {return nil,environmentbuild.ErrAuthorization}
...
func (v *EnvironmentVault) Delete(ctx context.Context,id string) error {
 ...
 if !environmentOwned(secret,id) {return fmt.Errorf("refusing to delete unmanaged environment authorization")}
 err=secrets.Delete(ctx,secret.Name,environmentDeleteOptions(secret));if apierrors.IsNotFound(err) {return nil};return err
}
