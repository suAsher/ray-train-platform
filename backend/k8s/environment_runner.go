...
func (r *EnvironmentRunner) Step(ctx context.Context, b environmentbuild.Build, credentials *environmentbuild.Credentials) (environmentbuild.StepResult, error) {
	...
	if b.Status == environmentbuild.Pushing {
		if credentials == nil {
			return environmentbuild.StepResult{}, environmentbuild.ErrAuthorization
		}
	...
	if !r.owned(job, b) {
		return environmentbuild.StepResult{}, fmt.Errorf("refusing unmanaged environment Job")
	}
...
func environmentPublishSecretName(b environmentbuild.Build) string {
	return environmentResourceName("push", b.ID+"/"+strconv.Itoa(b.Attempt))
}
func (r *EnvironmentRunner) ensureArtifacts(ctx context.Context, b environmentbuild.Build) error {
	if r.config.StorageGiB < 40 || r.config.StorageGiB > 80 || len(b.SnapshotJSON) == 0 || len(b.SnapshotJSON) > 1024*1024 || !json.Valid([]byte(b.SnapshotJSON)) {
	...
	if !r.owned(pvc, b) {
		return fmt.Errorf("refusing unmanaged environment volume")
	}
	maps := r.client.kubernetes.CoreV1().ConfigMaps(r.config.Namespace)
	name = environmentResourceName("snapshot", b.ID)
	cm, err := maps.Get(ctx, name, metav1.GetOptions{})
	if apierrors.IsNotFound(err) {
		immutable := true
		cm, err = maps.Create(ctx, &corev1.ConfigMap{ObjectMeta: r.metadata(name, b), Immutable: &immutable, Data: map[string]string{"capture.json": b.SnapshotJSON}}, metav1.CreateOptions{})
		if apierrors.IsAlreadyExists(err) {
			cm, err = maps.Get(ctx, name, metav1.GetOptions{})
		}
	}
	...
	if !r.owned(cm, b) || cm.Data["capture.json"] != b.SnapshotJSON {
		return fmt.Errorf("environment snapshot conflict")
	}
	return nil
}
func (r *EnvironmentRunner) ensurePublishSecret(ctx context.Context, b environmentbuild.Build, credentials environmentbuild.Credentials) error {
	if credentials.Username == "" || credentials.Secret == "" || len(credentials.Username) > 512 || len(credentials.Secret) > 32768 {
		return environmentbuild.ErrAuthorization
	}
	secrets := r.client.kubernetes.CoreV1().Secrets(r.config.Namespace)
	name := environmentPublishSecretName(b)
	secret, err := secrets.Get(ctx, name, metav1.GetOptions{})
	if apierrors.IsNotFound(err) {
		immutable := true
		secret, err = secrets.Create(ctx, &corev1.Secret{ObjectMeta: r.metadata(name, b), Type: corev1.SecretTypeOpaque, Immutable: &immutable, Data: map[string][]byte{"username": []byte(credentials.Username), "secret": []byte(credentials.Secret)}}, metav1.CreateOptions{})
		if apierrors.IsAlreadyExists(err) {
			secret, err = secrets.Get(ctx, name, metav1.GetOptions{})
		}
	}
	if err != nil {
		return fmt.Errorf("create scoped publishing credential failed")
	}
	if !r.owned(secret, b) {
		return fmt.Errorf("refusing unmanaged publishing credential")
	}
	return nil
}
	...
	for _, condition := range job.Status.Conditions {
		if condition.Type == batchv1.JobFailed && condition.Status == corev1.ConditionTrue {
			...
			if condition.Reason == "DeadlineExceeded" {
				...
				return environmentbuild.StepResult{}, &environmentbuild.PhaseError{Code: code}
			}
		}
	}
	if job.Status.Succeeded < 1 && !failed {
		return environmentbuild.StepResult{}, nil
	}
	pods, err := r.client.kubernetes.CoreV1().Pods(r.config.Namespace).List(ctx, metav1.ListOptions{LabelSelector: labels.Set{"batch.kubernetes.io/job-name": job.Name}.AsSelector().String()})
	if err != nil {
		return environmentbuild.StepResult{}, err
	}
	for _, pod := range pods.Items {
		if !r.owned(&pod, b) || (!failed && pod.Status.Phase != corev1.PodSucceeded) {
			continue
		}
		owner := false
		for _, ref := range pod.OwnerReferences {
			if ref.Kind == "Job" && ref.UID == job.UID && ref.Controller != nil && *ref.Controller {
				owner = true
			}
		}
		if !owner {
		...
		for _, status := range pod.Status.ContainerStatuses {
			if status.Name != "environment" || status.State.Terminated == nil {
				continue
			}
			if failed {
				var failure struct {
				...
				if b.Status == environmentbuild.Pushing && len(status.State.Terminated.Message) <= 4096 && json.Unmarshal([]byte(status.State.Terminated.Message), &failure) == nil && (failure.ErrorCode == "REGISTRY_AUTH_REQUIRED" || failure.ErrorCode == "REGISTRY_PUSH_DENIED") {
					return environmentbuild.StepResult{}, environmentbuild.ErrAuthorization
				}
				if safe := environmentSafeFailure([]byte(status.State.Terminated.Message)); safe != nil {
					return environmentbuild.StepResult{}, safe
				}
				if b.Status == environmentbuild.VerifyingPull {
					return environmentbuild.StepResult{}, &environmentbuild.PhaseError{Code: "PULL_FAILED"}
				}
			...
			if b.Status == environmentbuild.VerifyingPull {
				return environmentbuild.StepResult{Done: true, ChecksJSON: `{"pullVerified":true,"cpuImportCheck":true,"gpuValidation":"not_run"}`}, nil
			}
			message := status.State.Terminated.Message
			if len(message) > 4096 {
				return environmentbuild.StepResult{}, fmt.Errorf("environment result exceeds limit")
			}
			var result struct {
			...
			if json.Unmarshal([]byte(message), &result) != nil {
				return environmentbuild.StepResult{}, fmt.Errorf("environment Job returned invalid result")
			}
			if b.Status == environmentbuild.Building && environmentDigestPattern.MatchString(result.LayerSHA256) {
				...
				if result.CaptureSHA256 != hex.EncodeToString(expected[:]) {
					return environmentbuild.StepResult{}, fmt.Errorf("environment build capture provenance mismatch")
				}
				for _, check := range []string{"wheelHashesVerified", "rebuiltFilesVerified", "pipCheck", "platformCPU"} {
					if result.Checks[check] != true {
						return environmentbuild.StepResult{}, fmt.Errorf("environment build validation incomplete")
					}
				}
			...
			if b.Status == environmentbuild.Validating && environmentDigestPattern.MatchString(result.ArtifactDigest) {
				return environmentbuild.StepResult{Done: true, ArtifactDigest: result.ArtifactDigest, ChecksJSON: `{"ociBuilt":true,"dependencyCheck":true,"cpuImportCheck":true,"gpuValidation":"not_run"}`}, nil
			}
			if b.Status == environmentbuild.Pushing {
				...
				if environmentDigestPattern.MatchString(digest) && digest == b.ArtifactDigest {
					return environmentbuild.StepResult{Done: true, ImageDigest: digest}, nil
				}
			}
...
func environmentSafeFailure(data []byte) error {
	...
	if failure.ErrorCode != "" {
		if failure.Code != "" && failure.Code != failure.ErrorCode {
			return nil
		}
		failure.Code = failure.ErrorCode
	}
	switch failure.Code {
	case "UNSUPPORTED_WORKSPACE", "ENVIRONMENT_CHANGED", "WHEEL_UNAVAILABLE", "PACKAGE_MODIFIED", "BUILD_TIMEOUT", "PULL_FAILED", "TEMP_STORAGE_FULL", "REGISTRY_PUBLISH_FAILED", "OCI_ARTIFACT_INVALID", "REGISTRY_TARGET_INVALID":
