...
type EvaluationFinalizer interface { FinalizeEvaluationJob(context.Context,*domain.TrainingJob) error }

func(r *Reconciler) WithEvaluationFinalizer(finalizer EvaluationFinalizer)*Reconciler {r.evaluationJobs=finalizer;return r}
...
func(r *Reconciler)restoreEvaluationRuntime(ctx context.Context,job *domain.TrainingJob)(*domain.TrainingJob,error){
 if job.SubmissionOrigin!=domain.SubmissionOriginEvaluation||job.Spec.EvaluationRuntime!=nil{return job,nil}
 loaded,err:=r.store.GetByID(ctx,job.ID)
 if err!=nil{return nil,err}
 if loaded==nil||loaded.ID!=job.ID||loaded.TenantID!=job.TenantID||loaded.UserID!=job.UserID||loaded.SubmissionOrigin!=domain.SubmissionOriginEvaluation||loaded.Spec.EvaluationRuntime==nil{return nil,fmt.Errorf("trusted evaluation runtime does not match the current job")}
 restored:=*job
...
func appendEvaluationRuntime(environment []any,runtime *domain.EvaluationRuntime,options RenderOptions)[]any{
 if runtime==nil{return environment}
 values:=[][2]string{
 ...
 for _,value:=range values{environment=setEnvironmentValue(environment,value[0],value[1])}
 return environment
}
