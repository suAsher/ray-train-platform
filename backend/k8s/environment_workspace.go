...
func (r *EnvironmentRunner) workspacePod(ctx context.Context, w environmentbuild.Workspace) (*corev1.Pod, error) {
	...
	if cl["app.kubernetes.io/managed-by"] != "ray-train-platform" || cl["ray.io/workspace-id"] != w.ID || cl["ray.io/tenant-id"] != w.TenantID || cl["ray.io/dev-workspace"] != "true" || cluster.GetUID() == "" || cluster.GetDeletionTimestamp() != nil {
		return nil, fmt.Errorf("workspace ownership verification failed")
	}
	selector := labels.Set{"ray.io/cluster": w.ResourceName, "ray.io/node-type": "worker"}.AsSelector().String()
	pods, err := r.client.kubernetes.CoreV1().Pods(w.Namespace).List(ctx, metav1.ListOptions{LabelSelector: selector})
	if err != nil {
		return nil, err
	}
	...
	for i := range pods.Items {
		...
		for _, ref := range pod.OwnerReferences {
			if ref.Kind == "RayCluster" && ref.Name == cluster.GetName() && ref.UID == cluster.GetUID() && ref.Controller != nil && *ref.Controller {
				owned = true
			}
		}
		if !owned {
		...
		for _, container := range pod.Spec.Containers {
			...
			if !ready {
				return nil, fmt.Errorf("workspace image has not been verified by the container runtime")
			}
			if found != nil {
				return nil, fmt.Errorf("workspace has multiple active workers")
			}
			found = pod.DeepCopy()
		}
	}
	if found == nil || found.UID == "" {
		return nil, fmt.Errorf("workspace has no ready managed worker")
	}
	return found, nil
}

type environmentBoundedBuffer struct {
	bytes.Buffer
	limit int
}
...
func (b *environmentBoundedBuffer) Write(data []byte) (int, error) {
	if len(data) > b.limit-b.Len() {
		return 0, fmt.Errorf("environment capture exceeded output limit")
	}
...
func (r *EnvironmentRunner) capture(ctx context.Context, b environmentbuild.Build) (environmentbuild.StepResult, error) {
	...
	pod, err := r.workspacePod(ctx, workspace)
	if err != nil {
	...
	if err := r.execCapture(captureCtx, pod.Namespace, pod.Name, output, diagnostic); err != nil {
		...
		if errors.Is(err, context.DeadlineExceeded) || (errors.As(err, &exit) && (exit.ExitStatus() == 124 || exit.ExitStatus() == 137)) {
			return environmentbuild.StepResult{}, &environmentbuild.PhaseError{Code: "BUILD_TIMEOUT"}
		}
		if safe := environmentSafeFailure(diagnostic.Bytes()); safe != nil {
	...
	if err := json.Unmarshal(output.Bytes(), &envelope); err != nil || envelope.SchemaVersion != 1 || envelope.BaseImage != b.BaseImage {
		return environmentbuild.StepResult{}, &environmentbuild.PhaseError{Code: "ENVIRONMENT_CHANGED"}
	}
	for _, key := range []string{"baseUnchanged", "managedOnly", "installedFilesVerified", "stableCapture"} {
		if !envelope.Checks[key] {
			return environmentbuild.StepResult{}, &environmentbuild.PhaseError{Code: "PACKAGE_MODIFIED"}
		}
	}
	after, err := r.workspacePod(ctx, workspace)
	if err != nil {
		return environmentbuild.StepResult{}, err
	}
...
func (r *EnvironmentRunner) execCapture(ctx context.Context, namespace, pod string, stdout, stderr io.Writer) error {
	if r.captureExec != nil {
		return r.captureExec(ctx, namespace, pod, stdout, stderr)
	}
	if r.client.restConfig == nil {
		return environmentbuild.ErrUnavailable
	}
	...
	executor, err := remotecommand.NewSPDYExecutor(r.client.restConfig, "POST", request.URL())
	if err != nil {
		return err
	}
