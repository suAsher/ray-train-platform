...
每次提问都是独立请求，不会自动携带此前的问题和回答，也不在服务端保存聊天历史。请在当前问题里说明功能、现象和需要确认的事项。回答附带已发布使用说明的标题、摘录和引用，可打开引用核对完整步骤；没有匹配资料时，助手会说明依据不足，不代表该功能一定不存在。

需要了解具体任务时，显式选中任务并在发送前核对任务 ID。助手沿用任务详情权限：普通成员可查询当前团队任务，平台超级管理员按既有权限查询；不能借提问绕过其他团队的访问限制。清除任务选择后，后续问题只查询使用说明，不会沿用刚才的任务上下文。
