...
| 回答模式 | 使用方式与不可用时的结果 |
| --- | --- |
| 自动 | 按管理员配置的模型顺序尝试可用服务；不可用时降级为文档检索结果 |
| API | 使用已配置的 API 模型服务；不可用时展示文档检索结果 |
| 本地 | 使用已配置的自建模型服务；未配置或不可用时展示文档检索结果，不会自动申请 GPU |
| 仅文档 | 不调用模型，展示匹配的已发布说明及已选任务的授权只读信息 |

请以回答中实际显示的模式、引用和降级提示为准。看到“本地”选项不表示自建 GPU 服务已经启用；额度是否可用取决于所配置服务，助手不承诺剩余额度或平台月预算硬限制。

回答不贴合时，请核对实际回答模式和文档标题，补充具体功能、报错与操作步骤后重问；排查任务时先选中任务，再按需同意本次日志。模型失败时仍可使用“仅文档”核对操作依据，不能把降级回答当成模型已完成诊断。模型只可复用引用中已有的受认可平台地址，命令仍需您检查后自行执行。
