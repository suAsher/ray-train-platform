...
平台启用助手后，已登录用户才会看到页面上的浮动机器人入口；没有入口不表示您的训练任务异常。可拖拽入口调整位置，点击打开问答面板；暂时不用时可关闭面板。

每次提问都是独立请求，不会自动携带此前的问题和回答，也不在服务端保存聊天历史。请在当前问题里说明功能、现象和需要确认的事项，例如“macOS 怎样安装 CLI 并登录”“日志有 loss 但页面没有曲线”。命令问题请写操作系统、使用 CLI 还是网页，并附完整错误信息；不要只问“这个怎么办”。回答附带已发布使用说明的文档标题、相关步骤和引用，可打开引用核对完整上下文；没有足够相关的资料时，助手会追问缺少的信息，不会把仅有相同词语的说明强行当成答案。

需要了解具体任务时，显式选中任务并在发送前核对任务 ID。助手沿用任务详情权限：普通成员可查询当前团队任务，平台超级管理员按既有权限查询；不能借提问绕过其他团队的访问限制。按问题需要，助手可查询状态原因、提交的资源配置、逻辑输入输出路径及 MLflow Run 关联，也可以在问题里写出一个完整的平台 job ID；多个任务 ID 时会请您只指定一个。不会读取您的源码、文件内容或内部存储凭据。提交配置不代表实时分配，状态说明不等于已证明的根因。问 GPU 配额时仅查询当前登录团队，不能当作物理空闲卡或模型 API 余额。清除任务选择后，不会沿用刚才的任务上下文。

任务日志默认不发送。只有您为本次问题勾选同意附带日志时，助手才读取选中任务的限量日志节选，并可能将其发给本次使用的模型；下一问需要重新勾选。请勿在问题或日志中放入密码、令牌和不应共享的信息。常见凭据模式脱敏并不能保证识别全部敏感信息。任务状态和日志节选只是当次观察，不能单独证明故障原因。

| 回答模式 | 使用方式与不可用时的结果 |
| --- | --- |
| 自动 | 按管理员配置的模型顺序尝试可用服务；模型不可用时提示暂不可用，不会自动改成文档回答 |
| API | 使用已配置的 API 模型服务；没有共享额度或服务不可用时提示暂不可用 |
| 本地 | 使用已配置的自建模型服务；平台按训练优先借空闲卡，训练有需求时会暂停，临时不可用时提示暂不可用 |
| 仅文档 | 不调用模型，展示匹配的已发布说明及已选任务的授权只读信息 |

请以回答中实际显示的模式、引用和提示为准。管理员未配置可用模型或验收未通过时，不显示助手入口；模型已配置后，如果本地模式临时不可用，提问接口可能返回模型暂不可用，用户也可以显式选择“仅文档”。看到“本地”选项不表示自建 GPU 服务当前可用。额度是否可用取决于所配置服务，助手不承诺剩余额度或平台月预算硬限制。

回答不贴合时，请核对实际回答模式和文档标题，补充具体功能、报错与操作步骤后重问；排查任务时先选中任务，再按需同意本次日志。文档查询会明确展示来源；没有匹配资料时，模型应追问缺少的信息。模型失败时可改用“仅文档”核对操作依据，但不能把文档结果当成模型已完成诊断。模型只可复用引用中已有的受认可平台地址，命令仍需您检查后自行执行。

助手只能解释和查询，不能提交、修改、停止或重启任务，也不能替您执行命令。需要操作时请回到相应菜单，按完整说明核对后自行操作。
...
有效期可选 1–365 天或“永不过期”。永不过期令牌仍可在账户与安全中撤销，也仍受账号和团队成员状态约束；不会自动把已有令牌改为永久有效。
