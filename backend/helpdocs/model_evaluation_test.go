...
func TestEvaluationHelpPreservesQuestionsAndExplainsRealExecution(t *testing.T) {
	...
	for id, markers := range map[string][]string{
		...
		if !ok { t.Errorf("missing evaluation question %s", id); continue }
		if article.CategoryID != "debug" { t.Errorf("unexpected category for %s", id) }
		for _, marker := range markers {
			if !strings.Contains(article.Markdown, marker) { t.Errorf("%s missing %q", id, marker) }
		}
		for _, related := range article.RelatedIDs {
			if _, exists := articles[related]; !exists { t.Errorf("%s links to missing %s", id, related) }
		}
	}
	data := articles["datasets"].Markdown
	for _, marker := range []string{"训练集", "验证集", "测试集", "0", "发布", "训练数据", "评估数据"} {
		if !strings.Contains(data, marker) { t.Errorf("dataset guidance missing %q", marker) }
	}
}
