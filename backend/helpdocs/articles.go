...
var helpArticleMetaByID = map[string]HelpArticleMeta{
	...
	"cli-onboarding-v2":       {Title: "CLI 如何安装、登录和升级？", CategoryID: "start", Category: "开始使用与账号", SortOrder: 20, Summary: "确认 CLI 服务地址，安装或升级 spk-rayjob，用 PAT 登录，排查 PATH 与帮助版本不一致。", Keywords: []string{"CLI", "spk-rayjob", "PATH", "PAT", "升级", "server", "域名", "--help", "版本"}, RelatedIDs: []string{"access", "command-recipes", "quickstart"}, LegacyTopicID: "quickstart"},
	...
	"portal-user-feature-map": {Title: "各个菜单分别能做什么？", CategoryID: "start", Category: "开始使用与账号", SortOrder: 50, Summary: "按训练、数据、调试、实验和账号目标找到 Portal 菜单入口。", Keywords: []string{"菜单", "Portal", "入口", "使用说明", "助手", "机器人", "日志同意", "仅文档"}, RelatedIDs: []string{"quickstart", "observability", "datasets"}, LegacyTopicID: "quickstart"},
	...
	"observability":                          {Title: "训练状态、日志和指标在哪里看？", CategoryID: "debug", Category: "调试与训练结果", SortOrder: 330, Summary: "查看状态、日志和曲线；排查重复日志，保留 MLflow 指标上报。", Keywords: []string{"日志", "指标", "状态", "GPU 曲线", "MLflow", "重复日志", "两行", "propagate", "MMCV"}, RelatedIDs: []string{"mlflow", "mlflow-framework-metrics", "telemetry-boundary", "artifacts"}, LegacyTopicID: "mlflow"},
	...
	modelRegistrationArticleID:               {Title: "训练权重如何保存为共享模型或同步到功能仓？", CategoryID: "debug", Category: "调试与训练结果", SortOrder: 350, Summary: "保存共享权重版本，或将模型、配置及配套文件手动、自动同步到正式功能仓；查看操作步骤、三 ID 关联和失败处理。", Keywords: []string{"模型", "权重", "版本", "快照", "SHA-256", "数据来源", "功能仓", "config", "yaml", "配置文件", "手动同步", "自动同步", "重新授权", "job_id", "run_id", "experiment_id"}, RelatedIDs: []string{"artifacts", modelMaintenanceArticleID, "datasets", "mlflow-framework-metrics"}, LegacyTopicID: "mlflow"},
	...
	modelEvaluationStartID:                   {Title: "如何用固定数据版本评估一个模型？", CategoryID: "debug", Category: "调试与训练结果", SortOrder: 370, Summary: "从权重版本选择评估方案、固定数据划分和资源，发起独立计算。", Keywords: []string{"评估", "模型", "验证集", "测试集", "val", "test", "数据版本"}, RelatedIDs: []string{modelRegistrationArticleID, "datasets", modelEvaluationResultsID}, LegacyTopicID: "mlflow"},
	modelEvaluationResultsID:                 {Title: "评估报告在哪里看，为什么不能比较？", CategoryID: "debug", Category: "调试与训练结果", SortOrder: 380, Summary: "区分任务和报告状态，查看指标，比较同一数据与方案的成功结果。", Keywords: []string{"评估", "报告", "比较", "指标", "缺失", "失败"}, RelatedIDs: []string{modelEvaluationStartID, "errors", "mlflow"}, LegacyTopicID: "mlflow"},
	modelReleaseArticleID:                    {Title: "模型如何申请审核、正式发布和回滚？", CategoryID: "debug", Category: "调试与训练结果", SortOrder: 390, Summary: "固定成功评估作为证据，由其他人审核，再发布或回滚正式版本。", Keywords: []string{"审批", "发布", "回滚", "证据", "独立审核"}, RelatedIDs: []string{modelEvaluationResultsID, modelServingUseArticleID, modelMaintenanceArticleID}, LegacyTopicID: "mlflow"},
	modelServingUseArticleID:                 {Title: "如何启动、调用、停止和切换模型推理服务？", CategoryID: "debug", Category: "调试与训练结果", SortOrder: 395, Summary: "用获批版本创建单卡服务，实际调用 JSON 接口，并停止或切换版本。", Keywords: []string{"推理", "Serving", "调用", "models:invoke", "停止", "切换"}, RelatedIDs: []string{modelReleaseArticleID, modelServingCodeArticleID, modelServingErrorsArticleID}, LegacyTopicID: "mlflow"},
	modelServingCodeArticleID:                {Title: "如何准备离线推理代码和匹配的运行方案？", CategoryID: "data", Category: "代码、环境与数据", SortOrder: 180, Summary: "准备模型适配代码、源码 ZIP、依赖镜像和输入输出约定，交由有权限的维护者登记。", Keywords: []string{"推理方案", "离线", "源码 ZIP", "serving_sdk.py", "输入输出"}, RelatedIDs: []string{modelServingUseArticleID, "custom-environment", "code"}, LegacyTopicID: "data"},
	modelRegistryArticleID:                   {Title: "共享模型如何关联到 MLflow Model Registry？", CategoryID: "mlflow", Category: "MLflow 与 API", SortOrder: 450, Summary: "将 READY 权重快照关联到 Registry，核对注册名称、版本和原生记录。", Keywords: []string{"Model Registry", "注册模型", "版本", "关联", "权重"}, RelatedIDs: []string{modelRegistrationArticleID, "mlflow-api-with-pat", modelReleaseArticleID}, LegacyTopicID: "mlflow"},
	modelServingErrorsArticleID:              {Title: "推理服务排队、未就绪或调用失败怎么办？", CategoryID: "troubleshooting", Category: "常见故障", SortOrder: 550, Summary: "区分资源排队、代码加载、健康检查和真实推理失败，并核对请求限制。", Keywords: []string{"推理", "排队", "健康", "413", "429", "超时"}, RelatedIDs: []string{modelServingUseArticleID, modelServingCodeArticleID, "scheduling-topology"}, LegacyTopicID: "troubleshooting"},
	...
	"mlflow-framework-metrics": {Title: "如何向 MLflow 记录训练参数和指标？", CategoryID: "mlflow", Category: "MLflow 与 API", SortOrder: 420, Summary: "按已有 Hook、普通 PyTorch/torchrun 和托管 Ray Train 分别接入；提供完整代码和验证步骤。", Keywords: []string{"MLflow", "指标", "参数", "MMCV", "PyTorch"}, RelatedIDs: []string{"mlflow", "telemetry-boundary", "mlflow-external-tracking"}, LegacyTopicID: "mlflow"},
	"mlflow-api-with-pat":      {Title: "如何调用 MLflow API 查询实验与 Run？", CategoryID: "mlflow", Category: "MLflow 与 API", SortOrder: 430, Summary: "按当前服务认证方式，使用原生 MLflow SDK 和 HTTP 查询实验、Run、指标历史与文件。", Keywords: []string{"MLflow API", "PAT", "search_experiments", "search_runs", "page_token"}, RelatedIDs: []string{"mlflow", "mlflow-external-tracking", "access"}, LegacyTopicID: "mlflow"},
...
var publicGuideSectionTargets = []publicGuideSectionTarget{
	...
	{guideID: "troubleshooting", heading: "训练日志为什么重复两行？", articleID: "observability"},
}
