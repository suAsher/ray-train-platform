...
| --- | --- | --- | --- |
| 手动同步：训练结束后挑选已有权重 | 任务已结束 | 开始同步 | 提交后后台开始处理 |
| 自动同步：预先指定训练成功后要上传的权重 | 任务运行中 | 保存自动同步 | 仅本任务成功结束后开始处理 |
...
- **权重文件**：将需要同步的权重写入本任务输出目录，并确认可以共享。支持 .pth、.pt、.ckpt、.onnx、.safetensors；每次 1–8 个文件，单文件不超过 20 GiB，不支持目录或通配符。同一次不能选同名文件，即使位于不同子目录。
...
2. 在要同步的权重行点击“同步到功能仓”，路径会带入表单；也可在下方同步区域点击“新建同步”，手动填写文件路径。
3. 依次选择“功能仓”和“目标模型”，填写“版本名称”，例如 epoch-12；检查“权重路径”，多个文件每行一个。
4. 确认页面显示“任务已结束”，点击“开始同步”。提交后在同一页面的同步记录中查看进度，必要时点击“刷新记录”；无需先下载到本机再上传。
5. 记录显示“同步成功”后，核对文件和三个来源 ID，点击“打开功能仓”，查看所选目标模型下的新版本及权重文件。
...
2. 选择“功能仓”“目标模型”，填写“版本名称”和预计生成的“权重路径”，例如 checkpoints/best.pth。此时文件可以尚未生成，但训练结束时必须确实存在。
...
“权重路径”相对于本任务输出目录，不是电脑上的文件路径，也不是完整挂载路径。假设代码把文件写到 PLATFORM_OUTPUT_PATH 下的 checkpoints/best.pth，表单填写 checkpoints/best.pth；直接写在输出目录根下的 model.pt，就填写 model.pt。不要填写变量名 PLATFORM_OUTPUT_PATH、目录、*.pth 或 /mnt/data/ 开头的完整路径。
...
两条流程使用同一条后台处理链路：**任务结束并满足同步条件 → 核对任务与 Run → 创建共享权重副本并校验 → 上传功能仓 → 创建目标版本 → 查询并核对文件和三个 ID → 同步成功**。

浏览器负责选择和发起，文件由训练平台后端从权重副本传给功能仓，不占用额外训练 GPU。大文件使用分片上传，需要等待处理。复制完成前请保留原文件并避免修改。平台自动生成的共享模型副本可在「实验中心 → 模型」找到；副本对平台成员共享，原文件和原目录权限不变，目标仓仍按自身权限控制访问。
...
| 排队同步 / 上传权重中 / 登记功能仓版本中 | 等待并刷新原记录，不重复点击新建；大文件处理时间更长 |
