...
打开「我的训练任务 → 本人任务详情 → 训练产物」，选中权重后点击「同步到功能仓」。选择自己有写权限的功能仓和模型类型，填写版本名，确认权重相对路径，例如 checkpoints/best.pth。用户入口固定连接正式功能仓，无需选择环境、填写接口地址或提供令牌。每次最多8个文件，各文件的名称不能重复。

训练运行中可在此填写预计生成的权重相对路径，点击“保存自动同步”；成功结束后平台自动复制、校验、上传并登记，无需一直打开浏览器。已结束任务点击“开始同步”即可；失败任务的 checkpoint 只在你主动选择时同步，不会被当作训练成功后的自动成果。每次主动发起都允许创建新版本，同一 Job 或 Run 可以多次选择不同文件。不需要先到实验中心手工登记模型或申请发布。
