...
submit_native_ray_job() (
  ...
  while [[ -z "$platform_job" && "$(date +%s)" -lt "$deadline" ]]; do
    ...
    jq -e '(.items // .data.items) | type == "array"' <<<"$jobs" >/dev/null || e2e_die 'native submission mapping returned malformed JSON'
    matches="$(jq -c --arg id "$external_id" '[((.items // .data.items)[]) | select(.externalSubmissionId == $id and .submissionOrigin == "ray-cli")]' <<<"$jobs")"
    match_count="$(jq 'length' <<<"$matches")"
    [[ "$match_count" -le 1 ]] || e2e_die 'native submission identity mapped to multiple persisted jobs'
    if [[ "$match_count" -eq 1 ]]; then
      platform_job="$(jq -er '.[0].id' <<<"$matches")"
    fi
...
reconcile_submission_intent() {
  ...
  identity="$(submission_identity_for_flow "$flow" "$name")"
  case "$flow" in
  ...
  if [[ "$flow" == native-ray ]]; then
    matches="$(jq -c --arg identity "$identity" --arg origin "$expected_origin" --arg namespace "$TENANT_NAMESPACE" '[((.items // .data.items)[]) | select(.externalSubmissionId == $identity and .submissionOrigin == $origin and .kubernetesNamespace == $namespace)]' <<<"$jobs")"
  else
    matches="$(jq -c --arg name "$name" --arg origin "$expected_origin" --arg namespace "$TENANT_NAMESPACE" '[((.items // .data.items)[]) | select(.spec.name == $name and .submissionOrigin == $origin and .kubernetesNamespace == $namespace)]' <<<"$jobs")"
  fi
...
submit_acceptance_job() {
  ...
  identity="$(submission_identity_for_flow "$flow" "$name")"
  ledger_record submission-intent "$identity" "$TENANT_NAMESPACE" "$identity"
  case "$flow" in
    portal)
