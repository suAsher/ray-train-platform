...
缓存同时支持 Ray 编排 DDP 与 Ray Train 托管；它是数据模式，不是训练引擎。托管引擎的 `mount`、`cache`、`ray-data-stage` 选择及性能指标见 [Ray Train 托管指南](RAY_TRAIN_MANAGED_GUIDE.md)。
...
2026-08-29 又使用 Ray Train 2.56.1 的 `ray-data-stage` 对 20,010 个真实 S1H 文件（19.03 GB）复测：直接挂载完整读取为 93.50 秒、203.5 MB/s；Ray Data 预热为 134.91 秒；预热后的双 NVMe 完整读取为 1.24 秒、15.38 GB/s。短任务不一定收回预热成本，长训练和多 epoch 才是主要使用场景。
