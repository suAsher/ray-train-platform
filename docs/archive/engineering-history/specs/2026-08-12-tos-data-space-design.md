...
训练和调试任务不能写入团队/公共根。V1 中，TenantAdmin 可在页面直接维护团队共享根，SuperAdmin 可直接维护公共根；两者在任务中仍以只读路径出现。后续可增加“从个人空间发布”的异步复制工作流，但它不是当前上线前置条件。
...
- 现有控制面对象存储凭据只保存在平台 Secret，用于初始化目录、受控列举和签发上传请求；绝不注入训练或调试容器。
- VKE FSX CSI 使用 IRSA 作为正式工作负载挂载鉴权。当前 VKE csi-fsx v1.1.1 的 IRSA 是**组件级**角色：PV 省略 `secretName` 后由 CSI 组件角色访问 TOS，而不是由训练 Pod 的 ServiceAccount 选择角色。平台为每一个用户空间生成只指向个人前缀的受控 PV/PVC；训练 Pod 不持有 TOS API 凭据，且用户没有创建/查看其他 PV、PVC 或 Pod 的 Kubernetes 权限。
- CSI 组件角色只允许访问 `ray-train/` 平台前缀；用户隔离由“受控的单前缀 PV/PVC + 仅平台可创建工作负载 + 容器只挂载授权卷”共同保证。团队/公共空间使用只读 `volumeMount`，IDC NFS 同样显式只读。
- 如以后需要云 IAM 层面的每用户前缀策略，应改用平台签发、自动刷新的 STS 挂载辅助组件；不得把长期凭据写入任务 Spec 或用户容器环境。
...
火山文档说明 FSX CSI 可将 TOS 作为 POSIX 文件系统挂载，支持指定前缀；csi-fsx 启用 IRSA 后，PV 可以省略访问密钥并使用组件绑定的 IAM 角色。实现前必须以当前 VKE 组件版本作一次双前缀实际挂载验收，而不是只依赖模板。 [VKE FSX TOS 静态卷](https://www.volcengine.com/docs/6460/1828107) [使用 IRSA 实现存储挂载鉴权](https://www.volcengine.com/docs/6460/1861517)
...
- 平台记录每次空间初始化、挂载预检、目录维护、上传和训练路径解析的审计事件，绝不记录 Secret、Token 或预签名 URL。
...
3. 实现数据空间领域模型、受控文件浏览/上传/目录维护 API 和“我的数据”页面；“从个人空间发布”的异步复制功能后置。
