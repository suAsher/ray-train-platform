...
**Architecture:** TOS 是用户代码、数据、训练输出的唯一持久化来源。后端从已认证的 `tenant + subject` 派生逻辑数据空间；TOS、PVC、FSX CSI 都是后台实现。当前 VKE csi-fsx 的 IRSA 是组件级 IAM 角色，平台只能创建指向单个授权前缀的 PV/PVC，不能把每个训练 Pod 映射为一个 IAM 角色。任何数据挂载先经真实的双前缀 IRSA 挂载验证，失败时拒绝创建含数据空间的 RayJob 或调试环境。
...
创建 `40-irsa-prefix-smoke.yaml` 模板，含两个 `fsx.csi.volcengine.com` 静态 PV/PVC 和两个 Pod。当前 CSI 的 IRSA 角色绑定于 csi-fsx 组件，因此不创建虚假的工作负载 ServiceAccount 角色映射。两个根分别是：
...
模板只接收非敏感 `FSX_TOS_SERVER`、`FSX_TOS_REGION`、`FSX_TOS_BUCKET`；严禁 `secretName`、AK、SK。每个 Pod 仅挂自己的 PVC，并执行：
...
为 csi-fsx 组件配置一个 IAM Role：允许 `ray-train/` 下的挂载所需操作，不授予桶管理权限；PV 必须省略 `secretName`。脚本除验证 A/B 各自写入外，必须验证每个 Pod 的挂载根不能通过 `..` 访问另一个测试前缀。重跑：
...
`Get/List` 只能返回当前 subject 的个人绑定、当前 tenant 的共享绑定、公共绑定和可见 IDC 绑定；空 `user_id` 不能误作个人空间。对不存在的个人 binding，服务先创建数据库 `PENDING` 记录；仅在 Task 1 的 IRSA 模板契约可用时，才由平台创建对应的单前缀 PV/PVC 并将状态更新为 `READY`。CSI 组件的 IAM Role、信任策略和 `ray-train/` 前缀策略属于云账号权限面，必须由具备 IAM 权限的管理员完成；平台不能以长期 TOS Secret 模拟该步骤。
