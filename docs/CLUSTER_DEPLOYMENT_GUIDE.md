...
这套部署目标是：VKE + KubeRay + Kueue + PostgreSQL + Keycloak + Loki，在 3 台 RTX 4090×8 节点上提交真实 RayJob；另提供 1 台调试节点对应的单卡 RayCluster。平台不会接受用户提交的 LDAP 密码，LDAP 只作为现有 Keycloak 的 User Federation。
...
- V1 依赖运维预先创建租户 namespace、LocalQueue 和 IDC RWX PVC；平台不会根据用户输入创建任意 namespace、StorageClass 或 hostPath。

## 1. VKE 前置条件

### 1.1 节点和 GPU

确认 NVIDIA device plugin 已安装，并给 3 台训练节点打标：

```bash
kubectl label node <worker-1> <worker-2> <worker-3> accelerator=nvidia-rtx-4090 --overwrite
kubectl get nodes -L accelerator
kubectl get nodes -o custom-columns=NAME:.metadata.name,GPU:.status.allocatable.nvidia\\.com/gpu
```

Ray worker Pod 会固定选择 `accelerator=nvidia-rtx-4090`。不要把调试节点加入训练 label，避免调度器把 24 卡任务放到调试机。
...
V1 使用运维预先验证的 RWX PVC。训练任务和调试工作区位于租户 namespace，因此每个租户 namespace 都要有同名 claim，或者由 IDC CSI 的团队模板预创建：

```bash
kubectl -n tenant-team-a get pvc idc-training-rwx
kubectl -n tenant-team-a get pvc idc-training-rwx -o jsonpath='{.status.phase}{" "}{.spec.accessModes}{"\n"}'
```

必须确认 VKE 到 IDC 的专线/VPC 路由、DNS、MTU、读写权限和多节点并发访问。平台只挂载 `IDC_EXISTING_CLAIM`，不会创建 hostPath，也不会把用户输入转换成宿主机路径。
