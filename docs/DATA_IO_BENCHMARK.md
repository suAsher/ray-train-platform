...
223.146 秒，聚合源到缓存吞吐为 22.223 MiB/s。以这一次实测估算：

| 口径 | 持久数据直读 | NVMe 预热后读取 | 墙钟差异 |
| --- | ---: | ---: | ---: |
| 读取 1 遍 | 259.093 s | 223.146 + 0.882 = 224.028 s | 缩短约 13.5% |
| 读取 2 遍 | 259.093 + 43.178 = 302.271 s | 223.146 + 0.882 + 0.953 = 224.981 s | 缩短约 25.6% |

这不代表所有任务读取一遍都能回本：两组测试按顺序执行，且没有清理宿主机、FSX 或
对象存储缓存。正式结论必须在相同 manifest 上重复多轮，并把预热时间计入任务总墙钟。
epoch 更多、小文件更密集或远端读取长尾更严重时，缓存收益通常更容易摊销。
