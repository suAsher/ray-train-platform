...
- 原生 Dashboard 展示全平台实验；用户已接受跨团队可见。
...
- 原生界面只读：允许查看、搜索、比较实验和指标；禁止创建、修改、删除、恢复、注册模型和下载 Artifact。

## 方案选择

采用“平台鉴权访问票据 + 同域只读反向代理”。

未选择的方案：

1. 独立 `mlflow.*` 域名需要新增 IDC 代理、DNS、证书和 Keycloak Client，并会形成第二套登录流程。
2. 直接把 MLflow Ingress 暴露给内网无法复用平台身份，且会开放全局写操作和 Artifact API。
...
- 允许 `GET`、`HEAD`、`OPTIONS` 访问页面、静态资源和无副作用查询接口。
- 只允许 MLflow 原生 UI 查询所需的少量 `POST` 搜索接口，例如实验搜索和 Run 搜索。
- 拒绝实验、Run、Tag、模型注册表的创建、更新、删除和恢复接口。
- 拒绝全部 `/api/2.0/mlflow-artifacts/`、Artifact 下载、上传和代理接口。
- 对 MLflow JSON 响应移除 `artifact_uri`、`artifact_location` 等存储定位字段。
- 限制搜索请求体大小、响应体大小和上游超时；不把上游内部错误原样返回给浏览器。

如果 MLflow 升级后原生 UI 新增查询接口，默认结果是被拒绝；必须补充测试和显式允许后才能上线。
...
6. 更新 MLflow 运维和用户文档，说明实验中心与原生只读 Dashboard 的用途和权限差异。
...
- 创建、修改、删除、恢复、注册模型及全部 Artifact 接口被拒绝。
- JSON 响应不包含 Artifact URI、Bucket、AK/SK 或数据库地址。
...
- 页面中的删除或修改请求被拒绝，Artifact 不能下载。
- 原有实验中心、任务详情、训练上报和 2×8 训练链路不回归。

## 发布与回滚

发布顺序：后端只读代理与测试 → 前端按钮 → MLflow `static-prefix` → 构建不可变镜像 → Helm 滚动升级 → 浏览器与接口验收。
