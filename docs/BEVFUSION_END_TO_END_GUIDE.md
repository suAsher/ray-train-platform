...
当前租户的团队配额是 16 GPU，因此一个 2×8 任务会占满团队 GPU 配额。现行提交 API 会先做团队实时配额门禁：此时同一团队再提交 2×8 会以 `tenant GPU quota exceeded` 拒绝创建，而不是进入 Kueue；只有已经通过平台门禁但等待物理资源的任务才显示为排队。1 卡探针和 2×8 正式任务也共享这 16 GPU 配额，提交长任务前应先确认没有遗留的 `RUNNING`、`PROVISIONING` 或已准入任务。
