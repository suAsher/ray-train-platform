...
| 角色 | 能做什么 |
|---|---|
| `SuperAdmin` | 全平台：建租户、分配团队 GPU 额度、登记共享镜像、管理公共数据、查看和停止所有团队任务 |
...
| 镜像 | 支持显式 tag（如 `:cuda121-v3`）或不可变 `@sha256:` digest；不允许省略 tag |
| 框架标注 | 可选，帮用户快速识别 |
| 设为默认 | 每种用途只有一个默认，表单会预选它 |
| 全平台共享 | 只有 `SuperAdmin` 能勾，勾了所有团队都能用 |

已有镜像也可以修改范围：`SuperAdmin` 在镜像所在行点击 **设为全平台**；共享镜像可点击 **改为本团队** 收回到当前管理员所属团队。范围变更立即影响新提交，已经运行的任务不受影响。TenantAdmin 只能登记和删除本团队镜像，不能修改全平台范围。

生产目录建议只保留少量经过验收的条目：一个默认调试镜像、一个默认训练镜像，以及确有兼容用途的基础 PyTorch/Ray 镜像。两个 BEVFusion 分支共享同一个运行环境，代码随任务上传，不需要为每个分支各登记一个镜像。删除目录条目只会阻止后续选择，不会删除 Harbor 镜像层，也不会影响已经创建的任务。

### tag 与 digest 怎么选

平台允许 tag，带 tag 的任务每次启动都会重新拉取镜像，适合频繁迭代的开发环境。正式基线仍推荐 digest：`:latest` 或可变 tag 今天和三个月后可能是两个不同环境，无法严格复现。构建脚本结束时会打印 digest，可直接登记。
...
租户 GPU 配额是并发预算，不是节点探测结果。平台从带训练标签且 Ready 的节点动态计算物理容量；新增并正确标签 GPU 节点后，平台会自动扩大物理容量与 Kueue ClusterQueue。是否给某个团队增加额度，仍由 SuperAdmin 在页面决定。
...
SuperAdmin 的“集群算力”和“队列与运行中”是全平台视图，任务列表包含所有 `tenant-*` namespace。界面同时区分两种数字：

- **任务申请 GPU**：数据库中运行/准备状态任务的资源申请；
- **物理已分配 GPU**：Kubernetes 中仍存在的 GPU Pod 申请。

任务刚结束后，RayCluster 会在清理 TTL 内继续存在，因此“物理已分配”可能暂时大于“任务申请”。这时界面会标为“含释放中”，不是新增任务，也不是团队配额统计错误。

每个账号当前只属于一个团队，登录态中的 `tenantId` 决定普通操作的数据与任务边界。SuperAdmin 不需要切换团队即可查看和停止全平台任务。若未来需要一个普通账号加入多个团队，应新增成员关系和“当前团队”选择机制；不要直接改写现有登录态的租户字段。
