...
## 10. 管理平台使用说明（仅 SuperAdmin）

管理员清理版本化数据集失败记录见下文第 11 节；该功能不涉及数据文件删除。

登录 Portal → **使用说明** → **管理文档**。只有超级管理员的本地账号或 OIDC 登录会话可操作；团队管理员、普通用户及 PAT/CLI 机器令牌均不能新增、编辑、发布或查看历史。后端独立校验权限。

### 新增或修改

1. 点“新增文档”，或在管理窗口左侧选择现有文档。
2. 填写 ID、标题、分类和排序。ID 为 1–80 位小写字母、数字和连字符，创建后不可改；文档链接为 `/help#<id>`。排序数字越小越靠前。标题最多 200 字，分类最多 100 字，正文最多 512 KiB。
3. 用 Markdown 编写内容，切到“预览”核对效果。支持标题、列表、表格、链接和代码块；不支持原始 HTML、脚本链接、图片上传或远程图片。代码块可复制，示例模板可加文件名：
...
4. 点“保存草稿”。新建文档此时**对普通用户不可见**；编辑已发布文档时，用户仍看到上一次发布的正文。
5. 保存后点“发布”，确认后全平台可见。搜索及“下载为 Markdown”只使用已发布版本；已有页面需重新加载才能看到其他管理员的新发布。

### 下架、历史与冲突处理

- **下架**：从用户阅读、搜索与下载中移除，保留草稿及历史，不永久删除。
- **版本历史**：查看每次创建、保存、发布、下架、恢复的版本、操作人及时间；选择“查看”预览历史正文。
- **恢复为草稿**：把选中的历史内容复制成新的草稿版本，不覆盖历史，也不自动发布。核对后再点“发布”。
- **版本冲突（409）**：另一位管理员已修改文档，服务器拒绝覆盖。当前编辑内容会保留；先“下载当前草稿备份”，刷新列表并重新选择该文档，核对最新版本后合并修改再保存。
- 关闭管理窗口或离开页面时，有未保存内容会提示。浏览器崩溃、强制退出不能保证恢复，请及时保存草稿或下载备份。

### 运维与初始化

后端迁移 `0032_help_documents` 建立 `help_documents`、`help_document_revisions`。首次启动按稳定 ID 导入内置 22 篇已发布说明；后续启动只补缺失 ID，**不覆盖管理员编辑、不重新上架已下架内容**。持久化内容以数据库为准；后续修改仓库中的静态说明不会自动覆盖线上文档。

数据库备份需包含这两张表。只部署 API/UI，不涉及训练镜像或 RayJob。若前端提示文档服务不可用，先检查 API 版本、迁移及初始化日志；不要用静态旧文档兜底，否则可能重新暴露已下架内容。API 对每个实例的每位用户限制读请求 120 次/分钟、管理写请求 30 次/分钟；收到 429 后按提示稍后重试。

## 11. 清理版本化数据集失败记录（管理员）

在“版本化数据集”中，失败版本默认收起，可通过“显示失败记录”查看。普通用户只能查看；超级管理员可管理全部数据集，团队管理员仅能管理本团队的 TEAM 数据集，不能清理公共数据集。PAT/CLI 机器令牌不能执行删除。

1. 展开失败记录，在 FAILED 版本上点删除；或勾选失败版本后批量删除（每批最多 100 条）。发布任务记录也可单独删除。
2. 确认提示中的范围。这是**记录软删除**：移出目录并保留删除人、时间及审计信息，不删除 `/mnt/storage/public`、团队原始数据、Parquet 分片、清单或训练任务，**不释放存储容量**。
3. 服务端在删除时再次检查：版本与发布记录必须失败，没有活动发布工作，且未被任何训练任务引用（包括已结束或归档任务）。不满足时返回409并保留记录；跨团队或不可见记录返回404。
4. 批量操作会分别报告成功和失败项；失败项保留，可刷新核对原因后再试。不要为了删除记录而停止训练或删除发布 Pod。

软删除不会腾出版本 ID 或版本名称；新发布需使用新版本身份，以免迟到回调复活旧版本。当前不提供前台恢复入口，如误操作可由集群管理员核对审计后恢复数据库元数据。对象文件残留的物理清理不在此功能内，需要单独审批受控清理，不会因点“删除记录”自动执行。
