...
2026-09-13模型审批、Registry 与推理服务已部署：后端业务提交 `0ad8dba`，后续部署配置 `b8567f4` 修复代码快照临时卷，revision226/schema52；Portal dev `525de83b`、CI33893/revision1052。共享 Registry 复制、权重校验及 Run 跳转已完成生产验收；源码ZIP上传、评估方案登记及固定来源预检已通过；当前local配额24/24，独立评估成功报告、非自审和实际推理链路仍待空闲GPU验收，见[发布与验收记录](MODEL_RELEASE_SERVING_VALIDATION_20260913.md)。

2026-09-13 BEVFusion–MLflow 重复日志已在平台适配源码修复，并提供保留用户改动的升级工具；构建机真实依赖 CPU 测试验证两行变一行且指标仍上报。此修复需更新源码后重新打包提交，不改变运行中任务或旧快照，不发布后端或训练镜像；证据及边界见[重复日志修复记录](BEVFUSION_CODE_CHANGES.md#8-2026-09-13-mlflow-重复日志修复)。

2026-09-13 后续比较历史 LiDAR 与当前 Fusion 任务，确认相同镜像和相关源码下，MMCV/MLflow 日志初始化顺序不同导致 Epoch 日志表现不同；不能归因为近期平台发布引入。用户所需排查、处理示例和保留指标的方法已补入平台“使用说明”的既有日志与 MLflow 指标文章。帮助内容后端 `876ee4c` / `release-20260913-03-876ee4c` 已部署，Helm revision **227**，无数据库迁移；Portal 和训练镜像未变。这次部署发布的是帮助内容，不会修复运行中任务的日志；证据见[帮助发布记录](HELP_CONTENT_COVERAGE_20260912.md#2026-09-13-重复日志用户帮助发布)。

2026-09-14 功能仓同步已发布并完成正式私有验收仓的手动按钮、真实权重下载校验和“运行中配置→训练成功→自动上传”验收。后续手动/自动流程帮助已发布至后端 Helm234/schema53（release-20260914-06-80b147b），Portal dev `77d266b7`（CI33944成功）；用户入口固定正式功能仓，平台使用说明已更新。验收、源代码版本及长期授权等边界见[功能仓发布记录](FUNCTION_WAREHOUSE_INTEGRATION_PLAN_20260914.md#15-实际发布与用户入口收敛)。
...
2026-09-16 当前团队任务与实验可见范围已发布：普通成员可查看所属 RayTrain 团队的任务、指标和关联 Run，Portal 保留“仅我的”筛选；GPU 资源池向所有登录成员开放只读，其他团队工作负载标识隐藏。后端业务提交 `5633f31`，组合构建 `8168c7c` / `release-20260916-01-8168c7c`，Helm **242** / schema **54**；Portal dev `b3601dcf` / CI34082 全成功。当前管理员会话浏览器验收通过，普通角色按用户选择使用隔离测试验证，未声称真实普通 SSO 账号验收。平台使用说明同步更新，见[团队可见范围发布记录](QUOTA_MLFLOW_VALIDATION_20260912.md#2026-09-16-当前团队训练记录与-gpu-资源池)。
...
2026-09-13普通文件上传405排障及Portal修复见[上传验证记录](PORTAL_UPLOAD_VALIDATION_20260913.md)：普通与分片XHR漏加网关前缀，与评估ZIP代码包和浏览器工具读取本机文件限制是不同问题。
