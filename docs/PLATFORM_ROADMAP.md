...
- 两个 BEVFusion 分支的 2×8 GPU 训练已有可复现验收基线；双 NVMe 任务级缓存和输入预热已完成首版交付。
...
| 已交付 | 节点 NVMe 缓存 | 按任务选择临时 NVMe，默认关闭且不改变现有训练 | 可靠性基线、性能基线、平台提交 API | off/runtime、双盘临时卷和输入预热已上线；按相同负载 A/B 验证收益 |
...
1. GPU 节点 `/data1/ray-cache`、`/data2/ray-cache` 全局具备供应能力，用户按任务选择 `off` 或 `runtime`，默认关闭；
2. 管理员容量 allowlist 由生产 Profile 管理，当前为 200Gi / 500Gi / 1Ti / 2Ti / 4Ti / 5Ti，默认 200Gi、最大 5Ti；Web、`spk-rayjob` 和平台网关使用同一契约；
3. `runtime` 为 Ray Head/Worker 挂载 `/mnt/cache`、`/mnt/cache2`，注入 `PLATFORM_CACHE_PATH`，并配置 Ray temp-dir 和 object spilling；显式 `preload: input` 时，Worker 自动预热所选输入并切换 `PLATFORM_DATASET_PATH`；
4. 本地卷容量门禁、回收、监控和告警；
...
- 跨任务共享 dataset 模式、基于不可变数据集 manifest/ETag 的命中复用仍未开放；当前预热流程是任务级独立副本，收益须满足性能基准门禁；
