...
## 2026-09-13 重复日志用户帮助发布

用户要求操作指导必须出现在平台“使用说明”。本次将重复日志排障正文嵌入帮助服务，加入已有 `observability`（训练状态、日志和指标在哪里看？），并从 `mlflow-framework-metrics`（如何向 MLflow 记录训练参数和指标？）提供关联入口。保留六分类、43 篇独立问题文档和既有正文、自定义内容、旧七主题接口；没有新增管理员用户文档。

正文覆盖：原始 Worker 输出与页面对比、rank/PID 与 Hook 数量检查、相同镜像下初始化顺序不同的解释、源码维护者确认重复传播后的最小处理示例、保留 MLflow Hook/指标/Run 生命周期、重新打包新任务与旧任务不变的边界、指标及文件日志验证、向支持人员提供哪些非敏感证据。平台不修改用户源码，也不靠文本去重隐藏真实输出。

### 版本及验证

- 后端源码：`876ee4ce005984637954cdb4e8832f888dedf1bd`，发布前本地、GitHub、内部 GitLab 和正式构建目录一致。
- 后端镜像：`release-20260913-03-876ee4c`，digest `sha256:fcba340a9ca7ea3f306578c270e3990a8395481ba3a5989dc0df84e2c506c23f`。Helm revision **227**，2026-09-13 19:58:29 北京时间部署。
- 两个实际后端 Pod imageID 与该摘要一致，Ready、重启数 0；healthz 200。无数据库迁移，前端与训练镜像未构建或部署。
- 构建机先验证缺失正文的回归失败，再通过帮助测试、格式检查和完整 Go 测试；本次未配置真实 PostgreSQL 集成库，不将其跳过算作通过。
- Helm server dry-run 仅后端镜像一行变化。发布前后的全部 RayJob、RayCluster、tenant 命名空间 Pod UID、状态、重启数快照一致。
- 登录 Portal 的同域帮助 API 返回 200、43 篇；日志文章包含新增标题、处理代码及指标文章链接，指标文章包含返回日志排障的链接。API 读取已通过；浏览器刷新、搜索和点击正文的人工路径验收暂被自动审批阻止（无法确认按钮目标），待解除后补验，不以 API 读取替代 UI 验收。

构建机证据目录：`/root/raytrain-release-20260913-help-logging`，保存测试、构建、最小覆盖、manifest diff、部署日志及资源前后快照。此次上线的是用户帮助；适配代码修复仍需源码维护者采纳后重新打包，运行中的训练不会自动消除重复日志。评估、审批、推理生产闭环的待验收项不因此变成已完成。
