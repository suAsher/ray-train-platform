...
GPU 卡型由当前团队的资源池自动决定。用户不需要在网页、`.spk-rayjob.yaml` 或命令行中选择卡型，原有提交方式不变。旧版客户端仍可能发送 `acceleratorClass`，后端会用团队策略覆盖它；同一分布式任务的所有 Worker 会固化为同一卡型，不会混用 4090、A100、A800 或 H20。若旧模板里仍有 `acceleratorClass: rtx4090`，或日志里出现资源名 `nvidia-rtx-4090`，它们只用于兼容和排障识别，不是新任务需要手工填写的字段。
...
本次仅开放 Linux/macOS 自更新。Windows 可手动安装新版 CLI，但 `upgrade` 暂不执行替换，需按集群外提交页重装；下文 Windows 退出后替换机制待实机验收后才会启用。

平台「使用方法 → 完整 CLI 上手与升级：spk-rayjob / 原生 Ray」提供 Linux、macOS、Windows 的安装、登录及两套提交命令，并覆盖镜像、入口、CPU/内存/GPU、队列、数据版本、场地、日志、取消和续训。所有占位值均须替换为当前账号有权使用的值；spk-rayjob 队列由平台决定，原生 Ray 自定义资源须填完整六项资源元数据与授权队列名。

支持新版升级协议的客户端在 `login` / `submit` 时向 stderr 提示更新，不自动覆盖本机文件。手工运行 `spk-rayjob upgrade`，从已登录 server 下载并验证同域 SHA256 清单；无 upgrade 子命令的旧客户端第一次须按外部提交页重新下载安装。没有 minimumVersion 就没有强制最低版本要求，并非每次平台发布都需要升级客户端。
