...
对接方希望查询和写入平台现有的全部共享 MLflow，使用原生 MLflow SDK。无需先创建“外部实验”，无需把已有训练重新导入另一套接口。本页对应的原生程序入口须以本次发布验收记录为准；此前仅 `/mlflow/` 浏览器共享入口及受限 SDK 子集已发布。
...
`mlflow:full` 表示共享 MLflow 的完整读写能力，包含修改、删除和模型注册表操作。它与现有共享网页的资源范围一致，不按平台用户或训练 Job 过滤。旧令牌不会自动获得该权限。平台训练任务、个人目录和调度仍使用各自权限。
...
上传普通文件用 `log_artifact`；保存符合 MLflow 格式的模型用对应框架的 `log_model`。Model Registry 用 `create_registered_model`、`create_model_version` 等原生接口管理版本。注册版本不会自动部署推理服务，也不代表已经经过平台独立评估和审批。已有实验若显式配置了非代理 artifact URI，其文件仍按原存储配置访问，本次不会迁移旧文件。
...
- 原生共享入口能看到上游全部实验；可选平台 REST/六方法 SDK 只返回本人或明确授权的记录。其受控产物独立存放，不会自动变成原生 Run 的 Artifact 文件。
...
本轮集成身份、受控文件和六方法 SDK 的既有生产证据见[验收记录](MLFLOW_INTEGRATIONS_VALIDATION_20260912.md)。实际对接方的账号、凭据交付与运行机器网络仍需按其部署目标落实，本文不表示已经向第三方发送凭据。
